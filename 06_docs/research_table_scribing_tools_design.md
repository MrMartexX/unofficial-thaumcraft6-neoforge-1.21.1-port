...
| `thaumcraft.api.research.theorycraft.ITheorycraftAid`, `AidBookshelf`, `AidEnchantmentTable`, and `AidBeacon` | A 9x9x3 block area around the table plus entity range `5` is checked for aid objects. Selected aids reduce starting inspiration by one each and append their card ids to `aidCards`; `AidBookshelf` contributes `CardBalance`, two `CardNotation`, and three `CardStudy` entries. `AidEnchantmentTable` contributes `CardEnchantment`; `AidBeacon` contributes `CardBeacon`. |
...
| Table-top renderer | `TCResearchTableRenderer` registered through `TCBlockEntityRenderers` | Uses a modern BlockEntity renderer for the legacy visible table state: the scroll/tube appears only while theory data exists, and the inkwell plus quill appear while usable scribing tools are in slot `0`. Plain paper in slot `1` is not rendered as a vanilla paper item; legacy used the scroll model when theory work started. |
...
|---|---|
| Full legacy theorycraft card set | The public/API slice, dependency-free bridge cards, first item-consuming aspect-crystal/phial cards, first vanilla XP/aid cards, first vanilla-item/table-inventory Golemancy cards, and first Artifice item-option cards exist. Cards tied to curio, warp, celestial notes, portals, focus/caster runtime state, infusion/golem gameplay beyond `CardChannel`/`CardSculpting`, and other unported subsystems must wait for their target systems or explicit bridge policy. |
...
| Remaining research aids | Bookshelf, enchanting table, and beacon are active. Brain-in-a-jar, glyphed stone, portals, basic category aids, and any entity/item aids need their target blocks/entities/cards audited and ported by dependency family. |
...
4. Visually tune the table-top renderer against 1.12.2 screenshots or a live 1.12 instance, especially scroll position/scale and quill angle.
