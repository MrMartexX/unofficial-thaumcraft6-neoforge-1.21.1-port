...
| `thaumcraft.api.research.theorycraft.ITheorycraftAid`, `AidBookshelf`, `AidBasicAlchemy`, `AidBasicArtifice`, `AidBasicInfusion`, `AidEnchantmentTable`, `AidBeacon`, and `AidDragonEgg` | A 9x9x3 block area around the table plus entity range `5` is checked for aid objects. Selected aids reduce starting inspiration by one each and append their card ids to `aidCards`; `AidBookshelf` contributes `CardBalance`, two `CardNotation`, and three `CardStudy` entries. `AidBasicAlchemy` contributes `CardConcentrate`, `CardReactions`, `CardSynthesis`; `AidBasicArtifice` contributes `CardCalibrate`, `CardTinker`, `CardMindOverMatter`; `AidBasicInfusion` contributes `CardMeasure`, `CardChannel`, `CardInfuse`. `AidEnchantmentTable` contributes `CardEnchantment`; `AidBeacon` contributes `CardBeacon`; `AidDragonEgg` contributes `CardDragonEgg`. |
...
| Theory card registry | `TCTheorycraftManager` with first core-card slice plus bridge cards | Registers the public API/core legacy card ids for `CardStudy`, `CardAnalyze`, `CardBalance`, `CardNotation`, `CardPonder`, `CardRethink`, `CardReject`, `CardExperimentation`, and `CardInspired`. The common-card bridge adds dependency-free theory-total cards (`CardMeasure`, `CardCalibrate`, `CardFocus`, `CardSynergy`), aspect-crystal/phial cards (`CardConcentrate`, `CardReactions`, `CardSynthesis`, `CardChannel`, `CardInfuse`), the first vanilla XP/aid cards (`CardSpellbinding`, `CardEnchantment`, `CardBeacon`, `CardDragonEgg`), the first vanilla-item Golemancy cards (`CardSculpting`, `CardScripting`), and the first Artifice item-option cards (`CardTinker`, `CardMindOverMatter`). |
| Research aids | `TCTheorycraftAid`, `TCTheorycraftManager#collectNearbyAidKeys`, bookshelf/basic-block/enchanting-table/beacon/dragon-egg bridges, and minimal screen aid selection | Bookshelf, crucible, arcane workbench, infusion matrix, enchanting table, beacon, and dragon egg detection use the legacy 9x9x3 block scan, and selected aid keys are sent in the start-theory payload. Server revalidates selected keys against nearby aids and keeps the legacy `selectedAids.size()+1 < inspirationStart` limit before adding aid cards. Basic Auromancy/Golemancy/Eldritch aids remain deferred because their card/block dependencies are not safe yet. |
...
|---|---|
| Full legacy theorycraft card set | The public/API slice, dependency-free bridge cards, first item-consuming aspect-crystal/phial cards including `CardInfuse`, first vanilla XP/aid cards including Dragon Egg, first vanilla-item/table-inventory Golemancy cards, and first Artifice item-option cards exist. Cards tied to curio, warp, celestial notes, portals, focus/caster runtime state, infusion/golem gameplay beyond `CardChannel`/`CardInfuse`/`CardSculpting`, and other unported subsystems must wait for their target systems or explicit bridge policy. |
...
| Remaining research aids | Bookshelf, basic Alchemy, basic Artifice, basic Infusion, enchanting table, beacon, and dragon egg are active. Brain-in-a-jar, glyphed stone, portals, basic Auromancy, basic Golemancy, basic Eldritch, and any entity/item aids need their target blocks/entities/cards audited and ported by dependency family. |
...
3. Port the next safe advanced cards by dependency family, starting with cards that do not require unported subsystems. `CardAwareness`/Basic Auromancy needs a warp policy first.
