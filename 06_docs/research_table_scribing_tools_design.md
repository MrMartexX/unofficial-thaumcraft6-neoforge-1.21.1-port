...
| `thaumcraft.api.research.theorycraft.ITheorycraftAid`, `AidBookshelf`, `AidEnchantmentTable`, `AidBeacon`, and `AidDragonEgg` | A 9x9x3 block area around the table plus entity range `5` is checked for aid objects. Selected aids reduce starting inspiration by one each and append their card ids to `aidCards`; `AidBookshelf` contributes `CardBalance`, two `CardNotation`, and three `CardStudy` entries. `AidEnchantmentTable` contributes `CardEnchantment`; `AidBeacon` contributes `CardBeacon`; `AidDragonEgg` contributes `CardDragonEgg`. |
...
|---|---|
| Full legacy theorycraft card set | The public/API slice, dependency-free bridge cards, first item-consuming aspect-crystal/phial cards, first vanilla XP/aid cards including Dragon Egg, first vanilla-item/table-inventory Golemancy cards, and first Artifice item-option cards exist. Cards tied to curio, warp, celestial notes, portals, focus/caster runtime state, infusion/golem gameplay beyond `CardChannel`/`CardSculpting`, and other unported subsystems must wait for their target systems or explicit bridge policy. |
...
| Remaining research aids | Bookshelf, enchanting table, beacon, and dragon egg are active. Brain-in-a-jar, glyphed stone, portals, basic category aids, and any entity/item aids need their target blocks/entities/cards audited and ported by dependency family. |
