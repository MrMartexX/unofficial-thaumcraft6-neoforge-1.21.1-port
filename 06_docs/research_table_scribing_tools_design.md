...
| Table-top renderer | `TCResearchTableRenderer` registered through `TCBlockEntityRenderers` | Uses a modern BlockEntity renderer to draw paper and scribing tools from the synced table slots. The block entity now sends vanilla update packets/tags for slot visibility. This is only the first visible-top slice, not the final legacy inkwell/quill/paper animation. |
...
|---|---|
| Full legacy theorycraft card set | The public/API slice, dependency-free bridge cards, first item-consuming aspect-crystal/phial cards, first vanilla XP/aid cards, first vanilla-item Golemancy card, and first Artifice item-option cards exist. Cards tied to curio, warp, celestial notes, portals, focus/caster runtime state, infusion/golem gameplay beyond `CardChannel`/`CardSculpting`, and other unported subsystems must wait for their target systems or explicit bridge policy. |
| Polished legacy card rendering | The current screen is functional. Legacy paper draw animation, gilded aid sheets, category icon column, sparkle feedback, card hover/zoom animation, and legacy inkwell/quill treatment are still client-rendering work. |
...
4. Visually tune the table-top renderer against 1.12.2, then decide whether the inkwell/quill layer should be a rendered item, model part, or custom quad.
