...
- Server-authoritative Thaumonomicon index/entry/action payloads, item/open flow, and first browser/entry screens are implemented and the protocol audit passes `16/16`. The action audit covers exact legacy browser start, checked stage advance, entry acknowledgement flag semantics and known-entry final-stage progression, visibility filtering, server-owned unlockability/flags, client-cache invalidation, and explicit-open-versus-refresh separation. The remaining boundary is final search/visual parity and custom recipe-page renderers.
