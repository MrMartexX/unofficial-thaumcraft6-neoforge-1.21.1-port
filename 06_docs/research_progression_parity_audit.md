...
- Server-authoritative Thaumonomicon index/entry/action payloads, item/open flow, first browser/entry screens, and the first vanilla crafting-page snapshot/renderer are implemented and the protocol audit passes `19/19`. The audit covers exact legacy browser start, checked stage advance, entry acknowledgement flag semantics and known-entry final-stage progression, visibility filtering, server-owned unlockability/flags, client-cache invalidation, explicit-open-versus-refresh separation, and valid server snapshots for every live crafting catalog entry. The remaining boundary is final search/visual parity and custom recipe-page systems/renderers.
