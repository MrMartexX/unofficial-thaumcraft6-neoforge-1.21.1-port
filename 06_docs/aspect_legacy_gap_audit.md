...
| Current 1.21 exact item assignments | `676` | Current registered Thaumcraft ids, direct legacy vanilla seeds, modern-id exact audit entries, audited 1.21-only vanilla manual entries, dump-derived runtime parity overrides for legacy-equivalent plain vanilla stacks, and current registered Thaumcraft runtime parity overrides including table and research-table ids. Spawn eggs, firework star/rocket, and infested blocks are excluded for 1.12 parity. |
...
| Current 1.21 generated crafting assignments | `476` | Standard crafting cache for current `minecraft:*` and `thaumcraft:*` outputs whose ingredients have known aspects. Some legacy-equivalent vanilla and currently registered Thaumcraft outputs are now exact runtime parity overrides because final 1.12 lookup order, metadata flattening, or wildcard specificity proved different from generated/manual fallback. |
