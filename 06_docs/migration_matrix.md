...
| Gate 4 | Basic BlockEntities | Storage, ticking and save/load | Started narrowly with research table slots and menu opening data | Full theory data/card sync |
...
| Gate 8 | Menus and Screens | Modern GUI for machines and research | Started narrowly with the research table inventory menu and background screen | Large visual overhaul |
...
| 23 | BlockEntity base classes | `BlockEntity`, `EntityBlock#getTicker`, `CompoundTag`, `setChanged` | Very high | Create one simple non-inventory BE first | Started narrowly: `TCResearchTableBlockEntity` saves/drops scribing tools and paper slots and provides the server menu container; no ticking/theory data/card sync yet |
| 24 | Menus and machine GUI | `AbstractContainerMenu`, `MenuType`, `Screen`, data slots, payload actions | Very high | Port one simple inventory menu first | Started narrowly: `TCResearchTableMenu` and `TCResearchTableScreen` open the research table with legacy slot coordinates; no theorycraft buttons/cards/payload actions yet |
