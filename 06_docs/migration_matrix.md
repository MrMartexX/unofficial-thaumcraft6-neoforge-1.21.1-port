...
| Gate 4 | Basic BlockEntities | Storage, ticking and save/load | Started narrowly with storage-only research table slots | Final GUI and full sync |
...
| 23 | BlockEntity base classes | `BlockEntity`, `EntityBlock#getTicker`, `CompoundTag`, `setChanged` | Very high | Create one simple non-inventory BE first | Started narrowly: `TCResearchTableBlockEntity` saves/drops scribing tools and paper slots; no ticking/menu/sync yet |
...
| `TileEntity` | `BlockEntity` | Ticker in block class through `EntityBlock#getTicker`; first storage-only research table BlockEntity exists, but legacy `TileResearchTable` theory data is not copied. |
