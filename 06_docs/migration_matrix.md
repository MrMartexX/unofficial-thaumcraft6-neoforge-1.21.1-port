...
| 21 | Infusion crafting | Custom recipe type, modern BE scanning, payload FX, stability service | Critical | Write design document before gameplay code | Started only as recipe/data boundary: `thaumcraft:infusion` type/serializer, 2 imported legacy-corrected focus recipes, generated aspect cache support, and reload validation exist. Infusion Matrix BlockEntity, pedestal scan, stability, aspect consumption, instability, FX/sounds, and Thaumonomicon infusion pages are still not implemented. |
