...
| Shobie infusion recipe import | `06_docs/shobie_1_20_infusion_recipe_import.md` |
...
| Gate 1 | Item identity | Safe simple items and creative tab order scaffold | In progress, expanded beyond first slice; active item models/lang covered; first real tool-function layer restored for thaumium/void/elemental/primal crusher families | Research, aura, GUI, networking |
...
| 5 | Simple item registry | `DeferredRegister.Items`, item models, lang, textures, tags | Medium | Keep active 1.21 `models/item`, `textures/item`, and `en_us.json` authoritative; port legacy entries only when their ids are registered | In progress; active resources covered; `scribing_tools` model parent now matches legacy handheld; scribing tools preserve legacy stack/durability and `IScribeTools` marker; thaumium/void/elemental tools and `primal_crusher` now use real 1.21 tool classes with legacy tier values instead of inert placeholders |
...
| 8 | Armor, baubles and wearables | Vanilla equipment first, optional accessory integration later | High | Define 1.21 armor material + texture-layer mapping for legacy `textures/entity/armor` before making armor equipable | Port later; intentionally blocked after item audit to avoid wrong/missing armor visuals |
...
| 20 | Arcane crafting | Custom recipe type/serializer, menu, BlockEntity, vis checks | Very high | Define recipe JSON/serializer first | Started: recipe type/serializers, exact hand-audited fixtures, 56 translated Shobie arcane JSONs plus 22 already-covered Shobie arcane rows, exact current material/tag bridges, server-authored Thaumonomicon snapshots for the audited exact set, first Arcane Workbench BlockEntity/menu/no-charger current-chunk aura drain path, Workbench Charger 3 x 3 aura query/drain, first player vis-discount path, server-owned base/discounted cost/aura/crystal feedback, missing-vis ghost output, current arcane recipe-derived aspects, a 105-check arcane recipe audit over the original audited exact set, and a 23-check server behavior audit for matrix/crystal/vis/fallback/consumption/discount/Charger/ghost/menu-feedback behavior exist. Only the Shobie `primal_charm` arcane edge row remains deferred because `balanced_shard` identity is unresolved. Filter/resonator behavior, real equipment/Curios discount integration, and final visual parity remain blocked. |
| 21 | Infusion crafting | Custom recipe type, modern BE scanning, payload FX, stability service | Critical | Write design document before gameplay code | Started only as recipe/data boundary: `thaumcraft:infusion` type/serializer, 38 translated recipes, generated aspect cache support, and reload validation exist. Infusion Matrix BlockEntity, pedestal scan, stability, aspect consumption, instability, FX/sounds, and Thaumonomicon infusion pages are still not implemented. |
| 22 | Crucible and alchemy | Custom recipe types, BE save/tick, capability-based essentia access | Critical | Prototype crucible data model first | Started only as recipe/data boundary: `thaumcraft:crucible` type/serializer, 41 translated recipes, generated aspect cache support, and reload validation exist. Crucible BlockEntity, water/heat, in-world catalyst/result loop, pollution/flux, FX/sounds and Thaumonomicon crucible pages are still not implemented. |
...
| Old world generators | Datapack/datagen features and biome modifiers | Design before code. Shobie 1.20 worldgen/biome modifier JSON is present only under `src/shobieReference/resources`. |
...
current port's authoritative ids or subsystem architecture. The translated data
surface now includes all vanilla rows, the Shobie arcane set except the known
edge row, 41 crucible recipes and 38 infusion recipes. Shobie Java remains
...
gameplay, worldgen/biome modifiers, custom recipe page renderers, seal
DataComponents and remaining variant-heavy item behavior.

Unsafe experimental follow-up on the same branch:
- The final 39 Shobie recipe rows are translated under
  `data/thaumcraft/recipe/unsafe_shobie/`; no recipe catalog rows remain
  deferred.
- The unsafe layer uses separate recipe ids and does not overwrite the
  legacy-correct recipe set.
- Seal NBT variants are flattened in runtime JSON and must be revisited after
  seal DataComponents exist.
- The full Shobie Java/resource tree is retained in
  `05_neoforge_port/src/shobieReference/` as non-compiled reference content.
- Validation after the unsafe pass: build passes, server reload loads 1572
  recipes and 623 generated aspect-cache assignments, and client smoke has no
  matching ERROR/FATAL/missing-model/missing-texture lines.
