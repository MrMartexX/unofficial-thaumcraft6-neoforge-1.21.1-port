...
| Gate 4 | Basic BlockEntities | Storage, ticking and save/load | Started narrowly with research table slots, menu opening data, theory data save/load, first vanilla aid scan integration, update packet/tag sync, and first table-top item renderer consumer | Full theory data/card/aids polish |
...
| 28 | Rendering and FX | BERs, entity renderers, particles, overlays, `PoseStack` | Critical | Validate imported FX/textures and isolate old GL/shader assumptions before expanding | Early FX work; `rendering_model_pipeline_audit.md` records OBJ/MTL, BER and BEWLR migration rules; a narrow research table BER renders paper and scribing tools from synced slots |
