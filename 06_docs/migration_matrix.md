...
| 20 | Arcane crafting | Custom recipe type/serializer, menu, BlockEntity, vis checks | Very high | Define recipe JSON/serializer first | Started: recipe type/serializers, exact `thaumometer`, `vis_resonator`, and `workbenchcharger`, exact `plateIron` bridge, server-authored Thaumonomicon snapshots, first Arcane Workbench BlockEntity/menu/no-charger current-chunk aura drain path, Workbench Charger 3 x 3 aura query/drain, first player vis-discount path, server-owned base/discounted cost/aura/crystal feedback, a 25-check arcane recipe audit, and a 22-check server behavior audit for matrix/crystal/vis/fallback/consumption/discount/Charger/menu-feedback behavior exist. Remaining recipes, full equipment/Curios discount integration, final GUI parity, and custom recipe-derived aspect generation remain blocked. |
