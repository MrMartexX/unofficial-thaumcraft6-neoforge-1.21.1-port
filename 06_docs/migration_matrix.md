...
| Gate 7 | Player progression | Research, scanning, knowledge and warp | Research data/progression and the permanent recipe/page catalog are parity-closed; server-side knowledge, scanning, minimal warp storage, and server-authoritative Thaumonomicon view/action protocol exist. Full warp effects/events/client sync and final UI remain incomplete | Final Thaumonomicon UI polish |
...
| 19 | Thaumonomicon UI | Modern `Screen`, custom widgets, client data cache | Critical | Build real browser/entry screens only over authoritative view models | Protocol foundation ready: server-authoritative visible index, entry/stage/requirement/bookmark views, exact legacy start/stage-advance/entry-acknowledgement actions including known-entry final-stage progression, and invalidating client cache pass `15/15` audit checks. Real browser/entry screens and recipe-page renderers are next. |
