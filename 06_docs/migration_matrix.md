...
| 16 | Research model | Reloadable data loader/datapack format, validation, datagen | Very high | Create model classes and load a tiny test category | Progression/data core parity-closed: the loader preserves `148` entries, `271` stages, `16` addenda, entry rewards, and all seven legacy categories; source/runtime/category comparison reports `0` differences. Checked progression preserves exact legacy stage/empty-gate/warp/flag/reward/addendum/sibling/XP ordering. Requirements remain `0` unresolved with `16` subsystem bridge warnings. Permanent research recipe/page catalog, Thaumonomicon protocol/UI, full warp system, events, and remaining dependency-heavy theorycraft content are not implemented. |
| 17 | Player knowledge | Player attachment/capability, explicit sync payloads, server authority | Very high | Store one test research flag per player | Started: persistent player storage and GUI-ready sync cover observation/theory raw knowledge, known research keys, stages, and flags. Checked progression plans item/knowledge consumption before mutation and final sync. Minimal warp storage now receives exact legacy research warp split. Full warp effects/events/client sync, cancellable research/knowledge events, recipe/page catalog, and final Thaumonomicon consumers remain blocked. |
