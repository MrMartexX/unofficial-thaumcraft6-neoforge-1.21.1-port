...
| 20 | Arcane crafting | Custom recipe type/serializer, menu, BlockEntity, vis checks | Very high | Define recipe JSON/serializer first | Started: recipe type/serializers, exact `thaumometer` and `vis_resonator`, exact `plateIron` bridge, server-authored Thaumonomicon snapshots, and first Arcane Workbench BlockEntity/menu/current-chunk aura drain path exist. Remaining recipes, player vis discounts, Workbench Charger 3x3 aura, GUI parity, and custom recipe-derived aspect generation remain blocked. |
