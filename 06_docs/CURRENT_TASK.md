...
- The first custom entity foundation blocker is closed without broad AI/render scope: `TCEntityTypes` now registers `SpecialItem`, `FollowItem`, `FluxRift`, `ArcaneBore`, `TaintSeed` and `TaintSeedPrime`, preserves the 43-entry legacy entity registry catalog as explicit metadata, renders the two item-entity types through the vanilla item renderer, and writes `06_docs/audits/generated/thaumcraft_1_21_entity_foundation_audit.md`. The dedicated audit passes `11/11`.
...
1. Continue blocker-removal work outside the focus/caster layer unless explicitly requested.
   ...
   - Row 11 now has the finite Flux Goo -> Taint Fibre blocker slice plus the first audited TaintSeed/terrain ecology slice. Do not expand this into taint mobs, FallingTaint crust physics, Thaumic Slime spawning or broad rift consequences without a focused design/audit slice.
   - Row 14 now has an entity registry foundation, safe legacy item-entity implementations, and focused Flux Rift/Arcane Bore entity foundations. Do not expand this into mob AI, golems, focus projectiles/clouds/mines or broad custom renderers without a focused design/audit slice.
