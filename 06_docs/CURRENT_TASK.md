...
- Thaumonomicon recipe-page protocol hardening now validates server-owned vanilla crafting, arcane, crucible, infusion, blueprint and legacy fake/display page snapshots, selected-stage warp propagation, side-panel knowledge/aspect state, runtime PNG category background paths, open-time known-research sibling completion and server-owned recipe-search output aliases. The protocol audit at `05_neoforge_port/run/build/audits/thaumcraft_1_21_thaumonomicon_protocol_audit.md` passes `43/43`; the page catalog audit reports `203 READY`, `0 DEFERRED`, `0 LEGACY_MISSING`. The 13 legacy fake catalog ids are display-only Thaumonomicon pages, not craftable gameplay recipes.
...
2. Keep already closed behavior audits green after related changes:
   ...
   - Thaumonomicon protocol audit: `43/43`.
...
- Current protocol audit result: `43/43`, including server-owned vanilla crafting, arcane, crucible, infusion, blueprint and fake display page snapshots, selected-stage warp propagation, side-panel state, runtime PNG category background validation, open-time known-research sibling completion, server-owned recipe-search output aliases and recipe drilldown/history validation.
