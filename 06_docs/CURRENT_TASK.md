...
- The custom entity foundation blocker now includes the first WarpEvents outcome entity foundations without broad renderer/golem scope: `TCEntityTypes` registers `SpecialItem`, `FollowItem`, `BottleTaint`, `FluxRift`, `ArcaneBore`, `FallingTaint`, `TaintSeed`, `TaintSeedPrime`, `Wisp`, `ThaumSlime`, `TaintCrawler`, `Taintacle`, `TaintacleTiny`, `TaintSwarm`, `CultistPortalLesser`, `MindSpider` and `EldritchGuardian`, preserves the 43-entry legacy entity registry catalog as explicit metadata, renders item/projectile foundations through the vanilla item renderer where appropriate, gives FallingTaint a block-model renderer foundation and uses explicit no-op placeholders for newly server-complete mobs until measured visual parity lands. `06_docs/audits/generated/thaumcraft_1_21_entity_foundation_audit.md` now passes `33/33`.
...
2. Keep already closed behavior audits green after related changes:
   ...
   - Warp event behavior audit: `25/25`.
