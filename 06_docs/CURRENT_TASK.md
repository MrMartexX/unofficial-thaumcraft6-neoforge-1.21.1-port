...
- Flux Goo now has the missing legacy level-zero alternate result into real `taint_fibre`. `taint_fibre` is a registered block/BlockItem with the legacy 10-property state contract, deterministic support/growth state, face/growth shapes, light levels, non-undead walk taint, modernized multipart model paths and a dedicated runtime blocker audit passing `13/13`. Full `TaintHelper.spreadFibres`, TaintSeed entities, taint terrain transforms, taint mobs and Thaumic Slime spawning remain deferred to their owning row-11/14 slices.
...
1. Continue blocker-removal work outside the focus/caster layer unless explicitly requested.
   ...
   - Row 11 now has the finite Flux Goo -> Taint Fibre blocker slice. Do not expand this into full taint spread, TaintSeed, Thaumic Slime, taint terrain conversion or broad rift consequences without a focused design/audit slice.
