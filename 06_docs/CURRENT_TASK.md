...
1. Continue blocker-removal work outside the focus/caster layer unless explicitly requested.
   ...
   - Remaining row-4 decision: whether the old `essentia_importer` / `essentia_exporter` placeholders stay as hidden reference-only aliases or are removed from player-facing flows.
   - Next blocker-removal candidate: plan and implement the Thaumatorium row-10 slice over real crucible, jar/alembic/phial, Void Jar, Essentia Mirror and transport behavior.
