...
- The first special alchemy item/projectile behavior slice is active: `bath_salts` preserves the legacy dropped-item lifespan, `bottle_taint` preserves legacy stack/use/throw constants, `BottleTaint` applies Flux Taint with the legacy living-target predicate and places Flux Goo with the legacy support/fallback rules. `06_docs/audits/generated/thaumcraft_1_21_special_alchemy_behavior_audit.md` passes `13/13`; real Liquid Death/Purifying Fluid blocks, Warp Ward behavior and Bath Salts water-source conversion remain a separate fluid/effect blocker.
...
1. Continue blocker-removal work outside the focus/caster layer unless explicitly requested.
   ...
   - Row 9 now has the first special alchemy item/projectile behavior slice for Bath Salts item lifespan and Bottled Taint. Keep remaining Liquid Death/Purifying Fluid/Warp Ward and Bath Salts water-source conversion work under a focused fluid/effect design/audit slice.
...
- Do not expand this further into Flux Rift focus-cloud event execution, rift visual parity, taint mob natural-spawn/rendering parity, Thaumic Slime polish, essentia networks, automation, item pulling radius or the remaining LiquidDeath/PurifyingFluid/WarpWard/Bath Salts water-conversion effects without a new focused slice.
