...
- The first custom entity foundation blocker is closed without broad renderer/golem scope: `TCEntityTypes` now registers `SpecialItem`, `FollowItem`, `BottleTaint`, `FluxRift`, `ArcaneBore`, `FallingTaint`, `TaintSeed`, `TaintSeedPrime`, `Wisp`, `ThaumSlime`, `TaintCrawler`, `Taintacle`, `TaintacleTiny` and `TaintSwarm`, preserves the 43-entry legacy entity registry catalog as explicit metadata, renders item/projectile foundations through the vanilla item renderer where appropriate, gives FallingTaint a block-model renderer foundation and uses explicit no-op placeholders for newly server-complete mobs until visual parity lands. `06_docs/audits/generated/thaumcraft_1_21_entity_foundation_audit.md` now passes `27/27`.
...
- Warp now has a focused server-authoritative mutation/sync bridge and the first server-side `WarpEvents` behavior slice: `TCWarpManager` owns add/reduce/set/clear, sync payloads, client action-bar messages, FIRSTSTEPS-gated WARP unlock and the legacy `jar_brain` item-warp crafting bridge with `wussMode` suppression; `TCWarpEvents` owns the legacy 2000-tick scheduler, temporary-warp decay before roll, trigger/counter math, potion/status outcomes, Death Gaze range/cone basics, Unnatural Hunger relief and BATHSALTS/ELDRITCH unlock thresholds. `06_docs/audits/generated/thaumcraft_1_21_warp_behavior_audit.md` passes `9/9`, and `06_docs/audits/generated/thaumcraft_1_21_warp_event_behavior_audit.md` passes `19/19`. Client hallucination/stress visuals, real Eldritch Guardian/Mind Spider/lesser cultist portal spawns, fortress mask mitigation and exact potion-overlay visuals remain focused follow-up work.
...
1. Continue blocker-removal work outside the focus/caster layer unless explicitly requested.
   ...
   - Row 7/15 now has the focused Brain-in-a-Jar server/theorycraft blocker slice, the registered `CardTruth` Eldritch theorycraft card, the focused `jar_brain` item-warp crafting bridge and the first server-side `WarpEvents` behavior slice. Keep exact animated model parity, WarpEvents client hallucination/entity-spawn visuals, `AidBasicEldritch`, Crimson portal aid and broader golem/mind behavior as separate focused work.
...
2. Keep already closed behavior audits green after related changes:
   ...
   - Warp event behavior audit: `19/19`.
