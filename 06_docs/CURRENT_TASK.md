...
- The focused Wisp behavior/render/FX blocker is now active: Wisp preserves legacy free-flight waypoint AI, hurt/player aggro cadence, chase motion, zap attack cadence/damage/sound, ambient mote particles, billboard render layers, source/target-id zap payload and dynamic aspect rendering contract. `06_docs/audits/generated/thaumcraft_1_21_wisp_behavior_audit.md` passes `11/11`. The spawn-policy slice now activates the exact legacy Wisp Nether row, Angry Zombie overworld row, Firebat Nether row and Oct 31 Firebat overworld row mapped to 1.21 biome tags; `06_docs/audits/generated/thaumcraft_1_21_entity_spawn_policy_audit.md` passes `29/29`. Pech natural spawning and Thaumcraft-biome spawn rows plus measured in-world pixel parity remain separate row-14/17 work.
...
2. Keep already closed behavior audits green after related changes:
   ...
   - Entity spawn policy audit: `29/29`.
...
- Do not expand this further into Flux Rift focus-cloud event execution, rift visual parity, taint mob natural-spawn/rendering parity, Thaumic Slime polish, essentia networks, remaining automation families or exact fluid render/particle parity without a new focused slice. The legacy crucible item entity path is collision-only; avoid adding a fake radius pull to the crucible.
