...
- Latest Thaumonomicon protocol/UI correction on 2026-07-21 passes `48/48` checks. New coverage validates the creative-book-without-pickup state (`FIRSTSTEPS` visible but locked at stage `0`), legacy start visibility for `Discovering Artifice` / `Discovering Golemancy`, legacy-recursive debug `research all` behavior over all `7/7` categories and `148/148` entries, new-research flag surfacing, blank empty knowledge totals and normal item tooltip rendering for recipe/item hovers.
...
- Thaumonomicon recipe-page protocol hardening now validates server-owned vanilla crafting, arcane, crucible, infusion, blueprint and legacy fake/display page snapshots, selected-stage warp propagation, side-panel knowledge/aspect state, runtime PNG category background paths, open-time known-research sibling completion, server-owned recipe-search output aliases, creative locked-start behavior, legacy start visibility, recursive debug-all behavior and legacy new-flag surfacing. The protocol audit at `05_neoforge_port/run/build/audits/thaumcraft_1_21_thaumonomicon_protocol_audit.md` passes `48/48`; the page catalog audit reports `203 READY`, `0 DEFERRED`, `0 LEGACY_MISSING`. The 13 legacy fake catalog ids are display-only Thaumonomicon pages, not craftable gameplay recipes.
- Thaumonomicon row-8 navigation and current-scope entry UI are active: functional search/filter, recipe drilldown/history, fake display-page renderer slices, legacy-style recipe bookmark tabs, aspect side insert, knowledge side insert and selected-stage warp warning. The browser search box filters only the current server-authoritative visible index, search-result clicks reuse the same server action/revision payload as graph clicks, recipe output hits are carried by the server index and open the exact bookmark/page, recipe stack click-through sends a server-owned drilldown request and returns an authoritative page snapshot, and fake crafting/infusion/runic display pages render from server fixtures grounded in legacy `ConfigRecipes`. Latest UI repair also normalizes bare legacy texture paths to `thaumcraft:*`, clips map nodes/links inside the browser viewport, lowers the first TC category tab to the legacy offset, restores legacy locked-hover parent details, keeps recipe-history return as a hover-only legacy label, animates page arrows with the legacy bob scale and adds the `KNOWLEDGETYPES` in-page totals boundary. Remaining row-17 work is measured pixel-level browser/entry comparison and the cheat Thaumonomicon variant, not missing server-authoritative UI behavior.
...
- Row 6-11 blocker regression was re-run together on 2026-07-20 and the Thaumonomicon protocol/UI repair was re-run on 2026-07-21. The sweep found and fixed three audit/parity problems: Thaumatorium fake-player knowledge leakage in the audit harness, research category backgrounds using runtime `.png` paths while keeping legacy `.jpg` files as reference assets, and a Windows PowerShell 5 incompatibility in the crucible recipe-data audit script. Current row 6-11 evidence is green: item/equipment `17/17`, focus/caster core `10/10`, focus cast execution `5/5`, research data `0` source/runtime/category diffs with `10/10` progression checks, Thaumonomicon protocol `48/48`, page catalog `203 READY` / `0 DEFERRED` / `0 LEGACY_MISSING`, crucible behavior `16/16`, special alchemy `32/32`, alchemy automation `11/11`, crucible recipe data `78/78`, Thaumatorium `19/19`, transport `61/61`, Flux/Bore/Thaumatorium `11/11`, Flux Rift consequence `21/21`, Wisp `11/11`, spawn policy `34/34`, Flux Goo/Taint Fibre `13/13`, Taint ecology `15/15`, Taint mob `15/15`, FallingTaint `10/10`, and entity foundation `71/71`. Final measured pixel-level visual parity remains row 17 work, not part of this blocker closure.
...
2. Keep already closed behavior audits green after related changes:
   ...
   - Thaumonomicon protocol audit: `48/48`.
...
- Current protocol audit result: `48/48`, including server-owned vanilla crafting, arcane, crucible, infusion, blueprint and fake display page snapshots, selected-stage warp propagation, side-panel state, runtime PNG category background validation, open-time known-research sibling completion, server-owned recipe-search output aliases, recipe drilldown/history validation, creative locked-start behavior, legacy start visibility for Artifice/Golemancy discovery entries, legacy-recursive debug `research all`, new-research/page flag exposure, blank empty knowledge totals and inventory-style stack tooltips.
