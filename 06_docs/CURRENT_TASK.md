...
- The item/block parity framework quick preset is executable with registry/resource checks plus `item_visual_parity`, `legacy_shape_parity`, and `legacy_visual_collision_parity`. Current report-only quick summary: `2149 PASS`, `5 RENAMED_WITH_MAPPING`, `2 VARIANT_MAPPED`, `1 INTENTIONAL`, `301 MISSING`, `103` safe resource-boundary failures; visual-boundary subreports currently show `10` legacy-shape review rows, `2` legacy visual/collision facing-domain mismatches (`golem_builder`, `research_table`), `34` missing item models, and `219` item visual review rows.
