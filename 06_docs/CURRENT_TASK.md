...
- Latest Thaumonomicon protocol/UI correction on 2026-07-24 passes `55/55` checks. It retains the creative locked-start/debug-all contracts and additionally validates all seven knowledge categories over the wire, exact legacy knowledge type/category progression, category completion, `165` active research icon contracts, all `51` texture icons and the exact `11` legacy animated seal spritesheets. Recipe/side papers are mutually exclusive, open papers suppress requirement/result layers, item hovers use normal inventory tooltips, tabs use completed-research gates, research/page markers preserve legacy draw order, and animated texture layouts advance on the legacy `150 ms` frame cadence with reload-safe client caching.
- The 2026-07-24 client parity follow-up fixes the proven Liquid Death null-sprite crash, restores exact legacy fluid tints and effect icon cells, adds progressive colored Warded/Void Jar essentia rendering, removes Golem Press/Infernal Furnace from the creative tab, repairs active caster/Hungry Chest/Golem Press/crystal atlas paths, restores legacy robe dye layers/default color, makes lamp inventory models off, and corrects knowledge/focus render-state leakage plus node-flag coordinates.
- The in-world crucible behavior slices have a design boundary in `06_docs/gameplay/crucible_in_world_behavior_design.md`.
- The seven legacy dynamic HEDGE_ALCHEMY crucible costs are now explicit JSON aspect costs resolved from the current parity data, and `audit-crucible-recipe-data.ps1` reports `78/78` valid recipe files.
...
- Void Siphon now has its first row-13 standalone device slice: real `void_siphon` block/BlockItem/BlockEntity/menu/screen, legacy shape and redstone-enabled state, extract-only void-seed slot, progress persistence, rift-drain math and a future `TCVoidSiphonRiftAccess` adapter boundary. The standalone device blocker audit passes `9/9`; final Flux Rift visual parity remains row 17 work.
...
- Flux Goo now has the missing legacy level-zero alternate result into real `taint_fibre`. The first TaintSeed/ecology slice is also active: `taint_seed` and `taint_seed_prime` entities, `TCTaintHelper` seed-radius bookkeeping/spread transforms, `taint_crust`, `taint_soil`, `taint_rock`, `taint_geyser`, `taint_log`, `taint_feature`, Flux Taint tainted-mob healing, modern resources and deterministic validation are covered by the dedicated taint ecology blocker audit passing `15/15`. The follow-up taint mob foundation slice registers `thaum_slime`, `taint_crawler`, `taintacle`, `taintacle_tiny`, `taintacle_giant` and `taint_swarm`, adds legacy scan/aspect identities where source data exists, and closes the crawler feature-spawn, crawler fibre trail, geyser swarm, taintacle tiny-spawn, swarm NBT and Thaumic Slime size/xp contracts with a `15/15` audit. The FallingTaint slice registers `falling_taint`, ports the crusted-taint falling gates, TC6 gravity/damping, first-tick source removal, side-overhang fall path, landing placement and GORE sound path, and passes the focused `10/10` audit. The Flux Rift consequence slice registers `wisp` and `focus_cloud`, preserves Wisp dynamic aspect type, ports the legacy `50/10/20/20/1` event table, Wisp spawn, prime TaintSeed boost/pollution, infectious vis exhaustion, collapse effect fallthrough and Causality Collapser AABB rift collapse, and passes the focused `21/21` audit; event 3 now executes the rift-owned invisible FocusCloud path without legacy stability cost. The natural-spawn slice now activates the legacy Wisp Nether row, Angry Zombie overworld row, Firebat Nether row and Oct 31 Firebat overworld row with NeoForge biome modifiers, spawn placement predicates and a `34/34` audit. Exact custom mob renderer/model animation pixel parity, Thaumcraft-biome worldgen spawn families, measured rift/focus-cloud particle pixel parity, exact FallingTaint landing particles/render pixel parity and exact animated TaintSeed model parity remain deferred to focused row-11/14/17 slices.
...
- The custom entity foundation blocker now includes the first WarpEvents outcome entity foundations, the Eldritch Orb projectile path, the GolemOrb projectile path, Alumentum projectile foundation, Causality Collapser rift-closing projectile, Firebat, Pech, Angry/Furious Zombie foundations, the legacy Angry Zombie overworld natural-spawn row, Firebat Nether/Halloween natural-spawn rows, the rift-owned `FocusCloud` foundation and the lesser Crimson portal minion pair without broad mob/golem scope: `TCEntityTypes` registers `SpecialItem`, `FollowItem`, `Alumentum`, `CausalityCollapser`, `BottleTaint`, `EldritchOrb`, `GolemOrb`, `FluxRift`, `FocusCloud`, `ArcaneBore`, `FallingTaint`, `TaintSeed`, `TaintSeedPrime`, `Wisp`, `Firebat`, `Pech`, `BrainyZombie`, `GiantBrainyZombie`, `ThaumSlime`, `TaintCrawler`, `Taintacle`, `TaintacleTiny`, `TaintSwarm`, `CultistPortalLesser`, `CultistKnight`, `CultistCleric`, `MindSpider` and `EldritchGuardian`, preserves the 43-entry legacy entity registry catalog as explicit metadata, renders item foundations through the vanilla item renderer where appropriate, gives FallingTaint a block-model renderer foundation, gives FocusCloud an invisible renderer like legacy, gives Angry/Furious Zombies a legacy `bzombie.png` zombie-model renderer foundation, gives Firebat a legacy bat-model renderer foundation, gives Pech a legacy model/texture renderer foundation, gives EldritchOrb a source-informed particle/tendril renderer contract and gives GolemOrb the legacy electric-orb particle renderer foundation. Pech also owns the first legacy-shaped type/anger/tamed state, 9-slot pack, valued-item tame path, trade menu/catalog, `pech_wand`, scan identity and subtype aspect contract. `06_docs/audits/generated/thaumcraft_1_21_entity_foundation_audit.md` now passes `71/71` checks.
- The focused special alchemy behavior slice is active: `bath_salts` preserves the legacy dropped-item lifespan and water-source conversion, `alumentum` preserves the legacy throw/dispenser projectile contract, invisible projectile body, fixed `0.75F` projectile speed, fiery trail bridge and flaming explosion constants, `bottle_taint` preserves legacy stack/use/throw constants, `BottleTaint` applies Flux Taint with the legacy living-target predicate and places Flux Goo with the legacy support/fallback rules, `liquid_death` / `purifying_fluid` are real NeoForge fluids/blocks with bucket source mappings, `liquid_death` uses the legacy `thaumcraft:dissolve` damage identity and dissolve-crystal living-drop bridge, Warp Ward follows the legacy duration formula, and Sanity Soap uses the legacy Warp Ward/Purifying Fluid bonuses. `06_docs/audits/generated/thaumcraft_1_21_special_alchemy_behavior_audit.md` passes `32/32`; exact Alumentum/Causality fire-mote pixel parity, exact client fluid particles/render translucency and remaining automation consumers stay separate focused work. Legacy `BlockCrucible` / `TileCrucible` has collision-only item absorption, not a radius item-pull path.
- The first broader alchemy automation device slice is active: `spa` and `everfull_urn` are real BlockItem-backed blocks with BlockEntities, modern resources and legacy-shaped server behavior. Everfull Urn keeps the top-only drain/no-fill fluid capability, aura-to-water refill quanta, 5x3x5 cached scan and 333 mB cauldron/glass-bottle costs. Arcane Spa keeps the bath-salts slot, 5000 mB tank, mix-mode Purifying Fluid placement, liquid-only placement and adjacent 5x5 expansion. `06_docs/audits/generated/thaumcraft_1_21_alchemy_automation_device_audit.md` passes `11/11`; Spa GUI, Botania Petal Apothecary support, exact water-trail/client particles and pixel-level model parity remain focused follow-up work.
- Brain-in-a-Jar now has a focused behavior blocker slice: `jar_brain` is a real BlockItem-backed block with BlockEntity, legacy XP storage key, max XP, orb pull/absorb/release cadence, comparator formula, enchanting bonus, stack XP preservation tooltip, and `AidBrainInAJar -> CardDarkWhispers` theorycraft registration. `06_docs/audits/generated/thaumcraft_1_21_brain_jar_behavior_audit.md` passes `10/10`; animated brain BER/model rotation, exact sparkle pixel parity and broad golem/mind systems remain focused follow-up work. The legacy `jar_brain` item-warp entry is now covered by the focused warp bridge.
- Warp now has a focused server-authoritative mutation/sync bridge and a stronger server-side `WarpEvents` behavior slice: `TCWarpManager` owns add/reduce/set/clear, sync payloads, client action-bar messages, FIRSTSTEPS-gated WARP unlock and the legacy `jar_brain` item-warp crafting bridge with `wussMode` suppression; `TCWarpEvents` owns the legacy 2000-tick scheduler, temporary-warp decay before roll, trigger/counter math, potion/status outcomes, Death Gaze range/cone basics, Unnatural Hunger relief, BATHSALTS/ELDRITCH unlock thresholds, real spawn attempts for Eldritch Guardian, Mind Spider and lesser cultist portal foundations, the Eldritch Guardian no-gravity orb projectile attack path, lesser portal spawning for registered CultistKnight/CultistCleric minions, and the CultistCleric red targeted GolemOrb branch. `06_docs/audits/generated/thaumcraft_1_21_warp_behavior_audit.md` passes `9/9`, and `06_docs/audits/generated/thaumcraft_1_21_warp_event_behavior_audit.md` passes `35/35`. Client hallucination/stress visuals, exact Guardian/orb/cultist/portal renderer pixel parity, fortress mask mitigation and exact potion-overlay visuals remain focused follow-up work.
- `CardTruth` is now registered and validated as the safe legacy Eldritch option card. The research table diagnostic report at `06_docs/audits/generated/thaumcraft_1_21_research_table_audit.md` passes `59/59`. `AidBasicEldritch`, Crimson portal aid and Dragon Egg theorycraft remain deferred because their legacy block/entity ownership is not safely available in the current port layer.
- Thaumonomicon recipe-page protocol hardening now validates server-owned vanilla crafting, arcane, crucible, infusion, blueprint and legacy fake/display page snapshots, selected-stage warp propagation, full categorized knowledge state, runtime PNG category background paths, open-time known-research sibling completion, server-owned recipe-search output aliases, creative locked-start behavior, legacy start visibility, recursive debug-all behavior, category completion, icon resolution, animated icon frame layout and legacy new-flag surfacing. The protocol audit at `05_neoforge_port/run/build/audits/thaumcraft_1_21_thaumonomicon_protocol_audit.md` passes `55/55`; the page catalog audit reports `203 READY`, `0 DEFERRED`, `0 LEGACY_MISSING`. The 13 legacy fake catalog ids are display-only Thaumonomicon pages, not craftable gameplay recipes.
- Thaumonomicon row-8 navigation and current-scope entry UI are active: functional search/filter, recipe drilldown/history, fake display-page renderer slices, legacy-style recipe bookmark tabs, mutually exclusive aspect/knowledge/recipe papers, selected-stage warp warning and full seven-category knowledge totals. The browser and entry screens use server-authoritative visibility/revision data, legacy item tooltip colors, exact marker layering, category completion percentages, completed-research side-tab gates and metadata/component-aware research icon resolution. Remaining row-17 work is screenshot-measured pixel/animation tuning and the cheat Thaumonomicon variant, not missing server-authoritative UI behavior.
- The focused Wisp behavior/render/FX blocker is now active: Wisp preserves legacy free-flight waypoint AI, hurt/player aggro cadence, chase motion, zap attack cadence/damage/sound, ambient mote particles, billboard render layers, source/target-id zap payload and dynamic aspect rendering contract. `06_docs/audits/generated/thaumcraft_1_21_wisp_behavior_audit.md` passes `11/11`. The spawn-policy slice now activates the exact legacy Wisp Nether row, Angry Zombie overworld row, Firebat Nether row, Oct 31 Firebat overworld row and Pech magical-biome gate mapped to 1.21 biome tags; `06_docs/audits/generated/thaumcraft_1_21_entity_spawn_policy_audit.md` passes `34/34`. Pech magical-biome spawning is active through optional Thaumcraft biome tags; actual Thaumcraft biome providers plus measured in-world pixel parity remain separate row-14/17 work.
- The item/block parity framework quick preset is executable with registry/resource checks plus `texture_color`, `item_visual_parity`, `legacy_shape_parity`, and `legacy_visual_collision_parity`. The 2026-07-24 focused `visual` run reports `0` missing item/block models, `1287/1287` model-transform evidence rows with `0` review rows, and `2145/2209` item-visual pass rows with `64` explicit implementation reviews (`60` catalog bridge items and `4` enchanted placeholders). Legacy shape remains `123/139` pass with `16` review rows; strict visual/collision evidence is `271` match / `0` mismatch / `0` missing / `444` unknown. Texture comparison is `295/295` exact with `0` reviews, and visual completion is `13/17` pass with `4` review / `0` errors. `research_table` now has exact horizontal blockstate rotations and legacy Shift placement, while `golem_builder` has horizontal state and its original OBJ/MTL static bridge instead of empty geometry. CI-safe passes with `0` safe failures; JSON validity is `1648/1648`, and docs/registry consistency is `6/6`.
- Row 4/5 blocker regression was re-run together on 2026-07-20: the combined essentia transport utility audit passes `61/61`, the regular Arcane Workbench recipe id audit passes `109/109`, and the Arcane Workbench behavior/discount audit passes `28/28` with `wand_workbench` resolving to the current `TCFocalManipulatorBlock` identity.
- Row 6-11 blocker regression remains green, and the 2026-07-24 Thaumonomicon protocol/UI repair raises its protocol result to `55/55`. Final measured pixel-level visual parity remains row 17 work, not part of this blocker closure.
...
1. Continue blocker-removal work outside the focus/caster layer unless explicitly requested.
   ...
   - Row 11 now has the finite Flux Goo -> Taint Fibre blocker slice, the audited TaintSeed/terrain ecology slice, the first taint mob/Thaumic Slime server-foundation slice, the focused FallingTaint crust-physics slice, the focused Flux Rift consequence slice, the Wisp dependency behavior/FX slice, the exact Wisp Nether natural-spawn slice, the legacy Angry Zombie overworld natural-spawn slice, the Firebat Nether/Halloween natural-spawn slice, and the Pech magical-biome spawn gate slice. Do not expand this into broad Thaumcraft-biome worldgen spawn rows, measured rift/focus-cloud particle pixel parity or final taint/rift visual-particle parity without a focused design/audit slice.
   - Row 14 now has an entity registry foundation, safe legacy item/projectile entity implementations including `Alumentum`, `BottleTaint` and `CausalityCollapser`, focused Flux Rift/Arcane Bore entity foundations, Wisp behavior/render/spawn contracts, Firebat AI/spawn/aspect/renderer foundation, Pech trade/entity foundation, Angry/Furious Zombie foundations and the first taint mob server behavior contracts. Do not expand this into golems, broad player-authored focus projectiles/clouds/mines or broad custom renderers without a focused design/audit slice.
   - Row 9 now has the focused special alchemy behavior slice for Bath Salts, Bottled Taint, real Liquid Death/Purifying Fluid, Liquid Death dissolve damage/drop identity, Warp Ward and Sanity Soap bonuses, plus the first broader automation device slice for Arcane Spa and Everfull Urn. Keep exact client fluid particles/render translucency, crucible-derived aspect generation, Spa GUI/Botania support and remaining automation families under focused follow-up slices. Do not add a crucible item-pulling radius unless new legacy evidence identifies a different owner.
   ...
   - Row 7/15 now has the focused Brain-in-a-Jar server/theorycraft blocker slice, the registered `CardTruth` Eldritch theorycraft card, the focused `jar_brain` item-warp crafting bridge and the first server-side `WarpEvents` behavior slice with real Guardian/Spider/lesser-portal spawn foundations, the Guardian orb projectile path, lesser portal CultistKnight/CultistCleric minion spawning and CultistCleric GolemOrb branch. Keep exact animated model parity, WarpEvents client hallucination visuals, exact Guardian/orb/cultist/portal renderer pixel parity, `AidBasicEldritch`, Crimson portal aid and broader golem/mind behavior as separate focused work.
...
2. Keep already closed behavior audits green after related changes:
   ...
   - Entity foundation audit: `71/71`.
   - Special alchemy behavior audit: `32/32`.
   - Flux/Bore/Thaumatorium blocker audit: `11/11`.
   - Flux Rift consequence audit: `21/21`.
   - Wisp behavior/render contract audit: `11/11`.
   - Entity spawn policy audit: `34/34`.
   - Alchemy automation device audit: `11/11`.
   - Brain-in-a-Jar behavior audit: `10/10`.
   - Warp behavior audit: `9/9`.
   - Warp event behavior audit: `35/35`.
   - Research table diagnostic audit: `59/59`.
   - Thaumonomicon protocol audit: `55/55`.
   - Research page catalog audit: `203 READY`, `0 DEFERRED`, `0 LEGACY_MISSING`.
   - Flux Goo/Taint Fibre blocker audit: `13/13`.
   - Taint ecology blocker audit: `15/15`.
   - Taint mob blocker audit: `15/15`.
...
- Current live catalog audit result: `203 READY`, `0 DEFERRED`, `0 LEGACY_MISSING`.
- Current protocol audit result: `55/55`, including server-owned vanilla crafting, arcane, crucible, infusion, blueprint and fake display page snapshots, selected-stage warp propagation, full categorized knowledge state, runtime PNG category background validation, open-time known-research sibling completion, server-owned recipe-search output aliases, recipe drilldown/history validation, creative locked-start behavior, legacy start visibility, recursive debug `research all`, category completion, `165` icon contracts, `51` validated texture icons, the exact `11` animated seal spritesheets, new-research/page flag exposure, blank empty knowledge totals and inventory-style stack tooltips.
...
- Do not expand this further into rift visual parity, exact taint mob renderer pixel parity or Thaumcraft-biome spawn expansion, Thaumic Slime polish, essentia networks, remaining automation families or exact fluid render/particle parity without a new focused slice. The legacy crucible item entity path is collision-only; avoid adding a fake radius pull to the crucible.
