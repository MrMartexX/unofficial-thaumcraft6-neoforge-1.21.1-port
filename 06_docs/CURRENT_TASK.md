...
- Warp now has a focused server-authoritative mutation/sync bridge: `TCWarpManager` owns add/reduce/set/clear, sync payloads, client action-bar messages, FIRSTSTEPS-gated WARP unlock and the legacy `jar_brain` item-warp crafting bridge with `wussMode` suppression. `06_docs/audits/generated/thaumcraft_1_21_warp_behavior_audit.md` passes `9/9`; full random `WarpEvents` symptoms/hallucinations remain a focused follow-up subsystem.
...
1. Continue blocker-removal work outside the focus/caster layer unless explicitly requested.
   ...
   - Row 7/15 now has the focused Brain-in-a-Jar server/theorycraft blocker slice, the registered `CardTruth` Eldritch theorycraft card, and the focused `jar_brain` item-warp crafting bridge. Keep exact animated model parity, full random `WarpEvents`, `AidBasicEldritch`, Crimson portal aid and broader golem/mind behavior as separate focused work.
