...
1. Continue blocker-removal work outside the focus/caster layer unless explicitly requested.
   ...
   - Row 7/15 now has the focused Brain-in-a-Jar server/theorycraft blocker slice, the registered `CardTruth` Eldritch theorycraft card, the focused `jar_brain` item-warp crafting bridge and the first server-side `WarpEvents` behavior slice with real Guardian/Spider/lesser-portal spawn foundations, the Guardian orb projectile path and lesser portal CultistKnight/CultistCleric minion spawning. Keep exact animated model parity, WarpEvents client hallucination visuals, exact Guardian/orb/cultist/portal renderer pixel parity, CultistCleric GolemOrb branch, `AidBasicEldritch`, Crimson portal aid and broader golem/mind behavior as separate focused work.
...
2. Keep already closed behavior audits green after related changes:
   ...
   - Entity foundation audit: `39/39`.
   - Special alchemy behavior audit: `24/24`.
   ...
   - Warp event behavior audit: `31/31`.
