...
- The custom entity foundation blocker now includes the first WarpEvents outcome entity foundations, the Eldritch Orb projectile path, the GolemOrb projectile path, Alumentum projectile foundation, Causality Collapser rift-closing projectile, Angry/Furious Zombie foundations, the legacy Angry Zombie overworld natural-spawn row and the lesser Crimson portal minion pair without broad mob/golem scope: `TCEntityTypes` registers `SpecialItem`, `FollowItem`, `Alumentum`, `CausalityCollapser`, `BottleTaint`, `EldritchOrb`, `GolemOrb`, `FluxRift`, `ArcaneBore`, `FallingTaint`, `TaintSeed`, `TaintSeedPrime`, `Wisp`, `BrainyZombie`, `GiantBrainyZombie`, `ThaumSlime`, `TaintCrawler`, `Taintacle`, `TaintacleTiny`, `TaintSwarm`, `CultistPortalLesser`, `CultistKnight`, `CultistCleric`, `MindSpider` and `EldritchGuardian`, preserves the 43-entry legacy entity registry catalog as explicit metadata, renders item foundations through the vanilla item renderer where appropriate, gives FallingTaint a block-model renderer foundation, gives Angry/Furious Zombies a legacy `bzombie.png` zombie-model renderer foundation, gives EldritchOrb a source-informed particle/tendril renderer contract and gives GolemOrb the legacy electric-orb particle renderer foundation. `06_docs/audits/generated/thaumcraft_1_21_entity_foundation_audit.md` now passes `51/51` checks.
