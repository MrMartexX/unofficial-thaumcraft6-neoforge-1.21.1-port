...
- Thaumonomicon recipe-page protocol hardening now validates server-owned vanilla crafting, arcane, crucible and infusion page snapshots. The protocol audit at `06_docs/audits/generated/thaumcraft_1_21_thaumonomicon_protocol_audit.md` passes `34/34`; the page catalog audit reports `190 READY`, `13 DEFERRED`, `0 LEGACY_MISSING`. The remaining deferred infusion ids are fake/display page references, not live infusion recipe snapshots.
- Thaumonomicon row-8 navigation now has functional search/filter and recipe drilldown/history slices. The browser search box filters only the current server-authoritative visible index, search-result clicks reuse the same server action/revision payload as graph clicks, recipe stack click-through sends a server-owned drilldown request and returns an authoritative page snapshot, and the protocol audit report no longer churns on raw revision hash values. Final visual parity and fake/blueprint/special page renderers remain separate focused work.
...
2. Keep already closed behavior audits green after related changes:
   ...
   - Thaumonomicon protocol audit: `34/34`.
   - Research page catalog audit: `190 READY`, `13 DEFERRED`, `0 LEGACY_MISSING`.
...
- Current protocol audit result: `34/34`, including server-owned vanilla crafting, arcane, crucible and infusion page snapshots plus recipe drilldown/history validation.
