...
- Thaumonomicon recipe-page protocol hardening now validates server-owned vanilla crafting, arcane, crucible and infusion page snapshots. The protocol audit at `06_docs/audits/generated/thaumcraft_1_21_thaumonomicon_protocol_audit.md` passes `31/31`; the page catalog audit reports `190 READY`, `13 DEFERRED`, `0 LEGACY_MISSING`. The remaining deferred infusion ids are fake/display page references, not live infusion recipe snapshots.
- Thaumonomicon row-8 navigation now has the first functional search/filter slice. The browser search box filters only the current server-authoritative visible index, search-result clicks reuse the same server action/revision payload as graph clicks, and the protocol audit report no longer churns on raw revision hash values. Recipe drilldown/history and final visual parity remain separate focused work.
...
2. Keep already closed behavior audits green after related changes:
   ...
   - Thaumonomicon protocol audit: `31/31`.
...
- Current protocol audit result: `31/31`, including server-owned vanilla crafting, arcane, crucible and infusion page snapshots.
