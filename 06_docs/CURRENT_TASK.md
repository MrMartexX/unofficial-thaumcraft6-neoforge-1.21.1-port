...
- The custom entity foundation blocker now includes the first WarpEvents outcome entity foundations, the Eldritch Orb projectile path, the GolemOrb projectile path, Alumentum projectile foundation, Causality Collapser rift-closing projectile and the lesser Crimson portal minion pair without broad mob/golem scope: `TCEntityTypes` registers `SpecialItem`, `FollowItem`, `Alumentum`, `CausalityCollapser`, `BottleTaint`, `EldritchOrb`, `GolemOrb`, `FluxRift`, `ArcaneBore`, `FallingTaint`, `TaintSeed`, `TaintSeedPrime`, `Wisp`, `ThaumSlime`, `TaintCrawler`, `Taintacle`, `TaintacleTiny`, `TaintSwarm`, `CultistPortalLesser`, `CultistKnight`, `CultistCleric`, `MindSpider` and `EldritchGuardian`, preserves the 43-entry legacy entity registry catalog as explicit metadata, renders item foundations through the vanilla item renderer where appropriate, gives FallingTaint a block-model renderer foundation, gives EldritchOrb a source-informed particle/tendril renderer contract, gives GolemOrb the legacy electric-orb particle renderer foundation, and uses explicit no-op placeholders for newly server-complete mobs until measured visual parity lands. `06_docs/audits/generated/thaumcraft_1_21_entity_foundation_audit.md` now passes `45/45` checks.
...
1. Continue blocker-removal work outside the focus/caster layer unless explicitly requested.
   ...
   - Row 14 now has an entity registry foundation, safe legacy item/projectile entity implementations including `Alumentum`, `BottleTaint` and `CausalityCollapser`, focused Flux Rift/Arcane Bore entity foundations, Wisp behavior/render/spawn contracts, and the first taint mob server behavior contracts. Do not expand this into golems, focus projectiles/clouds/mines or broad custom renderers without a focused design/audit slice.
