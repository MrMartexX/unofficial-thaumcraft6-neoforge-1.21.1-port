...
| 10 | Thaumatorium / advanced alchemy machines | First server machine foundation is implemented over stable crucible recipe data and real essentia transport. Remaining work is GUI/screen recipe selection, exact model/renderer parity, and later special alchemy/device integrations. | Dedicated Thaumatorium behavior audit `16/16`, transport capability audit `61/61`, server smoke. | Automated alchemy foundation and alchemical construct identity blockers removed; final UX/visual polish still blocked. |
...
5. Row 10 Thaumatorium has its first real server foundation: two-block placement, catalyst slot, selected recipe ids, top delegation, heat/redstone checks, suction/fill/completion behavior and dedicated audit `16/16`. Final GUI/screen and visual parity remain open.
...
- choose whether to finish Thaumatorium GUI/screen/model parity now or defer it behind the next standalone device blocker;
- if deferring Thaumatorium polish, move to row 13 standalone devices with one design/audit per device family;
