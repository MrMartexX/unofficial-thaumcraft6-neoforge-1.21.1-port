...
| 4 | Remaining essentia utility devices | After transport controls exist, devices that consume/produce essentia can be implemented without inventing ad hoc paths. Essentia Input/Output transfusers, Void Jar and Essentia Mirror source bridging are now implemented as real blocks/BlockEntities where applicable. | Dedicated behavior audits per device family, combined transport audit `61/61`, server smoke. | Thaumatorium input/output dependency blockers; importer/exporter placeholder policy; normal item mirror/hand mirror remain row-13 device work. |
...
4. Remaining essentia utility devices are now advanced enough to unblock Thaumatorium planning: `essentiatransportin` and `essentiatransportout` are real blocks using the legacy `TileEssentiaInput` / `TileEssentiaOutput` contract; `jar_void` is a real Void Jar with overflow/continuing suction; `mirror_essentia` is a real linked source bridge with no direct pipe endpoint. The combined transport audit passes `61/61`.
...
- finish the remaining row-4 placeholder policy decision for `essentia_importer` / `essentia_exporter`: keep them as hidden reference-only aliases or remove them from player-facing flows;
- keep every utility device covered by focused behavior checks in the combined transport audit or a dedicated device audit;
- design and implement the row-10 Thaumatorium slice over the now-real input/output transfusers, Void Jar, Essentia Mirror, crucible semantics, jar/alembic/phial behavior and research-page state;
