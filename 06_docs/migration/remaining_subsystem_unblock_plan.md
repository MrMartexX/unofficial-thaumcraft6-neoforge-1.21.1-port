...
9. The remaining row-7 blocker is additional focus medium/effect execution plus Focus Pouch inventory/menu behavior and final Focal Manipulator editor visual parity.

## Immediate Next Batch

The next large safe batch should continue row 7, not reopen row 6:

- expand the first server-owned cast-effect execution path from audited `TOUCH/FIRE` into one additional medium/effect family;
- keep each medium/effect combination controlled before projectile/cloud/mine/entity-heavy paths;
