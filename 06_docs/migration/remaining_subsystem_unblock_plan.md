...
7. Row 10 Thaumatorium has its first real server foundation and functional recipe-selection screen: two-block placement, catalyst slot, selected recipe ids, top delegation, heat/redstone checks, research-gated catalyst recipe listing, server-side add/remove toggle validation, suction/fill/completion behavior and dedicated audit `19/19`. Exact legacy model/renderer/pixel parity remains open under row 17.
8. Arcane Workbench regular recipes are parity-closed at `89/89`; the recipe audit passes `109/109`, and the workbench behavior audit passes `28/28` including goggles, cloth robe, void robe and provider-bridge vis discounts.
9. Item/equipment behavior contracts are closed for goggles, cloth robes, void robes, sanity checker, sane soap and Crimson Rites. The runtime item/equipment audit passes `17/17`; the Arcane Workbench audit remains `28/28` after the armor conversion.
10. Focus/caster/Focal Manipulator core is implemented for the first stable data/behavior boundary. Runtime focus/caster audit passes `10/10`: root + 20 focus element definitions, caster/focus/focus-pouch identities, focus package/default-setting math, selected focus Data Component, area-0 aura drain, Focal Manipulator design request validation, XP/crystal/vis start contract and focus package writeback.
11. The first server-owned cast-effect execution slice is implemented for legacy `ROOT -> TOUCH -> FIRE`. Runtime focus cast execution audit passes `5/5`: supported plan validation, unsupported package fail-before-drain, supported caster vis/cooldown, entity damage/burn and block fire placement.
12. Row 7 remains started but paused: additional focus medium/effect execution, Focus Pouch inventory/menu behavior and final Focal Manipulator editor visual parity are not the immediate active blocker-removal target.
13. Row 14 has the first custom entity foundation slice: `TCEntityTypes` registers `SpecialItem` as `thaumcraft:special_item` and `FollowItem` as `thaumcraft:follow_item`, preserves the exact 43-entry legacy entity registration catalog, and passes the dedicated entity foundation audit `7/7`. This does not implement mob AI, attributes, spawn rules, Flux Rift, projectiles/clouds/mines, Arcane Bore/turrets, custom renderers or golems.
...
- if deferring visual polish, continue row 13 standalone devices with the next family after normal mirror/hand mirror and lamps, preferably Arcane Bore or Infernal Furnace as separate audited slices;
