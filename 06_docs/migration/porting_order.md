...
| Stage 11. Entities / Golems | In progress | The registry foundation now covers `special_item`, `follow_item`, `alumentum`, `causality_collapser`, `bottle_taint`, `eldritch_orb`, `golem_orb`, `flux_rift`, `arcane_bore`, `falling_taint`, `taint_seed`, `taint_seed_prime`, `wisp`, `thaum_slime`, `taint_crawler`, `taintacle`, `taintacle_tiny`, `taint_swarm`, `cultist_portal_lesser`, `cultist_knight`, `cultist_cleric`, `mind_spider` and `eldritch_guardian`; the full 43-entry legacy catalog is audit-visible and the foundation audit passes `45/45` checks. Alumentum, BottleTaint item/projectile behavior and special alchemy fluid/effect behavior are covered by a `32/32` special alchemy audit. Causality Collapser rift-closing behavior is covered by the Flux Rift consequence audit. Wisp behavior/render contracts are covered by an `11/11` focused audit; Wisp Nether spawn policy is covered by a `10/10` focused audit; taint mob server hooks are covered by a `14/14` focused audit; FallingTaint crust physics is covered by a `10/10` focused audit; Flux Rift consequences excluding the focus-cloud owner event are covered by a `16/16` focused audit. Focus projectiles/clouds/mines, custom mob render pixel parity, exact Alumentum/Causality/BottleTaint/FallingTaint landing particles/render pixel parity, non-Wisp natural spawns and golems remain deferred. |
