...
| Stage 11. Entities / Golems | In progress | The registry foundation now covers `special_item`, `follow_item`, `flux_rift`, `arcane_bore`, `falling_taint`, `taint_seed`, `taint_seed_prime`, `thaum_slime`, `taint_crawler`, `taintacle`, `taintacle_tiny` and `taint_swarm`; the full 43-entry legacy catalog is audit-visible and the foundation audit passes `23/23`. Taint mob server hooks are covered by a `14/14` focused audit; FallingTaint crust physics is covered by a `10/10` focused audit. Projectiles/clouds/mines, custom mob renderers, exact FallingTaint landing particles/render pixel parity, broad natural spawns and golems remain deferred. |
