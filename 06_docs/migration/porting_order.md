...
| Stage 11. Entities / Golems | In progress | The first registry foundation exists: `special_item` and `follow_item` are real item-entity types, the full 43-entry legacy entity catalog is audit-visible, and the dedicated foundation audit passes `7/7`. Mob AI, attributes, spawn rules, projectiles/clouds/mines, custom renderers and golems remain deferred. |
