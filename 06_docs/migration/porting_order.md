...
| Stage 11. Entities / Golems | In progress | The registry foundation now covers `special_item`, `follow_item`, `flux_rift`, `arcane_bore`, `falling_taint`, `taint_seed`, `taint_seed_prime`, `wisp`, `thaum_slime`, `taint_crawler`, `taintacle`, `taintacle_tiny` and `taint_swarm`; the full 43-entry legacy catalog is audit-visible and the foundation audit passes `25/25`. Wisp behavior/render contracts are covered by an `11/11` focused audit; Wisp Nether spawn policy is covered by a `10/10` focused audit; taint mob server hooks are covered by a `14/14` focused audit; FallingTaint crust physics is covered by a `10/10` focused audit; Flux Rift consequences excluding the focus-cloud owner event are covered by an `11/11` focused audit. Projectiles/clouds/mines, custom mob render pixel parity, exact FallingTaint landing particles/render pixel parity, non-Wisp natural spawns and golems remain deferred. |
