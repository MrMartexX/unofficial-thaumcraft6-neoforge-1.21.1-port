...
| Stage 8. Tile Entities | In progress | Research Table, Arcane Workbench, Crucible, Infusion Matrix/Pedestals, Focal Manipulator, tubes, Warded Jar, Void Jar, Essentia Mirror, Alembic, Bellows, all three smelter tiers and the first Thaumatorium machine foundation own persisted server state. The combined transport/smelter/label/phial/caster-control/Void-Jar/essentia-mirror audit passes `61/61`; Thaumatorium passes `19/19`; Bellows and focus/caster core have dedicated audits. |
| Stage 9. GUI | In progress | Research Table, Arcane Workbench, smelter, Thaumatorium recipe selection, Focal Manipulator minimal core screen and first Thaumonomicon screens exist as functional slices; final visual parity and recipe drilldown remain incomplete. |
