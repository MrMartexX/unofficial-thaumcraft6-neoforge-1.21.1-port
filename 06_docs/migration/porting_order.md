...
| Stage 7. Crafting | In progress | Vanilla fixtures, exact arcane recipes, Arcane Workbench behavior, crucible recipe/page data, first manual in-world crucible recipe-consumption slice, and the non-consuming infusion recipe/page start-plan plus completion-readiness boundary with matrix/pedestal discovery are active; flux/special alchemy, item-entity suction, infusion mutation/instability/essentia/FX and remaining recipe families are deferred. |
| Stage 8. Tile Entities | In progress | Research Table, Arcane Workbench, first Crucible, Infusion Matrix, and Infusion Pedestal BlockEntity slices exist; broad machine ticking and essentia-capable machines remain deferred. |
