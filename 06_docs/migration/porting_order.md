...
| Stage 2. Items / Materials | In progress | Identity, bridge items, selected materials, flattened legacy nugget/chunk bonus outputs, models, lang, tags, creative ordering, the first audited wearable/utility behavior contracts and the first caster/focus item Data Component core are active; many dependency-heavy item behaviors remain deferred. |
| Stage 3. Blocks / Basic Resources | In progress | Basic blocks, candles, tables, Arcane Workbench, Arcane/Growth/Fertility lamps and Infernal Furnace exist; all six tube blocks now use legacy multipart connection topology and shapes. Broad machine behavior remains incomplete. |
...
| Stage 6. Research | In progress | Data/progression/page-catalog core, scan knowledge, research table slice and current-scope Thaumonomicon flow exist, including server-authoritative search, drilldown/history, recipe tabs, aspect/knowledge side inserts and selected-stage warp warning. Measured pixel-level browser/entry polish and later dependency-heavy research content remain blocked. |
...
| Stage 8. Tile Entities | In progress | Research Table, Arcane Workbench, Crucible, Infusion Matrix/Pedestals, Focal Manipulator, tubes, Warded Jar, Void Jar, Essentia Mirror, normal item Mirror, Arcane/Growth/Fertility Lamps, Infernal Furnace, Alembic, Bellows, all three smelter tiers and the first Thaumatorium machine foundation own persisted server state. The combined transport/smelter/label/phial/caster-control/Void-Jar/essentia-mirror audit passes `61/61`; normal mirror/hand mirror passes `12/12`; lamp device audit passes `10/10`; Infernal Furnace passes `12/12`; Thaumatorium passes `19/19`; Bellows and focus/caster core have dedicated audits. |
| Stage 9. GUI | In progress | Research Table, Arcane Workbench, smelter, Hand Mirror sender, Thaumatorium recipe selection, Focal Manipulator minimal core screen and first Thaumonomicon screens exist as functional slices; Thaumonomicon search, recipe drilldown/history, blueprint construct pages, fake display pages, recipe tabs, side inserts and selected-stage warp warning are server-authoritative. Measured pixel-level visual parity and future owner-specific screen polish remain incomplete. |
| Stage 10. Networking | In progress | Aura, knowledge, research table, Thaumonomicon protocol version `8`, both clientbound infusion FX payload contracts, Wisp zap FX payload and the Focal Manipulator design-intent payload exist; future gameplay payloads need subsystem-specific design. |
| Stage 11. Entities / Golems | In progress | The registry foundation now covers `special_item`, `follow_item`, `alumentum`, `causality_collapser`, `bottle_taint`, `eldritch_orb`, `golem_orb`, `flux_rift`, `focus_cloud`, `arcane_bore`, `falling_taint`, `taint_seed`, `taint_seed_prime`, `wisp`, `firebat`, `pech`, `brainy_zombie`, `giant_brainy_zombie`, `thaum_slime`, `taint_crawler`, `taintacle`, `taintacle_tiny`, `taint_swarm`, `cultist_portal_lesser`, `cultist_knight`, `cultist_cleric`, `mind_spider`, `eldritch_guardian`, `cultist_portal_greater`, `cultist_leader`, `taintacle_giant`, `eldritch_crab` and `inhabited_zombie`; the full 43-entry legacy catalog is audit-visible and the foundation audit passes `71/71` checks. Alumentum, BottleTaint item/projectile behavior and special alchemy fluid/effect behavior are covered by a `32/32` special alchemy audit. Causality Collapser rift-closing behavior and Flux Rift consequences including the rift-owned focus-cloud event are covered by the `21/21` Flux Rift consequence audit. Wisp behavior/render contracts are covered by an `11/11` focused audit; Wisp Nether, Angry Zombie overworld, Firebat Nether/Halloween and Pech optional magical-biome spawn policy are covered by a `34/34` focused audit. Taint mob server hooks are covered by a `15/15` focused audit; FallingTaint crust physics is covered by a `10/10` focused audit. Focus projectiles/clouds/mines, custom mob render pixel parity, exact Alumentum/Causality/BottleTaint/Firebat/FallingTaint landing particles/render pixel parity, real Thaumcraft-biome providers and golems remain deferred. |
...
| Stage 13. Rendering / Particles | In progress | Thaumometer effects, table renderer, infusion FX, crucible FX, Wisp billboard/ambient/zap FX, legacy multipart tube geometry and valve/vent state rendering exist; lamp block/item models are functional for the device slice, but measured visual parity and broad render/BEWLR/shader work remain high risk. |
