...
| Stage 11. Entities / Golems | In progress | The registry foundation now covers `special_item`, `follow_item`, `flux_rift`, `arcane_bore`, `taint_seed`, `taint_seed_prime`, `thaum_slime`, `taint_crawler`, `taintacle`, `taintacle_tiny` and `taint_swarm`; the full 43-entry legacy catalog is audit-visible and the foundation audit passes `21/21`. Taint mob server hooks are covered by a `14/14` focused audit. Projectiles/clouds/mines, custom mob renderers, FallingTaint, broad natural spawns and golems remain deferred. |
