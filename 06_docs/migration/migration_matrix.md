...
| 21 | Infusion crafting | Custom recipe type, modern BE scanning, payload FX, stability service | Critical | Keep recipe/page/start-plan boundary separate from item consumption, instability and FX | Started narrowly: `thaumcraft:infusion` recipe serializer/page snapshots, `TCInfusionRecipeMatcher`, `TCInfusionAssembly`, `TCInfusionValidationResult`, `TCInfusionCraftingPlan`, one-slot pedestal BlockEntities, matrix BlockEntity legacy-range pedestal discovery, active `arcane_pedestal`/`ancient_pedestal`/`eldritch_pedestal` block ids, and `audit-infusion-behavior.ps1` exist. Legacy-compatible component matching is unordered but exact 1:1 by count. The current runtime audit passes `20/20` and covers start plan creation, recorded fields, pedestal positions, NBT round-trip, second-start rejection and abort. Item consumption timing, instability, essentia drain, beams, particles, sounds and completion behavior remain deferred. |
