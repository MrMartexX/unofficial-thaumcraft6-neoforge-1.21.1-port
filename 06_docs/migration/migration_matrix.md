...
| 21 | Infusion crafting | Custom recipe type, modern BE scanning, payload FX, stability service | Critical | Keep server progression authoritative and visual effects client-only | In progress: recipe/page/start/readiness data, persisted exact craft cycle/stability, exact altar structure profile and legacy stabilizer symmetry are implemented. The matrix advances at the profile-derived cadence, drains one essentia point from cached nearest sources, honors the legacy 200-tick failed-pass delay, spends six cycles per component, preserves remainders, completes on the next cycle, carries damage ratio, and sends the two legacy-shaped FX payloads. Exact stability thresholds, `5/6/7/8` loss modifiers, clamp/trigger/recovery ordering and all 24 instability event mappings are active. Eighteen rolls execute reviewed item/flux/zap/warp/explosion effects; six Flux Goo/custom-effect rolls fail closed. Current runtime audit is `83/83`. Player activation, missing event dependencies, pedestal mitigators, live surroundings invalidation and exact stream/debris rendering remain deferred. |
