...
- Brain-in-a-Jar, Crimson portal, Basic Eldritch, Dragon Egg theorycraft, remaining custom entities, full essentia behavior, full infusion matrix/pedestal gameplay, and broad render/worldgen systems remain deferred/reference-only until their dependencies are designed.
...
| 21 | Infusion crafting | Custom recipe type, modern BE scanning, payload FX, stability service | Critical | Keep recipe/page/validation boundary separate from item consumption, instability and FX | Started narrowly: `thaumcraft:infusion` recipe serializer/page snapshots, `TCInfusionRecipeMatcher`, `TCInfusionAssembly`, `TCInfusionValidationResult`, and `audit-infusion-behavior.ps1` exist. Legacy-compatible component matching is unordered but exact 1:1 by count. Full matrix/pedestal BlockEntity discovery, item consumption timing, instability, essentia drain, beams, particles, sounds and completion behavior remain deferred. |
