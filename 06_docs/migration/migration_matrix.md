...
| 21 | Infusion crafting | Custom recipe type, modern BE scanning, payload FX, stability service | Critical | Keep server progression authoritative and visual effects client-only | In progress: recipe/page/start/readiness data plus a persisted exact default cycle are implemented. The matrix advances every five ticks, drains one essentia point from cached nearest sources, honors the legacy 200-tick failed-pass delay, spends six cycles per component, preserves remainders, completes on the next cycle, carries damage ratio, and sends the two legacy-shaped FX payloads. Runtime audit passes `63/63`. Player activation, `getSurroundings()` structure modifiers, stability/instability events and exact stream/debris rendering remain deferred. |
