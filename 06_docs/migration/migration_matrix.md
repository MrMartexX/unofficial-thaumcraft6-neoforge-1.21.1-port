...
| 14 | Essentia transport API | Sided `BlockCapability` plus explicit internal transport interface | Very high | Preserve legacy suction/capacity/cadence while exposing modern sided lookup | In progress with a complete first consumer chain: six tube variants, Warded Jar and Alembic use `TCEssentiaCapabilities.BLOCK`; smelters remain internal slurry machines and output through matching-first Alembic columns/direct auxiliary pumps. Combined transport/machine runtime audit passes `46/46`, including labels, phial/jar transfer and caster tube controls; Bellows has a dedicated device/rendering audit. Remaining utility devices and final measured visual parity remain. |
