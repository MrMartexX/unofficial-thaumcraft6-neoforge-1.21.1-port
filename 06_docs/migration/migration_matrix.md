...
| 23 | BlockEntity base classes | `BlockEntity`, `EntityBlock#getTicker`, `CompoundTag`, `setChanged` | Very high | Persist machine state and register explicit server tickers | Research Table, Arcane Workbench and Crucible slices persist/sync their current state. `TCInfusionPedestalBlockEntity` owns one item; `TCInfusionMatrixBlockEntity` now persists the active plan and per-cycle aspects/components/countdown and runs a server-only ticker. `TCWardedJarBlockEntity` persists/filter-blocks one aspect up to 250. Broad capability networks and remaining machines are deferred. |
