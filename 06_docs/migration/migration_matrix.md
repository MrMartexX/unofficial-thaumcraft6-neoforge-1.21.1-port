...
| 21 | Infusion crafting | Custom recipe type, modern BE scanning, payload FX, stability service | Critical | Keep server progression authoritative and visual effects client-only | In progress: recipe/page/start/readiness data, persisted exact craft cycle, exact altar structure profile and legacy stabilizer symmetry are implemented. The matrix advances at the profile-derived cadence, drains one essentia point from cached nearest sources, honors the legacy 200-tick failed-pass delay, spends six cycles per component, preserves remainders, completes on the next cycle, carries damage ratio, and sends the two legacy-shaped FX payloads. Ancient/eldritch pillars, four matrix modifier positions, ancient/eldritch pedestal cost effects, cost flooring/truncation and candle/skull/pedestal diminishing returns are runtime-audited `72/72`. Player activation, persistent stability loss and instability event effects, live surroundings invalidation and exact stream/debris rendering remain deferred. |
