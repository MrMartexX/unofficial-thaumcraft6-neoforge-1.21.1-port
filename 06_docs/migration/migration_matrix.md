...
| 14 | Essentia transport API | Sided `BlockCapability` plus explicit internal transport interface | Very high | Preserve legacy suction/capacity/cadence while exposing modern sided lookup | In progress with a complete first consumer chain: six tube variants, Warded Jar, Void Jar, Alembic and Essentia Input/Output transfusers use `TCEssentiaCapabilities.BLOCK`; Essentia Mirror is deliberately not a direct pipe endpoint and instead bridges remote source containers through a Data Component-linked pair. Smelters remain internal slurry machines and output through matching-first Alembic columns/direct auxiliary pumps. Combined transport/machine runtime audit passes `61/61`, including labels, phial/jar transfer, Void Jar overflow/suction, essentia mirror link/source behavior, caster tube controls and the legacy five-tick transfuser remote-source path; Bellows has a dedicated device/rendering audit. Final measured mirror/valve/vent visual parity and normal item mirror/hand mirror behavior remain. |
