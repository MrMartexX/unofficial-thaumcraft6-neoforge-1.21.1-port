...
- The `thaumcraft:crucible` serializer/page-data boundary exists for HEDGE_ALCHEMY, METAL_PURIFICATION, Alumentum, Nitor, Brass/Thaumium Ingots, and vis crystals. Dynamic legacy HEDGE_ALCHEMY costs are explicit data and the current crucible recipe data audit reports `77/77` valid files. A first in-world crucible slice now exists for server-owned water/heat/aspect state, manual item insertion, research-gated recipe lookup, aspect-cost consumption and result ejection; flux, item-entity suction, special alchemy, essentia integration and particles remain deferred.
...
| 22 | Crucible and alchemy | Custom recipe types, BE save/tick, capability-based essentia access | Critical | Keep manual in-world behavior separate from full alchemy side effects and automation | Started narrowly: `thaumcraft:crucible` recipe type/serializer, explicit aspect costs, catalyst variants, server Thaumonomicon page snapshots, codec/payload wiring, HEDGE_ALCHEMY, METAL_PURIFICATION, Alumentum, Nitor, Brass/Thaumium Ingots and vis crystal recipe data are implemented. The first in-world slice adds `TCCrucibleBlock`, `TCCrucibleBlockEntity`, and `TCCrucibleRecipeMatcher` for water/heat/aspect state, manual top-side item insertion, existing item-aspect dissolution, research-gated highest-cost recipe lookup, aspect/water consumption, result ejection and comparator output. Item-entity absorption, flux/taint pollution, liquid death/special alchemy behavior, essentia/alembic/jar/tube/Thaumatorium integration, crucible-derived `generateTags`, and final visuals remain blocked. |
| 23 | BlockEntity base classes | `BlockEntity`, `EntityBlock#getTicker`, `CompoundTag`, `setChanged` | Very high | Create one simple non-inventory BE first | Started narrowly: `TCResearchTableBlockEntity` saves/drops scribing tools and paper slots, owns theory data under the legacy `note` tag, provides the server menu container, and syncs update packets/tags for the table-top renderer. `TCArcaneWorkbenchBlockEntity` stores the legacy 9 matrix + 6 crystal slot shape, saves/loads/drops contents, opens a server menu, and delegates no-charger current-chunk plus Charger 3 x 3 vis checks/drain to the aura core. `TCCrucibleBlockEntity` stores water/heat/aspect state, ticks heat from legacy-compatible heat sources, saves/loads NBT, syncs updates, and owns the first manual in-world crucible mutation path. Broad machine ticking and capability networks remain deferred. |
