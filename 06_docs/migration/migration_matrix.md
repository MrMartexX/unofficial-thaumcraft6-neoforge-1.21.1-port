...
| 14 | Essentia transport API | Sided `BlockCapability` plus explicit internal transport interface | Very high | Preserve legacy suction/capacity/cadence while exposing modern sided lookup | In progress with a complete first consumer chain: six tube variants, Warded Jar, Void Jar, Alembic, Thaumatorium and Essentia Input/Output transfusers use `TCEssentiaCapabilities.BLOCK`; Essentia Mirror is deliberately not a direct pipe endpoint and instead bridges remote source containers through a Data Component-linked pair. Smelters remain internal slurry machines and output through matching-first Alembic columns/direct auxiliary pumps. Combined transport/machine runtime audit passes `61/61`, and the dedicated Thaumatorium audit passes `19/19` with recipe-list/toggle checks. Final measured mirror/valve/vent/Thaumatorium visual parity and normal item mirror/hand mirror behavior remain. |
...
| 23 | BlockEntity base classes | `BlockEntity`, `EntityBlock#getTicker`, `CompoundTag`, `setChanged` | Very high | Persist machine state and register explicit server tickers | Research Table, Arcane Workbench, Crucible, Infusion, tubes, Warded Jar, Alembic, Bellows and all three smelter tiers persist/sync their owned state. Smelters now include exact tier/fuel/aspect/vent/Alembic output behavior; Bellows owns target classification, client inflation state and vanilla-furnace boost counters. |
| 24 | Menus and machine GUI | `AbstractContainerMenu`, `MenuType`, `Screen`, data slots, payload actions | Very high | Port one simple inventory menu first | Research Table and Arcane Workbench menus/screens are active. Smelter now has the legacy two-slot layout, five DataSlot progress values, server-owned quick-move validation and original GUI texture coordinates through `TCSmelterMenu`/`TCSmelterScreen`. Page/card animations and final machine visual parity remain. |
