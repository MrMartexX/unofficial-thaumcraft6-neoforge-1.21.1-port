...
| 14 | Essentia transport API | Sided `BlockCapability` plus explicit internal transport interface | Very high | Preserve legacy suction/capacity/cadence while exposing modern sided lookup | In progress: all six tube variants and Warded Jar use `TCEssentiaCapabilities.BLOCK`; server runtime audit passes `23/23`. Bellows, caster controls, Alembic and real smelter processing remain. |
...
| 23 | BlockEntity base classes | `BlockEntity`, `EntityBlock#getTicker`, `CompoundTag`, `setChanged` | Very high | Persist machine state and register explicit server tickers | Research Table, Arcane Workbench, Crucible and Infusion slices persist/sync current state. All six tube variants now tick server-side and persist directional/suction/storage state; Warded Jar persists one aspect up to 250. Alembic, Bellows and real smelter machines remain. |
