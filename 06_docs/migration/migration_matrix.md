...
- Gate 6 is in progress: arcane recipe type/serializers, selected exact arcane fixtures, Arcane Workbench server crafting, crucible gameplay slices, infusion recipe/page data, audit-only mutation, and the first real Warded Jar source-container relationship exist.
...
| 14 | Essentia transport API | Sided `BlockCapability` plus explicit internal transport interface | Very high | Preserve legacy suction/capacity/cadence while exposing modern sided lookup | In progress with a complete first consumer chain: six tube variants, Warded Jar, Alembic and Essentia Input/Output transfusers use `TCEssentiaCapabilities.BLOCK`; smelters remain internal slurry machines and output through matching-first Alembic columns/direct auxiliary pumps. Combined transport/machine runtime audit passes `52/52`, including labels, phial/jar transfer, caster tube controls and the legacy five-tick transfuser remote-source path; Bellows has a dedicated device/rendering audit. Void Jar, essentia mirrors and final measured visual parity remain. |
