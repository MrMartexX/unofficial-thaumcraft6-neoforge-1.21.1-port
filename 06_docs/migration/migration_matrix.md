...
| 21 | Infusion crafting | Custom recipe type, modern BE scanning, payload FX, stability service | Critical | Keep server progression authoritative and visual effects client-only | In progress: recipe/page/start/readiness data, persisted exact craft cycle/stability, exact altar structure profile and legacy stabilizer symmetry are implemented. The matrix advances at the profile-derived cadence, drains one essentia point from cached nearest sources, honors the legacy 200-tick failed-pass delay, spends six cycles per component, preserves remainders, completes on the next cycle, carries damage ratio, and sends the two legacy-shaped FX payloads. Exact stability thresholds, `5/6/7/8` loss modifiers, clamp/trigger/recovery ordering and all 24 instability effects are active. Flux Goo placement, Flux Taint/Vis Exhaust, inlay charge, Stabilizer energy/recharge and pedestal ejection mitigation are implemented. Current runtime audit is `86/86`. Player activation, live surroundings invalidation, full goo/taint world behavior and exact stream/debris rendering remain deferred. |
