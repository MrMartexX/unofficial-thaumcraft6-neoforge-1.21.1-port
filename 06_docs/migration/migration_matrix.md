...
- Full focus effect execution, Focus Pouch inventory GUI, Brain-in-a-Jar, Crimson portal, Basic Eldritch, Dragon Egg theorycraft, remaining custom mob/projectile/construct/golem behavior, Arcane Bore and Flux Rift lifecycle/rendering after the first Void Siphon foundation, final measured infusion/equipment/focus/lamp/device visual tuning, and broad render/worldgen systems remain deferred/reference-only until their dependencies are designed.
...
| Gate 10 | Entities and golems | Entity types, AI, rendering and tasks | Started with registry foundation: `special_item` and `follow_item` are registered/implemented and the full 43-entry legacy entity catalog is audit-visible | Mob AI, spawn rules, attributes, custom renderers, projectiles/clouds/mines, Flux Rift, Arcane Bore/turrets and golems |
...
| 25 | Networking | `CustomPacketPayload`, `StreamCodec`, `PayloadRegistrar`, validated handlers | Very high | Define packet categories and policies first | Started: aura sync, GUI-ready knowledge sync, research-table serverbound action/clientbound sync/result payloads, Thaumonomicon index/entry/action payloads, infusion FX payloads and the Focal Manipulator design-intent payload exist. Thaumonomicon actions validate visibility and stage requirements server-side; explicit open intent is separated from ordinary index refresh. Focal Manipulator design intent validates the open menu/block position and leaves research/cost checks server-side. Each new gameplay subsystem still needs a focused packet inventory and validation policy |
| 26 | Entities and golems | `EntityType`, `SynchedEntityData`, goals, attributes, renderers | Very high | Keep the registered `SpecialItem`/`FollowItem` foundation audited, then split future entity work by projectile/construct/mob/golem family | Started: `TCEntityTypes` registers `special_item` and `follow_item`; `TCEntityFoundationAudit` records the 43 legacy registrations and passes `7/7`. AI, spawn rules, attributes, custom renderers, Thaumcraft custom entity aspect assignments and golem logistics remain deferred. |
