...
- The permanent research recipe/page catalog, server-authoritative Thaumonomicon protocol, real item/open flow, first browser/entry screens, and first vanilla crafting page renderer are implemented. The catalog preserves `253` research occurrences, `203` direct references, and `325` entries including group members with `0` parity differences; the protocol audit passes `25/25`, including the legacy known-entry final-stage progression path, explicit-open-versus-refresh separation, server-built index revision, stale-action rejection without mutation, and server-owned crafting/arcane snapshots.
- The next research/crafting boundary is the first real custom recipe type and its authoritative page snapshot/renderer, not client-side recipe resolution or invented deferred content.
