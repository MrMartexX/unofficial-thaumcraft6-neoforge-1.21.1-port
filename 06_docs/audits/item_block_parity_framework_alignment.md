...
| 65 | Closed: focused visual preset and completion gate require item visual, legacy shape, legacy visual/collision, texture/color, model transform and FX reports before visual parity can be promoted | Visual preset/completion hardening |
...
Batch 62-65 closure: visual-boundary hardening now has a focused `visual` preset and six report-only evidence layers. `legacy_shape_parity` is the conservative source/manifest classifier, `item_visual_parity` is the all-registered-item asset/model placeholder audit, `legacy_visual_collision_parity` is the stricter original-jar/source-backed comparator with a separate `outline_contract` subcheck, `texture_color` maps modern `textures/block|item` refs back to legacy `textures/blocks|items`, and `visual_equivalence_completion` now consumes all visual reports before strict visual certification can be considered. Current known report rows include `research_table` and `golem_builder` facing-domain mismatches, texture review rows, FX review rows, plus item visual missing/review rows; these are parity blockers for future focused fixes, not framework wiring errors.
