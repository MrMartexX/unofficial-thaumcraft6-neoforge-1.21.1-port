...
|---|---:|---:|---:|---|
| Source quality and legacy evidence ($layer=source_quality) | 9 | 3 | 0 | access_transformers, auto_fix_candidates, check_invocation, docs_deferred, legacy_primary_manifest, original_jar_probe, public_api, report_freshness, report_schema, secondary_legacy_probe, source_conflict_report, status_taxonomy |
...
|---|---|---|---|---|---|
| Source quality and legacy evidence ($layer=source_quality) | Establishes what legacy, secondary legacy, original-jar, access-transformer, port and report-contract evidence is available before parity is judged. | legacy_primary_manifest reports, secondary legacy probe reports, access transformer visibility report, auto-fix candidate reports, check invocation self-test, report freshness guard report, report schema contract report, status taxonomy report, docs/registry consistency report | Primary decompile is not absolute authority; original jar and conflict aggregation are still planned; access transformer rows remain review-only until visibility-widening policy is classified. | Primary, secondary, original jar and access-transformer evidence must be schema-valid, fresh and conflict-classified. | Verifier must check schema, invocation, freshness and docs consistency for all source-quality reports. |
