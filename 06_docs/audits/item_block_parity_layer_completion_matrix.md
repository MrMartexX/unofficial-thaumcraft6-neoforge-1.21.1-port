...
| Behavior boundary ($layer=behavior_boundary) | 7 | 2 | 0 | block_properties, blockentities, capabilities, client_server_safety, data_components, equipment, item_properties, menus, networking |
...
| Behavior boundary ($layer=behavior_boundary) | Audits item properties, data component and legacy tag-state evidence, block properties, block entity/menu/capability references, networking and client/server boundaries. | item/block property reports, data component bridge report, behavior boundary report, client/server safety report | networking and equipment checks remain planned; data component rows remain review-only until stack-state policy is classified. | Every behavior dimension must classify static parity, runtime-required evidence and known review gaps. | Verifier must run focused behavior-boundary smoke and later runtime-backed checks. |
