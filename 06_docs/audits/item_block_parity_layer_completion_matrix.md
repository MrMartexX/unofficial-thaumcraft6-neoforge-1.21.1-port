...
| Rule and override governance | Rule override files exist, auto-fix candidate rules are present, and Batch 30 adds a report-only status taxonomy validator for observed report statuses plus a docs/registry consistency audit. | Status taxonomy must remain mapped, and final registry cleanup must remove ambiguous planned statuses. |
