...
| CI and verifier | Verifier v2 is available as `tools/audits/item-block-parity/verify-item-block-parity-framework.ps1` and validates clean/allowed-dirty state, PowerShell parser state, rule JSON, check invocation ownership, docs backlog state, report summaries and report-only audit smoke. | CI must publish verifier artifacts and support report-only/safe/strict modes without committing generated local reports. |
