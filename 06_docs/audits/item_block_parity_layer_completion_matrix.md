...
|---|---:|---:|---:|---|
| Source quality and legacy evidence ($layer=source_quality) | 3 | 5 | 0 | access_transformers, auto_fix_candidates, docs_deferred, legacy_primary_manifest, original_jar_probe, public_api, secondary_legacy_probe, source_conflict_report |
...
|---|---|---|---|---|---|
| Source quality and legacy evidence ($layer=source_quality) | Establishes what legacy, secondary legacy, original-jar and port evidence is available before parity is judged. | legacy_primary_manifest reports, secondary legacy probe reports, auto-fix candidate reports | Primary decompile is not absolute authority; original jar and conflict aggregation are still planned. | Primary, secondary and original jar evidence must be schema-valid, fresh and conflict-classified. | Verifier must check schema, invocation, freshness and docs consistency for all source-quality reports. |
...
| Report governance | Reports are generated consistently enough for current smoke tests; schema and freshness are not yet centrally enforced. | SchemaVersion, summary/results consistency, input hashes and repo commit metadata must be enforced. |
