...
| Rule and override governance | Rule override files exist, auto-fix candidate rules are present, status taxonomy/docs registry checks are present, Batch 34 adds legacy mapping review evidence, Batch 35 adds variant split review evidence, and Batch 36 adds creative tab grouping evidence. | Status taxonomy must remain mapped, and final registry cleanup must remove ambiguous planned statuses. |
