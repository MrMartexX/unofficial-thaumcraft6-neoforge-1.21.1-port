...
|---|---:|---:|---:|---|
| Source quality and legacy evidence ($layer=source_quality) | 6 | 5 | 0 | access_transformers, auto_fix_candidates, check_invocation, docs_deferred, legacy_primary_manifest, original_jar_probe, public_api, report_freshness, report_schema, secondary_legacy_probe, source_conflict_report |
...
| Report governance | Batch 27 adds a report-only schema contract validator and Batch 29 adds a report-only freshness guard for local JSON reports; input hashes and commit metadata remain planned. | SchemaVersion, summary/results consistency, freshness, input hashes and repo commit metadata must be enforced. |
