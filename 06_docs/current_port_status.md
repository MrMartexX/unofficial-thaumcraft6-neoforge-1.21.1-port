...
Last reviewed checkpoint: `2026-07-12` Flux Rift consequence blocker slice
...
- Current validation: `TCFluxTaintBlockerAudit` passes `13/13` and writes `06_docs/audits/generated/thaumcraft_1_21_flux_taint_blocker_audit.md`; `TCTaintEcologyBlockerAudit` passes `15/15`; `TCTaintMobBlockerAudit` passes `14/14`; `TCFallingTaintBlockerAudit` passes `10/10`; `TCFluxRiftConsequenceAudit` passes `11/11`. Full custom taint mob renderers/animations, exact FallingTaint landing particles/render pixel parity, broad natural spawn placement and Flux Rift focus-cloud event execution are still deferred.
...
8. For the next crucible/Thaumatorium gameplay batch, audit legacy alchemy side-effect and Thaumatorium machine calls before wiring taint mob natural-spawn/rendering parity, Thaumic Slime polish, liquid death, automation behavior or focus-cloud-owned Flux Rift event execution.
