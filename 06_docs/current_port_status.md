...
|---|---|---|
| Ready | Aspect lookup parity harness, item scan parity harness, entity scan parity harness, reload-safe research JSON parsing, research reference validation, read-only scan commands, current Thaumometer scan-key mutation, narrow completed-research-key client sync for highlight filtering, and the first research table/scribing tools storage/menu slice. | Keep these covered by build/server/client smoke checks before expanding consumers. |
| Placeholder / Bridge | Remaining legacy requirement bridges for focus/caster/vis, unfinished theorycraft table actions, crucible/smelter/nitor, infusion matrix, thaumium tools, mirror, curio, zombie brain, OreDictionary chest and flattened vanilla metadata. Generated parity reports and debug-only research completion commands are also bridge tooling. | These are scaffolds for validation and migration. Do not treat them as final gameplay, reward, recipe, or subsystem behavior. |
| Blocked | Thaumonomicon UI, full player knowledge/stage/flag sync, rewards, recipe unlocks, warp, theorycraft cards/actions, exact direct `required_craft` legacy hash mapping, essentia container behavior/filling/draining, custom Thaumcraft entities, and ScanSky celestial-note side effects. | These need their own focused design/validation slices. |
| Next subsystem | Theorycraft parity fixtures and next card family. | The storage/conversion/menu/action boundary is now in place; the next safe step is validating table data/card mutations against legacy and then porting advanced cards by dependency family. |
...
11. Keep the research table diagnostic harness passing after every `TCResearchTableData`, card, table menu, or action payload change.
12. Port remaining theorycraft cards only by audited dependency family; advanced cards that require warp, curios, focus/caster, infusion, celestial notes, portals, beacon/enchantment aids, or other unported systems must stay deferred or explicitly bridged.
