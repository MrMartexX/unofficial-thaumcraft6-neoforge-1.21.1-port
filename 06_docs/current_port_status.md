...
Last reviewed checkpoint: `2026-07-11` FallingTaint crust physics blocker slice
...
| Entities/golems | Foundation started | `TCEntityTypes` registers the first two non-mob item-entity types, `special_item` and `follow_item`, plus focused `flux_rift`, `arcane_bore`, `falling_taint`, `taint_seed`, `taint_seed_prime`, `thaum_slime`, `taint_crawler`, `taintacle`, `taintacle_tiny` and `taint_swarm` foundations, and keeps the full 43-entry legacy entity catalog explicit for future batches. Runtime entity foundation audit passes `23/23`; focused Flux/Bore/Thaumatorium blocker audit passes `11/11`; taint ecology blocker audit passes `15/15`; taint mob blocker audit passes `14/14`; FallingTaint blocker audit passes `10/10`. Remaining mob AI, spawn rules, broad custom renderers, focus projectile/cloud/mine entities, full turrets and golems remain deferred. |
...
8. For the next crucible/Thaumatorium gameplay batch, audit legacy alchemy side-effect and Thaumatorium machine calls before wiring full flux-rift consequences, taint mob natural-spawn/rendering parity, Thaumic Slime polish, liquid death or automation behavior.
