...
- Row 6-11 blocker regression was re-run together on 2026-07-20: item/equipment `17/17`, focus/caster core `10/10`, focus cast execution `5/5`, research data `0` source/runtime/category diffs with `10/10` progression checks, Thaumonomicon protocol `38/38`, page catalog `203 READY` / `0 DEFERRED` / `0 LEGACY_MISSING`, crucible behavior `16/16`, special alchemy `32/32`, alchemy automation `11/11`, crucible recipe data `78/78`, Thaumatorium `19/19`, transport `61/61`, Flux/Bore/Thaumatorium `11/11`, Flux Rift consequence `21/21`, Wisp `11/11`, spawn policy `34/34`, Flux Goo/Taint Fibre `13/13`, Taint ecology `15/15`, Taint mob `15/15`, FallingTaint `10/10`, and entity foundation `71/71`. The sweep fixed a Thaumatorium audit isolation bug caused by persistent fake-player research knowledge, restored legacy `.jpg` research-category background ids, made the crucible recipe-data audit script compatible with Windows PowerShell 5, and closed the rift-owned focus-cloud event-3 blocker with a registered invisible `focus_cloud` entity and runtime audit coverage. Final measured pixel-level visual parity remains row 17 work.
...
Still blocked:
- Thaumonomicon final search visual parity, exact direct craft-reference hash parity, future owner-specific recipe-page polish, WarpEvents client hallucination/stress visuals, exact Guardian/orb/cultist/portal renderer pixel parity, and cancellable research/knowledge events. The real item/open/browser/entry/search/drilldown flow, vanilla/arcane/crucible/infusion/blueprint/fake-display page snapshots, permanent page catalog, server-authoritative warp mutation/sync, first server-side WarpEvents behavior slice and server-authoritative index/entry/action/drilldown protocol are implemented; entry rewards and addendum notifications are implemented, but built-in TC6 data does not provide a real reward integration fixture.
...
- Current protocol audit passes `38/38`; current research page catalog audit reports `203 READY`, `0 DEFERRED`, `0 LEGACY_MISSING`.
