...
| BlockEntities | Started narrowly | `TCResearchTableBlockEntity` stores the two legacy research-table slots: scribing tools and paper. It is storage-only for now; full menu, screen, theory data and card sync are deferred. Do not copy legacy `TileEntity` classes directly. |
...
|---|---|---|
| Ready | Aspect lookup parity harness, item scan parity harness, entity scan parity harness, reload-safe research JSON parsing, research reference validation, read-only scan commands, current Thaumometer scan-key mutation, narrow completed-research-key client sync for highlight filtering, and the first storage-only research table/scribing tools slice. | Keep these covered by build/server/client smoke checks before expanding consumers. |
| Placeholder / Bridge | Remaining legacy requirement bridges for focus/caster/vis, unfinished research/crafting station menus, crucible/smelter/nitor, infusion matrix, thaumium tools, mirror, curio, zombie brain, OreDictionary chest and flattened vanilla metadata. Generated parity reports and debug-only research completion commands are also bridge tooling. | These are scaffolds for validation and migration. Do not treat them as final gameplay, reward, recipe, or subsystem behavior. |
| Blocked | Thaumonomicon UI, full player knowledge/stage/flag sync, rewards, recipe unlocks, warp, theorycraft cards, research table menu/screen, exact direct `required_craft` legacy hash mapping, essentia container behavior/filling/draining, custom Thaumcraft entities, and ScanSky celestial-note side effects. | These need their own focused design/validation slices. |
| Next subsystem | Research table menu/screen and minimal theorycraft data model. | The storage/conversion slice is now in place; the next safe step is the menu boundary before card gameplay. |
