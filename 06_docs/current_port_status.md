...
| Next subsystem | Arcane Workbench parity closure and recipe expansion planning: player vis discount design, Workbench Charger 3x3 aura design, blocked-output ghost/final GUI polish, then the next small exact arcane recipe family. | Do not mass-import arcane recipes or generate arcane-derived aspects until the workbench service, aura source behavior, and ingredient mappings are audited. |
...
15. For Arcane Workbench, design the exact player vis discount and Workbench Charger 3 x 3 aura behavior before importing more recipes. Current cost/aura/crystal-slot GUI feedback is server-owned and covered by the workbench audit; blocked-output ghost/final visual tuning remains a separate GUI polish task.
