...
Last reviewed checkpoint: `2026-07-12` Wisp behavior/render/FX blocker slice
...
- `Wisp` is now registered as `thaumcraft:wisp` with legacy tracking `64`, update interval `3`, no velocity updates, `0.9 x 0.9` size, dynamic aspect `Type` persistence, legacy random primal/compound type initialization and crystal drop identity. Its focused row-14/17 slice now ports the legacy free-flight waypoint AI, hurt aggro cooldown, player retarget cooldown, chase motion, zap attack cadence/damage/sound, ambient motes, billboard render layers and source/target-id zap FX payload. Broad natural spawn placement and measured in-world renderer pixel parity remain separate validation work.
...
- Current validation: `TCFluxTaintBlockerAudit` passes `13/13` and writes `06_docs/audits/generated/thaumcraft_1_21_flux_taint_blocker_audit.md`; `TCTaintEcologyBlockerAudit` passes `15/15`; `TCTaintMobBlockerAudit` passes `14/14`; `TCFallingTaintBlockerAudit` passes `10/10`; `TCFluxRiftConsequenceAudit` passes `11/11`; `TCWispBehaviorAudit` passes `11/11`. Full custom taint mob renderers/animations, exact Wisp/FallingTaint landing particles/render pixel parity, broad natural spawn placement and Flux Rift focus-cloud event execution are still deferred.
...
| Entities/golems | Foundation started | `TCEntityTypes` registers the first two non-mob item-entity types, `special_item` and `follow_item`, plus focused `flux_rift`, `arcane_bore`, `falling_taint`, `taint_seed`, `taint_seed_prime`, `wisp`, `thaum_slime`, `taint_crawler`, `taintacle`, `taintacle_tiny` and `taint_swarm` foundations, and keeps the full 43-entry legacy entity catalog explicit for future batches. Runtime entity foundation audit passes `25/25`; focused Flux/Bore/Thaumatorium blocker audit passes `11/11`; Flux Rift consequence audit passes `11/11`; Wisp behavior/render contract audit passes `11/11`; taint ecology blocker audit passes `15/15`; taint mob blocker audit passes `14/14`; FallingTaint blocker audit passes `10/10`. Remaining broad natural spawn placement, exact measured entity render pixel parity, focus projectile/cloud/mine entities, full turrets and golems remain deferred. |
...
| Blocked | Thaumonomicon final search/visual parity, recipe drilldown/history, final Arcane Workbench GUI polish, optional Curios/accessory slot adapter wiring, final robe/goggles armor model geometry, special void-jar stack-copy behavior, exact Wisp/FallingTaint/entity render pixel parity, final taint mob renderers/particles/natural-spawn parity, Flux Rift focus-cloud event execution, measured infusion/mirror/Thaumatorium visual parity, blueprint/fake recipe systems, full warp effects/sync, custom projectile/construct/golem AI/renderers, remaining focus effect execution/projectiles/clouds/mines, Focus Pouch GUI and ScanSky celestial-note side effects. | These need focused design and validation slices. |
...
| Check | Result | Notes |
|---|---|---|
| `.\gradlew.bat build --no-daemon` | Passed | Latest Wisp behavior/render/FX batch builds successfully. Re-run after each recipe, UI, menu, networking, gameplay or asset expansion batch. |
