...
- Row 6-11 blocker regression was re-run together on 2026-07-20: item/equipment `17/17`, focus/caster core `10/10`, focus cast execution `5/5`, research data `0` source/runtime/category diffs with `10/10` progression checks, Thaumonomicon protocol `36/36`, page catalog `203 READY` / `0 DEFERRED` / `0 LEGACY_MISSING`, crucible behavior `16/16`, special alchemy `32/32`, alchemy automation `11/11`, crucible recipe data `77/77`, Thaumatorium `19/19`, transport `61/61`, Flux/Bore/Thaumatorium `11/11`, Flux Rift consequence `16/16`, Wisp `11/11`, spawn policy `34/34`, Flux Goo/Taint Fibre `13/13`, Taint ecology `15/15`, Taint mob `15/15`, FallingTaint `10/10`, and entity foundation `69/69`. The sweep fixed a Thaumatorium audit isolation bug caused by persistent fake-player research knowledge, restored legacy `.jpg` research-category background ids, and made the crucible recipe-data audit script compatible with Windows PowerShell 5. Final measured pixel-level visual parity remains row 17 work.
...
- Flux Rift / Arcane Bore / Thaumatorium blocker update: Void Siphon now consumes real `thaumcraft:flux_rift` entities through `TCVoidSiphonRiftAccess`; Arcane Bore has its first real placer item, entity, pickaxe menu and server mining loop; Thaumatorium now uses the legacy OBJ/MTL asset instead of the cube fallback and has a BER that renders the selected recipe output at the front. `TCFluxBoreThaumatoriumBlockerAudit` passes `11/11`, including a guard against legacy `textures/blocks` MTL paths. Flux Rift consequence event 3 remains deferred to the focus-cloud owner; full turrets and measured Thaumatorium pixel parity remain deferred.
...
| Entities/golems | Foundation started | `TCEntityTypes` registers the first two non-mob item-entity types, `special_item` and `follow_item`, plus focused `alumentum`, `causality_collapser`, `bottle_taint`, `eldritch_orb`, `golem_orb`, `flux_rift`, `arcane_bore`, `falling_taint`, `taint_seed`, `taint_seed_prime`, `wisp`, `firebat`, `pech`, `brainy_zombie`, `giant_brainy_zombie`, `thaum_slime`, `taint_crawler`, `taintacle`, `taintacle_tiny`, `taint_swarm`, `cultist_portal_lesser`, `cultist_knight`, `cultist_cleric`, `mind_spider` , `eldritch_guardian`, `cultist_portal_greater`, `cultist_leader`, `taintacle_giant`, `eldritch_crab` and `inhabited_zombie` foundations, and keeps the full 43-entry legacy entity catalog explicit for future batches. Runtime entity foundation audit passes `69/69` checks including Pech state/pack/trade/menu/wand/scan/aspect contracts; focused WarpEvents behavior audit passes `35/35`; focused Special Alchemy behavior audit passes `32/32`; focused Flux/Bore/Thaumatorium blocker audit passes `11/11`; Flux Rift consequence audit passes `16/16`; Wisp behavior/render contract audit passes `11/11`; entity spawn-policy audit passes `34/34` for Wisp Nether, Angry Zombie overworld and Firebat Nether/Halloween; taint ecology blocker audit passes `15/15`; taint mob blocker audit passes `15/15`; FallingTaint blocker audit passes `10/10`. Remaining Thaumcraft-biome worldgen spawn expansion, exact measured entity render pixel parity, exact Alumentum/Causality fire-mote pixel parity, exact Guardian/orb/cultist/portal/Pech renderer pixel parity, focus projectile/cloud/mine entities, full turrets and golems remain deferred. |
...
| Check | Result | Notes |
|---|---|---|
| `.\gradlew.bat build --no-daemon` | Passed | Latest Firebat entity/spawn batch builds successfully. Re-run after each recipe, UI, menu, networking, gameplay or asset expansion batch. |
