...
| Blocked | Thaumonomicon final search/visual parity, recipe drilldown/history, final Arcane Workbench GUI polish, full equipment/Curios discount integration, remaining arcane recipes, full Flux Goo flow/taint transforms, measured infusion visual parity, blueprint/fake recipe systems, full warp effects/sync, broad essentia networks, custom entities, caster/mirror behavior, and ScanSky celestial-note side effects. | These need focused design and validation slices. |
| Next subsystem | Run the focused active-altar visual comparison, then continue the next dependency-owned gameplay/rendering slice from the migration matrix. | Server gameplay and FX queue contracts pass `95/95`; renderer startup is clean, but pixel-level parity requires in-world evidence. |
