...
Last reviewed checkpoint: `2026-06-19` crucible collision/absorption behavior slice after explicit HEDGE dynamic costs
...
- Special alchemy side effects, crucible-derived aspect generation, item pulling radius, Thaumatorium/alembic/jar/tube integration, infusion altar behavior, essentia transport, full custom entity/golem systems, broad worldgen, broad rendering polish, full equipment/Curios discount integration, and remaining dependency-heavy recipe/page families remain deferred.
...
| Check | Result | Notes |
|---|---|---|
| `.\gradlew.bat build --no-daemon` | Passed | Latest crucible collision/spill behavior slice builds successfully. Re-run after each recipe, UI, menu, networking, gameplay or asset expansion batch. |
...
| Gate 4+ | Aura is started as an isolated server-side storage/API slice after `aura_design.md`; the first research-table BlockEntity/menu/screen boundary exists; the first custom arcane recipe and Arcane Workbench server crafting boundary exists, including Workbench Charger 3 x 3 aura use and the first player vis-discount path; the first crucible recipe serializer/page snapshot boundary and current manual/collision/spill in-world crucible behavior slices exist. Capabilities, broad machine networks, full alchemy side effects, infusion recipes, broad networking, worldgen, and large rendering systems remain not started. |
