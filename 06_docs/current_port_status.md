...
| Research | Progression/page-catalog core parity-closed; first real Thaumonomicon UI active | Player knowledge storage, reload-safe research data, requirements, scan-key mutation, table/theorycraft slice, GUI-ready knowledge sync, permanent research recipe/page catalog, server-authoritative Thaumonomicon protocol, real Thaumonomicon item/open flow, browser, and entry screen exist. Checked stage completion preserves exact legacy ordering. Browser start preserves `first=true`/`checks=false`/`noFlags=true`, stage advance preserves `first=false`/`checks=true`/`noFlags=true`, and entry acknowledgement clears `RESEARCH`/`PAGE` before attempting the legacy known-entry final-stage checked progression with `noFlags=false`. The research data harness reports `148/148` entries and `10/10` semantic checks. The page catalog reports `253` research occurrences, `203` direct references, `325` total entries including group members, and `0` parity/structural differences. The protocol audit passes `16/16`, including explicit-open-versus-refresh separation. Full warp events/effects/client sync, cancellable research/knowledge events, final visual/search parity, and custom recipe-page renderers remain blocked. |
...
| Menus/screens | Started with research table and first Thaumonomicon screens | `TCResearchTableMenu` opens through `TCMenus.RESEARCH_TABLE`, exposes the two table slots plus the legacy-offset player inventory, and `TCResearchTableScreen` draws the legacy background with minimal functional theory controls. `TCThaumonomiconBrowserScreen` and `TCThaumonomiconEntryScreen` now consume only authoritative server view models and provide category graph navigation, pan/zoom, exact start/acknowledge/stage actions, translated stage text, requirements and non-invented bookmark availability. Search, exact visual parity and real recipe pages remain incomplete. |
...
| Blocked | Thaumonomicon final search/visual parity, custom arcane/crucible/infusion/blueprint/fake recipe-page renderers, full warp events/effects/client sync, cancellable research/knowledge events, exact direct `required_craft` legacy hash mapping, essentia container behavior/filling/draining, custom Thaumcraft entities, and ScanSky celestial-note side effects. | These need their own focused design/validation slices. |
| Next subsystem | Server-authoritative recipe-page view model and first real `READY` crafting-page renderer. | Do not invent contents for `DEFERRED` or `LEGACY_MISSING` catalog pages; this is the boundary into the crafting subsystem. |
...
The next research/crafting boundary is a server-authoritative recipe-page view model and the first real renderer for catalog pages that are actually `READY`. The existing browser/entry screens consume only authoritative server view models and must not bypass the deferred custom recipe-page blockers.
...
- Build the server-authoritative recipe-page view model and first real `READY` crafting-page renderer; do not duplicate recipe, visibility, requirement, or page-catalog decisions on the client.
