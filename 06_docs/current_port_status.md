...
| Placeholder / Bridge | Legacy requirement identity bridges, material-family mapping boundary, aspect crystal/phial variant identities, generated parity reports, and debug-only research completion commands. | These are scaffolds for validation and migration. Do not treat them as final gameplay, reward, recipe, or item semantics. |
| Blocked | Thaumonomicon UI, full player knowledge/stage/flag sync, rewards, recipe unlocks, warp, theorycrafting, exact direct `required_craft` legacy hash mapping, essentia container items, crystal/phial DataComponent semantics, custom Thaumcraft entities, and ScanSky celestial-note side effects. | These need their own focused design/validation slices. |
...
| Research/progression bridge identities | Thaumium/brass materials, arcane/research/wand workbench blocks, crucible/smelter/infusion matrix blocks, focus/caster/vis placeholder items, scribing tools, aspect crystal essence variants, phial variants, and enchanted-placeholder variants. These make legacy research requirements registry-resolvable, but many remain bridge/placeholder semantics until their subsystem slices are implemented. |
