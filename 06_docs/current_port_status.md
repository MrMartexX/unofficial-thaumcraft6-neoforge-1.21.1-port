...
- Bellows now has a focused device/rendering slice with legacy client inflation, tube-buffer extension, smelter/tube-buffer ownership and vanilla-furnace cook-progress boost. The next lamp device slice adds server behavior for Arcane/Growth/Fertility Lamps; final measured lamp item/block pixel parity remains a visual review task.
...
| `runServer` plus focused infusion audit | Passed | Dedicated server reached `Done`; aspect bootstrap/reload validated `694` exact assignments, `46` tag assignments and `32` complex exact assignments, and coverage remained `1230/1230`. The focused infusion exporter completed and stopped cleanly with `95/95` checks. |
