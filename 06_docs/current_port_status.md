...
| BlockEntities | Started narrowly | `TCResearchTableBlockEntity` stores the two legacy research-table slots: scribing tools and paper, implements the table `Container`, and provides the menu opening data. Theory data/card state is still deferred. Do not copy legacy `TileEntity` classes directly. |
| Menus/screens | Started narrowly | `TCResearchTableMenu` opens through `TCMenus.RESEARCH_TABLE`, exposes only the two table slots plus the legacy-offset player inventory, and `TCResearchTableScreen` draws the legacy background. No theorycraft buttons, cards, or client-to-server theory actions exist yet. |
...
| Next subsystem | Minimal server-owned theorycraft data model. | The storage/conversion/menu boundary is now in place; the next safe step is theory data serialization before card gameplay. |
