...
Last reviewed base commit: `274c91a`
...
| Placeholder / Bridge | Remaining legacy requirement bridges for focus/caster/vis, unfinished advanced theorycraft cards/aids, crucible/smelter/nitor, infusion matrix, thaumium tools, mirror, curio, zombie brain, OreDictionary chest and flattened vanilla metadata. The vanilla XP/aid theory cards are still bridge-level gameplay until the full table UI/reward loop is finished. Generated parity reports and debug-only research completion commands are also bridge tooling. | These are scaffolds for validation and migration. Do not treat them as final gameplay, reward, recipe, or subsystem behavior. |
