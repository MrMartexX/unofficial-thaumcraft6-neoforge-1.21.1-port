...
| Entities/golems | Foundation started | `TCEntityTypes` registers the first two non-mob item-entity types, `special_item` and `follow_item`, plus focused `alumentum`, `causality_collapser`, `bottle_taint`, `eldritch_orb`, `golem_orb`, `flux_rift`, `arcane_bore`, `falling_taint`, `taint_seed`, `taint_seed_prime`, `wisp`, `brainy_zombie`, `giant_brainy_zombie`, `thaum_slime`, `taint_crawler`, `taintacle`, `taintacle_tiny`, `taint_swarm`, `cultist_portal_lesser`, `cultist_knight`, `cultist_cleric`, `mind_spider` and `eldritch_guardian` foundations, and keeps the full 43-entry legacy entity catalog explicit for future batches. Runtime entity foundation audit passes `51/51` checks; focused WarpEvents behavior audit passes `35/35`; focused Special Alchemy behavior audit passes `32/32`; focused Flux/Bore/Thaumatorium blocker audit passes `11/11`; Flux Rift consequence audit passes `16/16`; Wisp behavior/render contract audit passes `11/11`; entity spawn-policy audit passes `17/17` for Wisp Nether plus Angry Zombie overworld; taint ecology blocker audit passes `15/15`; taint mob blocker audit passes `14/14`; FallingTaint blocker audit passes `10/10`. Remaining Firebat/Pech/Thaumcraft-biome spawn placement, exact measured entity render pixel parity, exact Alumentum/Causality fire-mote pixel parity, exact Guardian/orb/cultist/portal renderer pixel parity, focus projectile/cloud/mine entities, full turrets and golems remain deferred. |
...
| Check | Result | Notes |
|---|---|---|
| `.\gradlew.bat build --no-daemon` | Passed | Latest Causality Collapser rift-projectile batch builds successfully. Re-run after each recipe, UI, menu, networking, gameplay or asset expansion batch. |
...
| Entity spawn-policy audit exporter | Passed | `.\gradlew.bat runServer --no-daemon -PtcEntitySpawnPolicyAudit=true "-PtcEntitySpawnPolicyAuditPath=..\\..\\06_docs\\audits\\generated\\thaumcraft_1_21_entity_spawn_policy_audit.md"` reached `Done`, wrote the audit, and stopped automatically. Current `17/17` checks validate the active Wisp Nether row and Angry Zombie overworld row, their NeoForge biome modifiers, Brainy Zombie land-biome tag, placement types, dark-cell success paths, config gate and peaceful denial. |
