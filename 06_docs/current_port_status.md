...
25. `06_docs/shobie_1_20_arcane_recipe_import.md` - experimental Shobie arcane data import mapping.
26. `06_docs/shobie_1_20_material_recipe_import.md` - experimental Shobie material/equipment recipe import mapping.
27. `06_docs/shobie_1_20_crucible_recipe_import.md` - experimental Shobie crucible data import mapping.
28. `06_docs/shobie_1_20_infusion_recipe_import.md` - experimental Shobie infusion data import mapping.
29. `06_docs/shobie_1_20_tag_compat_import.md` - experimental Shobie tag taxonomy translated to current ids.
30. `06_docs/gate1_items_plan.md` - historical Gate 1 workflow, not a full current inventory.
...
| Tags | Aspect tag audit expanded plus Shobie compatibility taxonomy | Tags replace old `OreDictionary` patterns. `aspect_assignment_tag_audit.md` maps current legacy aspect-related keys; safe current common tag resources exist for amber/cinnabar/quartz ores, amber gems, vanilla ore/gem/ingot/dust/raw-material bridges, plate bridges, brass/thaumium/void ingots, rods/wooden, current imported nuggets, and copper material bridges; exact `thaumcraft:legacy_ore_dictionary/*` item/block tags now preserve all already registered 1.12 OreDictionary entries, including `plateIron`, `plateBrass`, `plateThaumium`, `plateVoid`, `stickWood`, `ingotGold`, `ingotBrass`, `ingotThaumium`, and `nitor` for current exact arcane recipes. The Shobie tag taxonomy is also present under modern `tags/block` and `tags/item` paths, translated to current 1.12-style ids and limited to registered content. |
| Aspects | Core/API slice parity-clean for comparable stacks/entities, with two documented modern entity policy rows | `Aspect`, `AspectList`, pure `AspectHelper` logic, reload-safe data-driven exact/tag/manual assignments, vanilla material tag bridges, crafting/current arcane/current crucible/current infusion generated-cache slice, legacy stack-sensitive bonus rules, component-aware potion and enchanted-book lookup, spawn-egg exclusion, vanilla entity aspect assignments, bootstrap parity validation, server-data-load tag validation, OreDictionary-to-tag audit, `generateTags` audit, read-only Shift inventory tooltip rendering, and assignment/cache/manual-policy docs are implemented/documented. All assignable current `minecraft:*` item ids have aspects after reload validation; spawn eggs, firework star/rocket, infested blocks, and empty component-only potion carriers remain intentionally excluded for legacy runtime parity. Current registered Thaumcraft option items used by theorycraft now include dump-derived exact values for `alumentum`, `salis_mundus`, `brain`, `nitor_yellow`, `thaumium_ingot`, and `brass_ingot`. `filter`/`morphic_resonator` reload-validate generated arcane recipe aspects, `focus_1`/`cluster_iron` reload-validate generated crucible recipe aspects, and `focus_2`/`focus_3` reload-validate generated infusion recipe aspects using the legacy central-plus-components-plus-sqrt-cost formula. `elder_guardian` and `zombie_villager` now have documented living-mob aspect rows as intentional modern-policy corrections. The runtime dump harness now runs on both original Forge 1.12.2 Thaumcraft and the 1.21.1 port; the comparers separate expected version differences from real port gaps. Current comparable item aspect parity is `1139/1139` with `0` amount/set/order/kind/null gaps; current comparable entity scan report has `83/85` fully parity-ok rows plus `2` expected modern entity aspect policy rows and `0` actionable gaps. Crucible gameplay, Infusion Matrix gameplay, essentia transport, Thaumcraft custom entity aspects, and gameplay-heavy consumers remain blocked until their own design slices. |
...
| Research | Progression/page-catalog core parity-closed; first real Thaumonomicon UI, vanilla crafting page and arcane page active | Player knowledge storage, reload-safe research data, requirements, scan-key mutation, table/theorycraft slice, GUI-ready knowledge sync, permanent research recipe/page catalog, server-authoritative revision-gated Thaumonomicon protocol, real Thaumonomicon item/open flow, browser, entry screen, vanilla crafting-page renderer, and first arcane recipe page renderer exist. Checked stage completion preserves exact legacy ordering. Browser start preserves `first=true`/`checks=false`/`noFlags=true`, stage advance preserves `first=false`/`checks=true`/`noFlags=true`, and entry acknowledgement clears `RESEARCH`/`PAGE` before attempting the legacy known-entry final-stage checked progression with `noFlags=false`. The research data harness reports `148/148` entries and `10/10` semantic checks. The page catalog reports `253` research occurrences, `203` direct references, `325` total entries including group members, and `0` parity/structural differences; latest live availability is `17 READY`, `178 DEFERRED`, `8 LEGACY_MISSING`. The protocol audit passes `25/25`; stale client action revisions are rejected without progression mutation, and all seven live vanilla crafting catalog entries plus all fourteen live arcane catalog entries produce valid server-resolved snapshots. Full warp events/effects/client sync, cancellable research/knowledge events, final visual/search parity, and the remaining custom crucible/infusion/blueprint/fake recipe-page systems/renderers remain blocked. |
| Recipes | Vanilla crafting fixtures, research bridges, Shobie data imports, first custom arcane path, first crucible data layer, and first infusion data layer started | Simple modern `data/thaumcraft/recipe` crafting recipes exist for generated-aspect validation and the first research table slice: `tablewood`, `tablestone`, `scribingtoolscraft2`, `scribingtoolsrefill`, exact legacy `ironplate`, `brassplate`, and `thaumiumplate` recipes, and the current Shobie-derived material/equipment set documented in `shobie_1_20_material_recipe_import.md`. Remaining `data/thaumcraft/recipe/research_bridge` recipes make registered/placeholder research requirement outputs craft-detectable for the modern `required_craft` marker path, but exact arcane and imported focus infusion recipes remove their obsolete bridges. The custom arcane boundary exists: `thaumcraft:arcane` `RecipeType`, `thaumcraft:arcane_shaped`/`thaumcraft:arcane_shapeless` serializers, public `IArcaneRecipe`, public `IArcaneWorkbench` marker, exact hand-audited fixtures, 56 Shobie-derived arcane JSONs translated through the current serializers, and 22 Shobie arcane rows covered by existing current recipes as documented in `shobie_1_20_arcane_recipe_import.md`. Arcane Workbench server crafting now has research/vis/crystal checks, no-charger current-chunk aura drain, Workbench Charger 3x3 aura query/drain, first player vis-discount cost path, atomic output-take consumption, crystal consumption, vanilla 3x3 fallback, missing-vis ghost output, and server-owned menu feedback for base/discounted cost, available aura and required crystal slots. The first crucible boundary exists as `thaumcraft:crucible` `RecipeType`/serializer plus 27 Shobie-derived, legacy-corrected JSON recipes documented in `shobie_1_20_crucible_recipe_import.md`. The first infusion boundary exists as `thaumcraft:infusion` `RecipeType`/serializer plus legacy-corrected `focus_2` and `focus_3` JSON recipes documented in `shobie_1_20_infusion_recipe_import.md`. Current `TCArcaneRecipe`, `TCCrucibleRecipe`, and `TCInfusionRecipe` outputs feed the generated aspect cache with the exact legacy ingredient/vis, catalyst/sqrt-cost, and central/components/sqrt-cost formulas. Full equipment/Curios discount integration, real void/thaumium equipment behavior, crucible BlockEntity gameplay, and Infusion Matrix gameplay are not implemented. |
| BlockEntities | Started narrowly | `TCResearchTableBlockEntity` stores the two legacy research-table slots: scribing tools and paper, implements the table `Container`, provides the menu opening data, saves/loads theory state under `note`, can finish theories into raw THEORY knowledge by category, and now sends vanilla update packets/tags so the table-top renderer can see slot changes. `TCArcaneWorkbenchBlockEntity` now stores the legacy 5x3 workbench inventory, exposes six fixed primal crystal slots, opens the menu server-side, saves/loads contents, drops contents on removal, and delegates no-charger current-chunk plus Charger 3x3 vis query/drain to the existing aura core. Do not copy legacy `TileEntity` classes directly. |
...
|---|---|---|
| Ready | Aspect lookup parity harness, item scan parity harness, entity scan parity harness, reload-safe research JSON parsing, research reference validation, read-only scan commands, current Thaumometer scan-key mutation, GUI-ready knowledge sync payload/cache, first research table slice, first server-authoritative revision-gated Thaumonomicon item/open/browser/entry/vanilla-crafting-page flow, exact arcane recipe/page snapshots for `thaumcraft:thaumometer`, `thaumcraft:vis_resonator`, `thaumcraft:workbenchcharger`, `thaumcraft:goggles`, `thaumcraft:mechanism_simple`, `thaumcraft:mechanism_complex`, `thaumcraft:wand_workbench`/Focal Manipulator, `thaumcraft:caster_basic`, `thaumcraft:enchantedfabric`, `thaumcraft:mirrorglass`, `thaumcraft:filter`, `thaumcraft:morphicresonator`, `thaumcraft:essentiasmelter`, and `thaumcraft:infusionmatrix`, the first server-authoritative Arcane Workbench crafting/menu-feedback path for those exact recipes, current exact-arcane/crucible/infusion recipe-derived aspect generation, missing-vis ghost output, Workbench Charger 3 x 3 aura behavior, first player vis-discount service, and the Arcane Workbench server behavior audit harness. | Keep these covered by build/server/client smoke checks before expanding consumers. |
| Placeholder / Bridge | Remaining legacy requirement bridges for unresolved vis and other requirement outputs, unfinished advanced theorycraft cards/aids, minimal warp storage/debug commands before real warp events, crucible/smelter/nitor behavior, infusion matrix behavior, hidden Golem Press block identity before full golem-builder multiblock logic, Brain-in-a-Jar block/entity behavior, Crimson portal entity behavior, Alumentum/Salis Mundus behavior, caster, mirror, curio, zombie brain behavior, OreDictionary chest and flattened vanilla metadata. The crucible and infusion serializer/data layers are active, but actual crucible BlockEntity gameplay and Infusion Matrix gameplay are not implemented. The current XP/phial/vanilla-aid/Golemancy/table-inventory/Artifice/basic-Auromancy/basic-Infusion/basic-Golemancy/safe-Eldritch theory cards are still bridge-level gameplay until the full table UI/reward loop is finished. Research bridge recipes, generated parity reports, and debug-only research completion commands remain bridge/deferred tooling. | These are scaffolds for validation and migration. Do not treat them as final gameplay, reward, recipe, or subsystem behavior. |
| Blocked | Thaumonomicon final search/visual parity, recipe drilldown/history, custom crucible/infusion/blueprint/fake recipe page renderers, final Arcane Workbench GUI polish, full equipment/Curios vis-discount integration, remaining arcane/crucible/infusion recipe families, real crucible and Infusion Matrix behavior, full warp events/effects/client sync, cancellable research/knowledge events, exact direct `required_craft` legacy hash mapping, essentia container behavior/filling/draining, custom Thaumcraft entities, caster/mirror behavior, and ScanSky celestial-note side effects. | These need their own focused design/validation slices. |
| Next subsystem | Continue merging Shobie reference data by safe dependency family, then move to the next behavior slice only after build/server/client smoke stays clean. | Do not import Shobie Java wholesale. Keep current 1.21 registry ids and legacy 1.12 public names authoritative. Crucible/infusion behavior and equipment behavior still need focused design/validation before code. |
...
| Check | Result | Notes |
|---|---|---|
| `.\gradlew.bat build --no-daemon` | Passed | Latest Shobie arcane/material/crucible/infusion data slice builds successfully. Re-run after each recipe, UI, menu, networking or asset expansion batch. |
...
| `.\gradlew.bat runServer --no-daemon` | Startup/reload smoke reached `Done` through the Shobie arcane edge import server run | Dedicated server reached `Done`; bundled bootstrap, data-resource assignment reload, generated crafting/arcane/crucible/infusion cache rebuild, tag reload validation, research data reload, aura event/command registration, automatic aura chunk initialization, and the client-only tooltip registration boundary all passed. Latest smoke loaded `688` exact assignments, `46` tag assignments, `32` complex exact assignments, `1482` recipes, rebuilt `584` generated cache entries, loaded `7` research categories / `148` entries / `271` stages / `16` addenda, and reported `1230 of 1230` assignable minecraft item ids with non-empty aspects. Latest full audit checkpoint also passed the page catalog audit with `17 READY` / `178 DEFERRED` / `8 LEGACY_MISSING`, the Thaumonomicon protocol audit `25/25`, the prior arcane recipe audit `105/105` over the hand-audited exact fixture set, and the Arcane Workbench behavior audit `23/23`. |
...
| Generated crafting assignments | `584` | Built after server data/tag reload from current `RecipeType.CRAFTING`, current `thaumcraft:arcane`, current `thaumcraft:crucible`, and current `thaumcraft:infusion` recipes for `minecraft:*` and `thaumcraft:*` outputs that have known ingredient/catalyst/central-component aspects. Exact/tag/manual/runtime-parity assignments still win over generated values. The current count includes remaining safe research bridge recipes used to make modern `required_craft` markers observable, exact normal plate recipes, translated Shobie arcane data including `tube`, `resonator`, and `sanitychecker`, Shobie-derived material/equipment recipes, the first 27 Shobie-derived legacy-corrected crucible recipes, and the 2 legacy-corrected focus infusion recipes; bridge recipes and placeholder equipment/custom-crafting recipes are not final gameplay implementations. |

## Generated aspect recipe cache

`TCAspectStackKey`, `TCGeneratedAspectCache`, and `TCGeneratedAspectRecipeGenerator` define the current generated-aspect cache boundary. The key uses item registry id plus the stack data component patch and ignores stack count, matching the legacy normalization intent. The cache is cleared on aspect assignment bootstrap/reload, then rebuilt after server data/tag reload from loaded vanilla crafting, current arcane, current crucible, and current infusion recipes.

The normal crafting slice does not depend on whether a recipe is crafted from the 2x2 inventory grid or the 3x3 crafting table; it scans `RecipeType.CRAFTING`, and each recipe's own dimensions decide where it can be crafted. The current arcane slice scans the custom `thaumcraft:arcane` recipe type and adds the legacy `praecantatio` bonus from `vis` after the ingredient formula. The current crucible slice scans the custom `thaumcraft:crucible` recipe type and adds the legacy catalyst aspects plus `floor(sqrt(requiredAspectAmount) / outputCount)` for each required aspect. The current infusion slice scans the custom `thaumcraft:infusion` recipe type, uses central input plus components through the legacy ingredient formula, and adds `floor(sqrt(requiredAspectAmount) / outputCount)` for each required aspect.
...
- current crucible recipe-derived outputs match the legacy catalyst-plus-sqrt-cost formula for `focus_1` and `cluster_iron`.
- current infusion recipe-derived outputs match the legacy central-plus-components-plus-sqrt-cost formula for `focus_2` and `focus_3`.
...
| Gate 4+ | Aura is started as an isolated server-side storage/API slice after `aura_design.md`; the first research-table BlockEntity/menu/screen boundary exists; the first custom arcane recipe and Arcane Workbench server crafting boundary exists, including Workbench Charger 3 x 3 aura use and the first player vis-discount path. The first crucible and infusion recipe serializer/data/aspect-cache boundaries exist, but crucible BlockEntity gameplay and Infusion Matrix gameplay are not started. Capabilities, broad machine networks, broad networking, worldgen, and large rendering systems remain not started. |
...
damage types, tag compatibility, all vanilla recipe rows, 56 arcane rows, 27
...
1 arcane edge row, 30 crucible behavior/identity rows, 60 infusion
...
7. For new arcane, crucible, or infusion recipe families, add validation coverage alongside the recipe import so the generated-aspect paths cannot silently accept wrong ingredient/catalyst/central-component mapping; keep real crucible and Infusion Matrix gameplay blocked until their machine behavior designs exist.
