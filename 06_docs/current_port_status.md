...
Last reviewed checkpoint: `2026-07-01` focus/caster/Focal Manipulator core closure
...
- `caster_basic` consumes aura from the legacy area-0 current chunk path, applies raw vis-discount gear to caster costs with the legacy 10% minimum consumption clamp, stores its selected focus stack component, sends aura HUD data while selected, and applies focus activation cooldown.
...
- Current validation: `TCFocusCasterCoreAudit` passes `10/10` and writes `07_Test_Instance_and_Comparisons/focus_caster/thaumcraft_1_21_focus_caster_core_audit.md`. Full cast effect execution, projectile/cloud/mine behavior, Focus Pouch inventory GUI and final Focal Manipulator editor visual parity remain deferred.
...
| Blocked | Thaumonomicon final search/visual parity, recipe drilldown/history, final Arcane Workbench GUI polish, optional Curios/accessory slot adapter wiring, final robe/goggles armor model geometry, special void-jar stack-copy behavior, full Flux Goo flow/taint transforms, measured infusion visual parity, blueprint/fake recipe systems, full warp effects/sync, custom entities, full focus effect execution/projectiles/clouds/mines, Focus Pouch GUI, mirror behavior, remaining essentia utility devices and ScanSky celestial-note side effects. | These need focused design and validation slices. |
| Next subsystem | Continue row 7 from `06_docs/migration/remaining_subsystem_unblock_plan.md`: focus cast-effect execution over the audited core. | Start with one server-owned medium/effect path and add projectile/cloud/mine behavior only after payload, cooldown, aura and failure behavior audits exist. |
...
| Check | Result | Notes |
|---|---|---|
| `.\gradlew.bat build --no-daemon` | Passed | Latest focus/caster/Focal Manipulator core batch builds successfully. Re-run after each recipe, UI, menu, networking, gameplay or asset expansion batch. |
