...
Last reviewed checkpoint: `2026-07-11` Flux Goo and Taint Fibre blocker slice
...
- Current validation: `TCFluxTaintBlockerAudit` passes `13/13` and writes `06_docs/audits/generated/thaumcraft_1_21_flux_taint_blocker_audit.md`. Full `TaintHelper.spreadFibres`, TaintSeed registry/entity behavior, taint terrain transforms, taint mobs and Thaumic Slime spawning are still deferred.
...
- Flux Rift / Arcane Bore / Thaumatorium blocker update: Void Siphon now consumes real `thaumcraft:flux_rift` entities through `TCVoidSiphonRiftAccess`; Arcane Bore has its first real placer item, entity, pickaxe menu and server mining loop; Thaumatorium now uses the legacy OBJ/MTL asset instead of the cube fallback and has a BER that renders the selected recipe output at the front. `TCFluxBoreThaumatoriumBlockerAudit` passes `11/11`, including a guard against legacy `textures/blocks` MTL paths. Full Flux Rift event consequences, finite Flux Goo/taint ecology, full turrets and measured Thaumatorium pixel parity remain deferred.
- Final measured valve/vent pixel parity remains an in-client visual review task.

### Deferred boundaries

- Special alchemy side effects, crucible-derived aspect generation, item pulling radius, exact Thaumatorium model/renderer/pixel parity, special void-jar stack-copy crafting, finite Flux Goo spreading/taint transforms, Flux Rift stabilization, custom mob/projectile/construct/golem AI and renderers, broad worldgen, broad rendering polish, full equipment/Curios discount integration, remaining dependency-heavy recipe/page families, and final measured valve/vent/mirror/Thaumatorium visual parity remain deferred.
...
| Aura | Started with server-side core | `AuraHelper`, per-level `SavedData`, chunk `base/vis/flux`, automatic chunk initialization, legacy formula for aura base generation, main-thread 20-tick legacy-like update loop, permission-level-2 debug commands, and the first Flux Rift entity consumer/source bridge are implemented. The biome category mapper is legacy-like because 1.21 has no `BiomeDictionary`. HUD sync, broad aura FX, finite Flux Goo/taint ecology, research-aware preservation, and most gameplay consumers remain intentionally narrow or deferred. |
...
| Entities/golems | Foundation started | `TCEntityTypes` registers the first two non-mob item-entity types, `special_item` and `follow_item`, plus focused `flux_rift` and `arcane_bore` foundations, and keeps the full 43-entry legacy entity catalog explicit for future batches. Runtime entity foundation audit passes `7/7`; focused Flux/Bore/Thaumatorium blocker audit passes `11/11`. Mob AI, spawn rules, broad custom renderers, focus projectile/cloud/mine entities, full turrets and golems remain deferred. |
