...
- The `thaumcraft:infusion` boundary has `42/42` valid JSON recipes, page snapshots, active matrix/pedestal BlockEntities and a real `jar_normal` source container. The server now persists remaining aspects/components/countdown, advances the default matrix every five ticks, drains one aspect point nearest-first, waits 200 ticks after a failed source pass, spends six cycles per component, preserves component remainders, carries damaged-catalyst ratio to an undamaged damageable result, completes on the following cycle, and emits completion/failure sounds. Runtime audit passes `63/63`.
...
- Special alchemy side effects, crucible-derived aspect generation, item pulling radius, broad Thaumatorium/alembic/mirror integration, infusion stability/instability and structure modifiers, exact infusion stream/debris rendering, full custom entity/golem systems, broad worldgen, broad rendering polish, full equipment/Curios discount integration, and remaining dependency-heavy recipe/page families remain deferred.
...
| Blocked | Thaumonomicon final search/visual parity, recipe drilldown/history, final Arcane Workbench GUI polish, full equipment/Curios discount integration, remaining arcane recipes, player-facing infusion activation, infusion stability/instability and structure modifiers, exact stream/debris renderers, blueprint/fake recipe systems, full warp effects/sync, broad essentia networks, custom entities, caster/mirror behavior, and ScanSky celestial-note side effects. | These need focused design and validation slices. |
| Next subsystem | Continue infusion with exact `getSurroundings()` timing/cost/stability modifiers and instability progression, then audit safe player activation. | Do not enable caster-start gameplay until those server-side rules are green; exact render parity can proceed independently behind the existing FX payload contract. |
...
| Infusion behavior audit exporter | Passed | `tools/audits/audit-infusion-behavior.ps1` passes `63/63`. In addition to recipe/start/readiness checks it verifies real jar discovery/drain, source ordering/range/blocking, persistent cycle NBT, exact five-tick default cadence, one-point drains, six-cycle component timing, 63-cycle Cloud Ring completion, damaged-result ratio, 200-tick failed-source rescan delay, component remainders and catalyst-change abort safety. |
