...
Last reviewed checkpoint: `2026-07-20` row 6-11 blocker regression sweep
...
- Row 6-11 blocker regression was re-run together on 2026-07-20: item/equipment `17/17`, focus/caster core `10/10`, focus cast execution `5/5`, research data `0` source/runtime/category diffs with `10/10` progression checks, Thaumonomicon protocol `41/41`, page catalog `203 READY` / `0 DEFERRED` / `0 LEGACY_MISSING`, crucible behavior `16/16`, special alchemy `32/32`, alchemy automation `11/11`, crucible recipe data `78/78`, Thaumatorium `19/19`, transport `61/61`, Flux/Bore/Thaumatorium `11/11`, Flux Rift consequence `21/21`, Wisp `11/11`, spawn policy `34/34`, Flux Goo/Taint Fibre `13/13`, Taint ecology `15/15`, Taint mob `15/15`, FallingTaint `10/10`, and entity foundation `71/71`. The sweep fixed a Thaumatorium audit isolation bug caused by persistent fake-player research knowledge, kept legacy `.jpg` research-category backgrounds as reference assets while using runtime `.png` paths in modern client views, made the crucible recipe-data audit script compatible with Windows PowerShell 5, closed the rift-owned focus-cloud event-3 blocker with a registered invisible `focus_cloud` entity and runtime audit coverage, and added the final current-scope Thaumonomicon entry UI pass: legacy-style recipe tabs, aspect/knowledge side inserts, selected-stage warp warning, legacy research text markup image/page/line handling and side-panel sync audit coverage. Final measured pixel-level visual parity remains row 17 work.
...
| Research | Progression/page-catalog core parity-closed; first real Thaumonomicon UI, vanilla, arcane, crucible, infusion, blueprint and fake display page snapshots active | Player knowledge storage, reload-safe research data, requirements, scan-key mutation, table/theorycraft slice, GUI-ready knowledge sync, permanent research recipe/page catalog, server-authoritative warp mutation/sync, first server-side WarpEvents behavior slice, server-authoritative revision-gated Thaumonomicon protocol, real Thaumonomicon item/open flow, browser, entry screen, server-visible browser search/filter, server-owned recipe drilldown/history, vanilla crafting-page renderer, first arcane recipe page renderer, first crucible recipe page renderer, infusion recipe page renderer, blueprint construct page renderer, and legacy fake/display page renderer exist. Checked stage completion preserves exact legacy ordering. Browser start preserves `first=true`/`checks=false`/`noFlags=true`, stage advance preserves `first=false`/`checks=true`/`noFlags=true`, and entry acknowledgement clears `RESEARCH`/`PAGE` before attempting the legacy known-entry final-stage checked progression with `noFlags=false`. The research data harness reports `148/148` entries and `10/10` semantic checks. The page catalog reports `253` research occurrences, `203` direct references, `325` total entries including group members, and `0` parity/structural differences; latest live availability is `203 READY`, `0 DEFERRED`, `0 LEGACY_MISSING`. The protocol audit passes `41/41`; stale client action and drilldown revisions are rejected without progression mutation, runtime category backgrounds use PNG texture paths, and live vanilla crafting, arcane, crucible, infusion, blueprint and fake display catalog entries produce valid server-resolved snapshots. Entry UI now has legacy-style recipe tabs, aspect/knowledge side inserts, selected-stage warp warning and legacy research text markup handling for page breaks, divider lines and inline images. WarpEvents client hallucination/stress visuals, cancellable research/knowledge events and measured browser/entry pixel parity remain blocked. |
...
| Check | Result | Notes |
|---|---|---|
| `.\gradlew.bat build --no-daemon` | Passed | Latest row-11 FocusCloud/Flux Rift consequence closure builds successfully. Re-run after each recipe, UI, menu, networking, gameplay or asset expansion batch. |
...
- Current protocol audit passes `41/41`; current research page catalog audit reports `203 READY`, `0 DEFERRED`, `0 LEGACY_MISSING`.
