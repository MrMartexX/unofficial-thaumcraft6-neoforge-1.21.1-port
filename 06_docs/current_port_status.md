...
| `.\gradlew.bat runServer --no-daemon` | Startup/reload smoke reached `Done` through the research and arcane audit server runs | Dedicated server reached `Done`; bundled bootstrap, data-resource assignment reload, generated crafting cache rebuild, tag reload validation, research data reload, aura event/command registration, automatic aura chunk initialization, and the client-only tooltip registration boundary all passed. Latest audit run loaded `686` exact assignments, `46` tag assignments, `32` complex exact assignments, rebuilt `494` generated crafting cache entries from `1349` loaded recipes, loaded `7` research categories / `148` entries / `271` stages / `16` addenda, reported `1230 of 1230` assignable minecraft item ids with non-empty aspects, passed the page catalog audit with `15 READY` / `180 DEFERRED` / `8 LEGACY_MISSING`, passed the Thaumonomicon protocol audit `25/25` including revision/stale-action checks, passed the arcane recipe audit `91/91` with twelve exact loaded arcane recipes, and passed the Arcane Workbench behavior audit `23/23`. |
...
| Generated crafting assignments | `494` | Built from `RecipeType.CRAFTING` recipes after server data/tag reload for current `minecraft:*` and `thaumcraft:*` outputs that have known ingredient aspects. Exact/tag/manual/runtime-parity assignments still win over generated values. The current count includes remaining safe research bridge recipes used to make modern `required_craft` markers observable plus exact normal plate recipes; those recipes are not final gameplay implementations. Exact arcane recipes are intentionally not counted by this vanilla crafting cache. |
