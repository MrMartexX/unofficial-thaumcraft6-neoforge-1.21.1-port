...
| `/tc research requirements 50` | Not re-run from Codex terminal | Gradle/stdin did not pass the command into the running server in this environment. The existing blocker audit remains the working reference until this is re-run manually in-game/server console or replaced by a non-interactive validation task. |
