...
Last reviewed checkpoint: `2026-07-21` Thaumonomicon UI parity repair
...
- Latest Thaumonomicon protocol/UI correction on 2026-07-21 supersedes the earlier `43/43` checkpoint with `48/48` checks passing. The added coverage verifies creative-book-without-pickup behavior (`FIRSTSTEPS` visible but locked at stage `0`), legacy start visibility for `Discovering Artifice` and `Discovering Golemancy`, legacy-recursive debug `research all` over all `7/7` categories and `148/148` entries, `RESEARCH`/`PAGE` flag surfacing, blank empty knowledge totals and inventory-style recipe stack tooltips.
...
- Row 6-11 blocker regression was re-run together on 2026-07-20, then the Thaumonomicon UI parity repair was validated on 2026-07-21: item/equipment `17/17`, focus/caster core `10/10`, focus cast execution `5/5`, research data `0` source/runtime/category diffs with `10/10` progression checks, Thaumonomicon protocol `48/48`, page catalog `203 READY` / `0 DEFERRED` / `0 LEGACY_MISSING`, crucible behavior `16/16`, special alchemy `32/32`, alchemy automation `11/11`, crucible recipe data `78/78`, Thaumatorium `19/19`, transport `61/61`, Flux/Bore/Thaumatorium `11/11`, Flux Rift consequence `21/21`, Wisp `11/11`, spawn policy `34/34`, Flux Goo/Taint Fibre `13/13`, Taint ecology `15/15`, Taint mob `15/15`, FallingTaint `10/10`, and entity foundation `71/71`. The sweep fixed a Thaumatorium audit isolation bug caused by persistent fake-player research knowledge, kept legacy `.jpg` research-category backgrounds as reference assets while using runtime `.png` paths in modern client views, made the crucible recipe-data audit script compatible with Windows PowerShell 5, closed the rift-owned focus-cloud event-3 blocker with a registered invisible `focus_cloud` entity and runtime audit coverage, and added current-scope Thaumonomicon UI repairs: bare legacy texture paths normalize into the `thaumcraft` namespace, browser map nodes/links clip inside the legacy frame, first TC category tabs use the lower legacy offset, locked-hover tooltips show missing parent research, known-entry siblings complete on book open, recipe-output search results are server-owned, recipe bookmark tabs switch while a recipe page is open, the `KNOWLEDGETYPES` in-page totals boundary is active, page arrows use the legacy bob scale, and the later correction restores creative locked-start behavior, legacy start visibility, debug-all semantics, new flag surfacing, blank knowledge totals and normal stack tooltips. Final measured pixel-level visual parity remains row 17 work.
...
- `CardTruth` is now active as a registered Eldritch option card and is covered by the research table diagnostic audit. `AidBasicEldritch` is still deferred because the legacy `BlocksTC.eldritch` owner is ambiguous in the available source/jar evidence; Crimson portal aid still waits for the theorycraft/page integration around the now-registered Crimson portal entity layer; Dragon Egg theorycraft remains unregistered because original TC6 source declares the class but does not register it in `ConfigResearch`.
...
- `Wisp` is now registered as `thaumcraft:wisp` with legacy tracking `64`, update interval `3`, no velocity updates, `0.9 x 0.9` size, dynamic aspect `Type` persistence, legacy random primal/compound type initialization and crystal drop identity. Its focused row-14/17 slice ports the legacy free-flight waypoint AI, hurt aggro cooldown, player retarget cooldown, chase motion, zap attack cadence/damage/sound, ambient motes, billboard render layers, source/target-id zap FX payload and the exact legacy Nether natural-spawn row (`#minecraft:is_nether`, weight `5`, `1-1`) behind the `allowSpawnWisp` config gate. Firebat is registered as `thaumcraft:firebat` with legacy `0.5 x 0.9` size, tracking `64`, update interval `3`, no velocity updates, hanging/free-flight AI, wet/fire/explosion/dropping rules, aspects, scannable identity and the legacy Nether plus Oct 31 overworld spawn rows. Pech is registered as `thaumcraft:pech` with legacy `0.6 x 1.8` size, tracking `64`, update interval `3`, velocity updates, subtype/tamed/anger state, 9-slot loot pack, valued-item taming, trade menu/catalog, `pech_wand`, subtype aspect assignments and scan identity. Angry/Furious Zombies are also registered with legacy tracking, attributes, brain/flesh loot formulas, scan/aspect identities, angry-zombie spawn config and the legacy Angry Zombie overworld spawn row mapped to `#thaumcraft:legacy_angry_zombie_spawn_biomes`. Thaumcraft-biome spawn rows and measured in-world renderer pixel parity remain separate validation work.
...
- `CausalityCollapser` is now registered as `thaumcraft:causality_collapser` with legacy tracking `64`, update interval `20`, velocity updates and a `0.25 x 0.25` projectile size. Its item preserves the legacy stack size `16`, egg-throw sound constants and `shootFromRotation` offset/speed/inaccuracy; the projectile owns the legacy invisible body, forced `0.8F` velocity, `2.0F` fiery explosion and exact `3.0D` AABB Flux Rift collapse trigger.
...
- Current validation: `TCEntityFoundationAudit` passes `71/71` checks and writes `06_docs/audits/generated/thaumcraft_1_21_entity_foundation_audit.md`; `TCWarpEventBehaviorAudit` passes `35/35`; `TCSpecialAlchemyBehaviorAudit` passes `32/32` and writes `06_docs/audits/generated/thaumcraft_1_21_special_alchemy_behavior_audit.md`. The focused Flux/Bore/Thaumatorium blocker audit passes `11/11` and writes `06_docs/audits/generated/thaumcraft_1_21_flux_bore_thaumatorium_blocker_audit.md`; the focused Flux Rift consequence audit passes `21/21` and writes `06_docs/audits/generated/thaumcraft_1_21_flux_rift_consequence_audit.md`; the focused Wisp behavior/render contract audit passes `11/11` and writes `06_docs/audits/generated/thaumcraft_1_21_wisp_behavior_audit.md`; the focused entity spawn-policy audit passes `34/34` for Wisp Nether, Angry Zombie overworld, Firebat Nether/Halloween and Pech magical-biome rows and writes `06_docs/audits/generated/thaumcraft_1_21_entity_spawn_policy_audit.md`; the focused taint ecology blocker audit passes `15/15`; the focused taint mob blocker audit passes `15/15` and writes `06_docs/audits/generated/thaumcraft_1_21_taint_mob_blocker_audit.md`; the focused FallingTaint blocker audit passes `10/10` and writes `06_docs/audits/generated/thaumcraft_1_21_falling_taint_blocker_audit.md`. This does not claim final custom mob renderer/model/particle pixel parity, exact Alumentum/Causality fire-mote pixel parity, exact BottleTaint break FX pixel parity, exact FallingTaint landing particles/render pixel parity, Thaumcraft-biome worldgen spawn expansion, exact Guardian/orb/cultist/portal/Pech renderer pixel parity, broad player-authored focus projectile/cloud/mine gameplay, full turrets or golem logistics parity.
...
- Taint terrain blocks are real blocks/BlockItems: `taint_crust`, `taint_soil`, `taint_rock`, `taint_geyser`, `taint_log` and `taint_feature`. Their active blockstates/models/item models use modern paths. TaintSeed/TaintSeedPrime now use an active legacy-texture ModelPart renderer foundation; exact animated model/pixel parity remains separate row-17 visual/entity-render work.
...
- Current validation: `TCFluxTaintBlockerAudit` passes `13/13` and writes `06_docs/audits/generated/thaumcraft_1_21_flux_taint_blocker_audit.md`; `TCTaintEcologyBlockerAudit` passes `15/15`; `TCTaintMobBlockerAudit` passes `15/15`; `TCFallingTaintBlockerAudit` passes `10/10`; `TCFluxRiftConsequenceAudit` passes `21/21`; `TCWispBehaviorAudit` passes `11/11`; `TCEntitySpawnPolicyAudit` passes `34/34` for the Wisp Nether, Angry Zombie overworld, Firebat Nether/Halloween and Pech magical-biome rows; `TCEntityFoundationAudit` passes `71/71` including Pech and FocusCloud. Exact custom taint mob renderer/model animation pixel parity, exact Wisp/Firebat/Pech/FallingTaint landing particles/render pixel parity, Thaumcraft-biome worldgen spawn expansion and measured rift/focus-cloud particle pixel parity are still deferred.
...
| Menus/screens | Started with research table, Arcane Workbench, smelter, and first Thaumonomicon screens/pages | `TCResearchTableMenu`, `TCArcaneWorkbenchMenu` and `TCSmelterMenu` are server-owned modern menus. The smelter screen uses the original background, two legacy slot positions and five synced progress values. Thaumonomicon browser/entry screens consume authoritative server view models; browser search filters only the server-visible index plus server-owned recipe-output aliases; recipe search hits and recipe stack click-through open server-owned snapshots with local history only; vanilla, arcane, crucible, infusion, blueprint construct and fake/display pages all render from server snapshots; entry-side aspect/knowledge inserts, `KNOWLEDGETYPES` in-page totals and selected-stage warp warning render from synced server state. Browser rendering now clips graph contents under the frame and normalizes legacy texture paths, but exact measured pixel parity, final Arcane Workbench GUI polish, and future owner-specific screen polish remain incomplete. |
| Networking | Started narrowly | Modern custom payloads exist for aura sync, GUI-ready research knowledge sync, research-table action/state/result sync, the Thaumonomicon index/entry/action/drilldown flow and Wisp zap FX. Thaumonomicon protocol version `9` carries separate vanilla crafting, arcane, crucible, infusion, blueprint construct and fake/display recipe page snapshots, server-owned recipe-search aliases plus a server-built index revision; item use sends an explicit server-owned open intent and runs the legacy known-sibling completion pass first; ordinary index refreshes cannot reopen the screen; stale client entry/action/drilldown revisions force authoritative refresh without mutation. Wisp zap uses the legacy source/target-id packet contract through a clientbound custom payload. Do not treat this as a complete networking subsystem yet; every new payload still needs focused design and server validation. |
| Entities/golems | Foundation started | `TCEntityTypes` registers the first two non-mob item-entity types, `special_item` and `follow_item`, plus focused `alumentum`, `causality_collapser`, `bottle_taint`, `eldritch_orb`, `golem_orb`, `flux_rift`, `focus_cloud`, `arcane_bore`, `falling_taint`, `taint_seed`, `taint_seed_prime`, `wisp`, `firebat`, `pech`, `brainy_zombie`, `giant_brainy_zombie`, `thaum_slime`, `taint_crawler`, `taintacle`, `taintacle_tiny`, `taint_swarm`, `cultist_portal_lesser`, `cultist_knight`, `cultist_cleric`, `mind_spider` , `eldritch_guardian`, `cultist_portal_greater`, `cultist_leader`, `taintacle_giant`, `eldritch_crab` and `inhabited_zombie` foundations, and keeps the full 43-entry legacy entity catalog explicit for future batches. Runtime entity foundation audit passes `71/71` checks including Pech state/pack/trade/menu/wand/scan/aspect contracts and the rift-owned invisible FocusCloud foundation; focused WarpEvents behavior audit passes `35/35`; focused Special Alchemy behavior audit passes `32/32`; focused Flux/Bore/Thaumatorium blocker audit passes `11/11`; Flux Rift consequence audit passes `21/21`; Wisp behavior/render contract audit passes `11/11`; entity spawn-policy audit passes `34/34` for Wisp Nether, Angry Zombie overworld and Firebat Nether/Halloween; taint ecology blocker audit passes `15/15`; taint mob blocker audit passes `15/15`; FallingTaint blocker audit passes `10/10`. Remaining Thaumcraft-biome worldgen spawn expansion, exact measured entity render pixel parity, exact Alumentum/Causality fire-mote pixel parity, exact Guardian/orb/cultist/portal/Pech renderer pixel parity, broad player-authored focus projectile/cloud/mine gameplay, full turrets and golems remain deferred. |
...
|---|---|---|
| Ready | Aspect/scan parity harnesses, reload-safe research data, current Thaumometer scan path, knowledge sync, research table and first server-authoritative Thaumonomicon flow including vanilla/arcane/crucible/infusion/blueprint/fake-display page snapshots, server-owned recipe-output search aliases, open-time known-sibling completion, server-authoritative warp mutation/sync plus the legacy `jar_brain` item-warp crafting bridge, the `35/35` server-side `WarpEvents` behavior slice with Guardian/Spider/lesser-portal spawn foundations, Guardian orb projectile path, GolemOrb branch and lesser portal minion spawning, complete regular `89/89` Arcane Workbench recipe id set/behavior, crucible behavior slices, infusion recipe/page data, persistent Warded Jar/Void Jar storage, the `61/61` tube/jar/Alembic/smelter/transfuser/label/phial/caster-control/Void-Jar/essentia-mirror runtime boundary, the `12/12` normal mirror/hand mirror behavior boundary, the `19/19` Thaumatorium server-machine and recipe-selection screen boundary, the `11/11` Arcane Spa/Everfull Urn alchemy automation device boundary, the focused Bellows device/rendering audit, the audited default infusion cycle through output placement, the first focus/caster/Focal Manipulator core, the audited `ROOT -> TOUCH -> FIRE` cast execution slice, the `71/71` custom entity foundation slice including Pech state/trade and rift-owned FocusCloud foundations, the `32/32` special alchemy item/projectile/fluid/effect behavior slice, the `11/11` Flux/Bore/Thaumatorium blocker slice, the `21/21` Flux Rift consequence slice, the `11/11` Wisp behavior/render contract slice, the `34/34` Wisp/Angry Zombie/Firebat/Pech spawn-policy slice, the `13/13` Flux Goo/Taint Fibre blocker slice, the `15/15` TaintSeed/terrain ecology blocker slice, the `15/15` taint mob/Thaumic Slime server-foundation slice, and the `10/10` FallingTaint crust-physics slice. | Keep these covered by build/server/client smoke checks before expanding consumers. |
...
| Blocked | Measured Thaumonomicon browser/entry pixel parity, final Arcane Workbench GUI polish, optional Curios/accessory slot adapter wiring, final robe/goggles armor model geometry, special void-jar stack-copy behavior, Spa GUI/Botania support, exact LiquidDeath/PurifyingFluid/Spa client fluid particles/render translucency, exact BottleTaint/Wisp/Firebat/Pech/FallingTaint/entity render pixel parity, exact Guardian/orb/cultist/portal renderer pixel parity, final taint mob renderer/particle pixel parity, Thaumcraft-biome worldgen natural-spawn expansion, measured infusion/mirror/Spa/Thaumatorium/rift visual parity, WarpEvents client hallucination/stress visuals, fortress mask mitigation, custom projectile/construct/golem AI/renderers, remaining player-authored focus effect execution/projectiles/clouds/mines, Focus Pouch GUI and ScanSky celestial-note side effects. | These need focused design and validation slices. |
| Next subsystem | Continue non-focus blocker removal from `06_docs/migration/remaining_subsystem_unblock_plan.md`: likely next choices are remaining alchemy automation families, taint/rift visual parity, measured machine/entity render parity, or the next focused entity/projectile/device family. | Focus/caster row 7 is paused by user request; do not expand medium/effect execution until explicitly resumed. |
...
| Check | Result | Notes |
|---|---|---|
| `.\gradlew.bat build --no-daemon` | Passed | Latest Thaumonomicon UI parity repair builds successfully. Re-run after each recipe, UI, menu, networking, gameplay or asset expansion batch. |
| Thaumonomicon protocol audit exporter, latest | Passed | `.\gradlew.bat runServer --no-daemon "-PtcThaumonomiconProtocolAudit=true" "-PtcThaumonomiconProtocolAuditPath=build/audits/thaumcraft_1_21_thaumonomicon_protocol_audit.md"` wrote `05_neoforge_port/run/build/audits/thaumcraft_1_21_thaumonomicon_protocol_audit.md` and passed `48/48` checks. Added coverage validates creative locked-start behavior, legacy start visibility for Artifice/Golemancy discovery entries, legacy-recursive debug `research all`, new flag surfacing, blank empty knowledge totals and inventory-style stack tooltips. |
...
| Thaumatorium behavior audit exporter | Passed | `.\gradlew.bat runServer --no-daemon -PtcThaumatoriumBehaviorAudit=true "-PtcThaumatoriumBehaviorAuditPath=..\\..\\06_docs\\audits\\generated\\thaumcraft_1_21_thaumatorium_behavior_audit.md"` reached `Done`, wrote the audit, and stopped automatically. Current `19/19` checks validate the two-block Block/BlockEntity identity, hidden importer/exporter alias policy, heat source two blocks below, redstone pause, sided capabilities/top delegation, `thaumcraft:alumentum` recipe fixture, clean fake-player research-gated recipe list exposure, server-side select/remove toggle validation, suction `128`, manual input clamping, one-point tube pull, completion/ejection and the current mnemonic-matrix placeholder slot bonus. |
...
| Entity foundation audit exporter | Passed | `.\gradlew.bat runServer --no-daemon -PtcEntityFoundationAudit=true "-PtcEntityFoundationAuditPath=..\\..\\06_docs\\audits\\generated\\thaumcraft_1_21_entity_foundation_audit.md"` reached `Done`, wrote the audit, and stopped automatically. Current `71/71` checks validate the full 43-entry legacy catalog remains explicit and the registered foundation set now includes FocusCloud, Pech state/pack/trade/menu/wand/scan/aspect contracts, Firebat, Angry/Furious Zombie contracts plus `thaumcraft:causality_collapser` with TC6 tracking/update/velocity metadata. |
| Flux Rift consequence audit exporter | Passed | `.\gradlew.bat runServer --no-daemon -PtcFluxRiftConsequenceAudit=true "-PtcFluxRiftConsequenceAuditPath=..\\..\\06_docs\\audits\\generated\\thaumcraft_1_21_flux_rift_consequence_audit.md"` reached `Done`, wrote the audit, and stopped automatically. Current `21/21` checks validate the TC6 rift event table, Wisp dependency, Prime TaintSeed, infectious vis exhaustion, rift-owned FocusCloud event execution, cloud damage/cooldown behavior, collapse effects, and Causality Collapser item/projectile/rift-collapse AABB behavior. |
...
| Taint mob blocker audit exporter | Passed | `.\gradlew.bat runServer --no-daemon -PtcTaintMobBlockerAudit=true "-PtcTaintMobBlockerAuditPath=..\\..\\06_docs\\audits\\generated\\thaumcraft_1_21_taint_mob_blocker_audit.md"` reached `Done`, wrote the audit, and stopped automatically. Current `15/15` checks validate Thaumic Slime, TaintCrawler, Taintacle, Tiny Taintacle, Giant Taintacle and TaintSwarm ids/tracking/type contracts, legacy attribute baselines, explicit custom entity aspect assignments, scan keys, crawler fibre trail, feature break crawler spawn, geyser swarm spawn, tiny taintacle spawn, swarm NBT and Thaumic Slime size/xp. |
...
| Entity spawn-policy audit exporter | Passed | `.\gradlew.bat runServer --no-daemon -PtcEntitySpawnPolicyAudit=true "-PtcEntitySpawnPolicyAuditPath=..\\..\\06_docs\\audits\\generated\\thaumcraft_1_21_entity_spawn_policy_audit.md"` reached `Done`, wrote the audit, and stopped automatically. Current `34/34` checks validate the active Wisp Nether row, Angry Zombie overworld row and Firebat Nether/Halloween rows, their NeoForge biome modifiers, land-biome tags, placement types, dark-cell success paths, light roll/config/date gates and peaceful denial. |
...
| Essentia transport behavior audit exporter | Passed | `.\gradlew.bat runServer --no-daemon -PtcEssentiaTransportBehaviorAudit=true "-PtcEssentiaTransportBehaviorAuditPath=D:\Thaumcraft_6_port_to_1.21.1\06_docs\audits\generated\thaumcraft_1_21_essentia_transport_behavior_audit.md"` reached `Done`, wrote the curated generated audit, and stopped automatically. Current `61/61` checks confirm the combined tube/jar/Alembic/smelter/transfuser/Void-Jar/essentia-mirror contract still passes after the row-4 utility device closure. |
...
| Warp event behavior audit exporter | Passed | `.\gradlew.bat runServer --no-daemon -PtcWarpEventBehaviorAudit=true "-PtcWarpEventBehaviorAuditPath=..\..\06_docs\audits\generated\thaumcraft_1_21_warp_event_behavior_audit.md"` reached `Done`, wrote `06_docs/audits/generated/thaumcraft_1_21_warp_event_behavior_audit.md`, reported `35` passed / `0` failed checks, and stopped automatically. Current checks cover the 2000-tick scheduler constants, temporary-warp decay/trigger/counter math, exact legacy outcome thresholds including `73 -> UNNATURAL_HUNGER_LONG` and `76 -> MOMENT_OF_CLARITY`, warp-only potion ids/colors, Guardian/Spider/lesser-portal entity foundation contracts, warp entity aspect contracts, Vis Exhaust/Death Gaze/Unnatural Hunger outcome durations/amplifiers, rotten-flesh relief, normal-food rejection, BATHSALTS/ELDRITCH unlock thresholds, Eldritch Guardian orb projectile path, lesser portal CultistKnight/CultistCleric minion spawning and CultistCleric GolemOrb branch. |
...
8. For the next crucible/Thaumatorium gameplay batch, keep `TCSpecialAlchemyBehaviorAudit` and `TCAlchemyAutomationDeviceAudit` green and audit the remaining automation owner before wiring exact taint mob renderer pixel parity or Thaumcraft-biome spawn expansion, Thaumic Slime polish, additional automation behavior or measured rift/focus-cloud particle pixel parity.
...
Still blocked:
- Exact direct craft-reference hash parity, WarpEvents client hallucination/stress visuals, exact Guardian/orb/cultist/portal renderer pixel parity, measured Thaumonomicon pixel-level polish, exact legacy recipe-output search parity and cancellable research/knowledge events remain open. The real item/open/browser/entry/drilldown flow, legacy-shaped browser search activation/list rendering, vanilla/arcane/crucible/infusion/blueprint/fake-display page snapshots, legacy-style recipe tabs, legacy-shaped crafting/arcane/crucible/infusion/fake-display paper-page geometry, aspect/knowledge side inserts, selected-stage warp warning, permanent page catalog, server-authoritative warp mutation/sync, first server-side WarpEvents behavior slice and server-authoritative index/entry/action/drilldown protocol are implemented; entry rewards and addendum notifications are implemented, but built-in TC6 data does not provide a real reward integration fixture.
...
- Superseded current result: protocol audit now passes `48/48`; current research page catalog audit reports `203 READY`, `0 DEFERRED`, `0 LEGACY_MISSING`.
...
- `audit-crucible-recipe-data.ps1` now validates `78/78` crucible JSON files.
