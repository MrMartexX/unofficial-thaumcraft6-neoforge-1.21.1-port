...
| Thaumonomicon protocol audit exporter | Passed | `.\gradlew.bat runServer --no-daemon "-PtcThaumonomiconProtocolAudit=true" "-PtcThaumonomiconProtocolAuditPath=build/audits/thaumcraft_1_21_thaumonomicon_protocol_audit.md"` reached `Done`, wrote `05_neoforge_port/run/build/audits/thaumcraft_1_21_thaumonomicon_protocol_audit.md`, and passed `43/43` checks. New coverage validates legacy open-time known-sibling completion and server-owned recipe-output search aliases in addition to existing authoritative index/entry/action/drilldown checks. |
...
- Superseded current result: protocol audit now passes `43/43`; current research page catalog audit reports `203 READY`, `0 DEFERRED`, `0 LEGACY_MISSING`.
