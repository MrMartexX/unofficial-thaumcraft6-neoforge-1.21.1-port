...
Last reviewed checkpoint: `2026-06-20` exact infusion structure and stability/instability boundary
...
- The `thaumcraft:infusion` boundary has `42/42` valid JSON recipes, page snapshots, active matrix/pedestal BlockEntities, all three pillar variants, both matrix modifier stones and a real `jar_normal` source container. The server persists structure-derived timing/cost/replenishment, stability, remaining aspects/components/countdown, drains one aspect point nearest-first, waits 200 ticks after a failed source pass, spends six cycles per component, preserves component remainders, carries damaged-catalyst ratio to an undamaged damageable result, completes on the following cycle, and emits completion/failure sounds. Candle/skull/pedestal symmetry follows legacy diminishing-return and mismatch rules. Exact stability categories/loss/clamp/trigger math and all 24 event mappings are active; 18 rolls execute reviewed real effects and six dependency-owned rolls fail closed. Runtime audit passes `83/83`.
- Clientbound equivalents of legacy `PacketFXEssentiaSource` and `PacketFXInfusionSource` now preserve the 32-block send range, source fields, 15/60-tick cache lifetimes and client-only authority boundary. The current billboard rendering is a bridge; exact polycone essentia streams and item/block debris particles are not yet visually final.
...
- Special alchemy side effects, crucible-derived aspect generation, item pulling radius, broad Thaumatorium/alembic/mirror integration, the six Flux Goo/custom-effect instability rolls, pedestal instability mitigation, live surroundings invalidation, exact infusion stream/debris rendering, full custom entity/golem systems, broad worldgen, broad rendering polish, full equipment/Curios discount integration, and remaining dependency-heavy recipe/page families remain deferred.
...
|---|---|---|
| Ready | Aspect/scan parity harnesses, reload-safe research data, current Thaumometer scan path, knowledge sync, research table and first server-authoritative Thaumonomicon flow, current exact Arcane Workbench recipes/behavior, crucible behavior slices, infusion recipe/page data, persistent Warded Jar storage, and the audited default infusion cycle through output placement. | Keep these covered by build/server/client smoke checks before expanding consumers. |
| Placeholder / Bridge | Remaining research requirement bridges, unfinished advanced theorycraft cards/aids, minimal warp storage, unresolved machine/entity identities, and the infusion FX billboard renderer behind the exact payload/cache contract. Research bridge recipes, generated parity reports, and debug-only completion commands remain tooling. | These are scaffolds for validation and migration. Do not treat them as final gameplay or visual parity. |
| Blocked | Thaumonomicon final search/visual parity, recipe drilldown/history, final Arcane Workbench GUI polish, full equipment/Curios discount integration, remaining arcane recipes, player-facing infusion activation, Flux Goo/custom harm instability effects, pedestal mitigation, exact stream/debris renderers, blueprint/fake recipe systems, full warp effects/sync, broad essentia networks, custom entities, caster/mirror behavior, and ScanSky celestial-note side effects. | These need focused design and validation slices. |
| Next subsystem | Continue infusion with exact Flux Goo and Flux Taint/Vis Exhaust dependencies, pedestal mitigators and live surroundings refresh, then audit safe player activation. | Stability and 24-way event selection are complete; do not enable caster-start gameplay while six event rolls remain dependency-blocked. |
...
| Check | Result | Notes |
|---|---|---|
| `.\gradlew.bat build --no-daemon` | Passed | Latest infusion stability/instability boundary batch builds successfully. Re-run after each recipe, UI, menu, networking, gameplay or asset expansion batch. |
...
| `tools/ci/server-smoke.ps1 -TimeoutSeconds 420 -WorldName tc_server_smoke_infusion_stability -ServerPort 0 -KillStaleRunServer` | Startup/reload smoke reached `Done` | Dedicated server reached `Done`; bundled bootstrap, data-resource assignment reload, generated crafting/arcane cache rebuild, tag reload validation, research data reload, aura event/command registration, automatic aura chunk initialization, and the client-only tooltip registration boundary all passed. Latest smoke loaded `692` exact assignments, `46` tag assignments, `32` complex exact assignments, rebuilt `638` generated cache entries, loaded `7` research categories / `148` entries / `271` stages / `16` addenda, and reported `1230 of 1230` assignable minecraft item ids with non-empty aspects. Latest full audit checkpoint also passed the page catalog audit with `113 READY` / `86 DEFERRED` / `4 LEGACY_MISSING`, the Thaumonomicon protocol audit `27/27`, the arcane recipe audit `105/105`, the Arcane Workbench behavior audit `23/23`, the current Crucible behavior audit `12/12`, and the current Infusion behavior audit `83/83`. |
...
| Infusion behavior audit exporter | Passed | `tools/audits/audit-infusion-behavior.ps1` passes `83/83`. In addition to recipe/start/readiness checks it verifies real jar discovery/drain, source ordering/range/blocking, persistent cycle/stability NBT, structure-derived cadence/cost, ancient/eldritch and matrix-stone modifiers, pedestal/candle symmetry and diminishing returns, exact stability thresholds/loss/clamp/trigger math, all 24 event mappings, fail-closed missing dependencies, one-point drains, six-cycle component timing, 63-cycle Cloud Ring completion, damaged-result ratio, 200-tick failed-source rescan delay, component remainders and catalyst-change event-before-abort ordering. |
...
| Gate 4+ | Aura is started as an isolated server-side storage/API slice after `aura_design.md`; the first research-table BlockEntity/menu/screen boundary exists; the first custom arcane recipe and Arcane Workbench server crafting boundary exists, including Workbench Charger 3 x 3 aura use and the first player vis-discount path; the first crucible recipe serializer/page snapshot boundary and current manual/collision/spill in-world crucible behavior slices exist; the infusion recipe/page plus active matrix/pedestal structure, cycle and stability boundary exists. Capabilities, broad machine networks, full alchemy side effects, remaining infusion event dependencies/player activation/visual FX, broad networking, worldgen, and large rendering systems remain not started. |
