...
Last reviewed checkpoint: `2026-06-14` Shobie safe data merge expansion checkpoint
...
| `.\gradlew.bat runServer --no-daemon` | Startup/reload smoke reached `Done` through the Shobie safe data merge expansion | Dedicated server reached `Done`; bundled bootstrap, data-resource assignment reload, generated crafting/arcane/crucible/infusion cache rebuild, tag reload validation, research data reload, aura event/command registration, automatic aura chunk initialization, and the client-only tooltip registration boundary all passed. Latest smoke loaded `688` exact assignments, `46` tag assignments, `32` complex exact assignments, `1533` recipes, rebuilt `620` generated cache entries, loaded `7` research categories / `148` entries / `271` stages / `16` addenda, and reported `1230 of 1230` assignable minecraft item ids with non-empty aspects. Latest full audit checkpoint also passed the page catalog audit with `17 READY` / `178 DEFERRED` / `8 LEGACY_MISSING`, the Thaumonomicon protocol audit `25/25`, the prior arcane recipe audit `105/105` over the hand-audited exact fixture set, and the Arcane Workbench behavior audit `23/23`. |
...
| Generated crafting assignments | `620` | Built after server data/tag reload from current `RecipeType.CRAFTING`, current `thaumcraft:arcane`, current `thaumcraft:crucible`, and current `thaumcraft:infusion` recipes for `minecraft:*` and `thaumcraft:*` outputs that have known ingredient/catalyst/central-component aspects. Exact/tag/manual/runtime-parity assignments still win over generated values. The current count includes remaining safe research bridge recipes used to make modern `required_craft` markers observable, exact normal plate recipes, translated Shobie arcane data including `tube`, `resonator`, and `sanitychecker`, Shobie-derived material/equipment recipes, 41 translated crucible recipes, and 38 translated infusion recipes; bridge recipes and placeholder equipment/custom-crafting recipes are not final gameplay implementations. |
