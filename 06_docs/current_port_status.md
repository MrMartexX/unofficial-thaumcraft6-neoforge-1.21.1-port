...
- Special alchemy side effects, crucible-derived aspect generation, item pulling radius, broad Thaumatorium/alembic/mirror integration, finite Flux Goo spreading/taint transforms, Flux Rift stabilization, live infusion surroundings invalidation, exact infusion stream/debris rendering, full custom entity/golem systems, broad worldgen, broad rendering polish, full equipment/Curios discount integration, and remaining dependency-heavy recipe/page families remain deferred.
...
| Blocked | Thaumonomicon final search/visual parity, recipe drilldown/history, final Arcane Workbench GUI polish, full equipment/Curios discount integration, remaining arcane recipes, full Flux Goo flow/taint transforms, exact matrix/stream/debris renderers, blueprint/fake recipe systems, full warp effects/sync, broad essentia networks, custom entities, caster/mirror behavior, and ScanSky celestial-note side effects. | These need focused design and validation slices. |
| Next subsystem | Continue infusion with a focused Stage 13 matrix animation, polycone stream and item/block debris renderer batch. | Server gameplay activation/completion now passes `93/93`; rendering must consume synced display state only. |
...
| Check | Result | Notes |
|---|---|---|
| `.\gradlew.bat build --no-daemon` | Passed | Latest infusion activation/live-surroundings batch builds successfully. Re-run after each recipe, UI, menu, networking, gameplay or asset expansion batch. |
...
| `runServer` plus focused infusion audit | Passed | Dedicated server reached `Done`; aspect bootstrap/reload validated `694` exact assignments, `46` tag assignments and `32` complex exact assignments, and coverage remained `1230/1230`. The focused infusion exporter completed and stopped cleanly with `93/93` checks. |
...
| Infusion behavior audit exporter | Passed | The focused exporter passes `93/93`. It verifies all 24 executable event dependencies, Flux Goo placement, inlay charge attenuation, Stabilizer NBT/pre-mutation ejection absorption, two-click activation readiness, idle charging, live modifier refresh, active-state sync and invalid-structure deactivation. |
