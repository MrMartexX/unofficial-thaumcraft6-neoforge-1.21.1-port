...
| Next subsystem | Arcane Workbench parity closure and recipe expansion planning: in-game matrix/crystal/vis validation, GUI cost/slot feedback, player vis discount design, Workbench Charger 3x3 aura design, then the next small exact arcane recipe family. | Do not mass-import arcane recipes or generate arcane-derived aspects until the workbench service, aura source behavior, and ingredient mappings are audited. |
...
7. Implement arcane recipe-derived `generateTags` behavior from `aspect_generate_tags_audit.md` after the Arcane Workbench service is visually/runtime validated; keep crucible and infusion recipe-derived generation blocked until their serializers and machines exist.
