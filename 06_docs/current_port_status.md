...
| Placeholder / Bridge | Remaining legacy requirement bridges for focus/caster/vis, unfinished advanced theorycraft cards/aids, crucible/smelter/nitor, infusion matrix, thaumium tools, mirror, curio, zombie brain, OreDictionary chest and flattened vanilla metadata. The current XP/phial/vanilla-aid/Golemancy theory cards are still bridge-level gameplay until the full table UI/reward loop is finished. Generated parity reports and debug-only research completion commands are also bridge tooling. | These are scaffolds for validation and migration. Do not treat them as final gameplay, reward, recipe, or subsystem behavior. |
...
| Next subsystem | Theorycraft advanced cards/aids by dependency family. | The first vanilla aid family now proves the start-theory aid path. The next safe step is more theory cards whose dependencies are already present, followed by table-top rendering and legacy card animation polish. |
...
| `.\gradlew.bat runClient --no-daemon` | Startup/resource smoke reached atlas creation | Client reached resource loading, aspect assignment reload, generated cache rebuild, tag validation and texture atlas creation. The session was stopped manually after startup; latest reviewed log had no Thaumcraft missing texture/model errors. Remaining warnings were vanilla/NeoForge asset URL schema noise, missing vanilla goat horn sounds, and one vanilla shader sampler warning. |
...
12. Visually check vanilla aid selection in `runClient`; then port remaining theorycraft cards/aids only by audited dependency family. Advanced cards that require warp, curios, focus/caster, infusion, celestial notes, portals, or other unported systems must stay deferred or explicitly bridged.
