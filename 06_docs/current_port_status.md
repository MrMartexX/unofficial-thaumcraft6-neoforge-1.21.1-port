...
| Placeholder / Bridge | Remaining legacy requirement bridges for focus/caster/vis, unfinished advanced theorycraft cards/aids, crucible/smelter/nitor behavior, infusion matrix behavior, Alumentum/Salis Mundus behavior, special Thaumium tool behavior, mirror, curio, zombie brain behavior, OreDictionary chest and flattened vanilla metadata. The current XP/phial/vanilla-aid/Golemancy/table-inventory/Artifice/basic-Infusion theory cards are still bridge-level gameplay until the full table UI/reward loop is finished. Generated parity reports and debug-only research completion commands are also bridge tooling. | These are scaffolds for validation and migration. Do not treat them as final gameplay, reward, recipe, or subsystem behavior. |
...
| Next subsystem | Theorycraft advanced cards/aids by dependency family. | The first vanilla aid family, basic block aids, Artifice item-option cards, `CardInfuse`, and `CardScripting` prove start-theory aid, required-item card, and table-inventory card paths. The next safe step is either Basic Auromancy after `CardAwareness`/warp policy is audited, or a narrow Thaumonomicon/progression UI slice before heavier alchemy/infusion gameplay. |
