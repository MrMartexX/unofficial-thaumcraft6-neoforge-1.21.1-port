...
Last reviewed checkpoint: `2026-07-11` TaintSeed and taint ecology blocker slice
...
- Both active item-entity types use the vanilla item entity renderer for now. The remaining 41 legacy custom entities are intentionally classified as deferred by owning subsystem: flux/rifts, taint, projectiles, focus projectile/cloud/mine, constructs/turrets/bore, Eldritch/Crimson mobs and golems.
...
- Current validation: `TCEntityFoundationAudit` passes `11/11` and writes `06_docs/audits/generated/thaumcraft_1_21_entity_foundation_audit.md`. The focused Flux/Bore/Thaumatorium blocker audit passes `11/11` and writes `06_docs/audits/generated/thaumcraft_1_21_flux_bore_thaumatorium_blocker_audit.md`; the focused taint ecology blocker audit passes `15/15`. This does not claim remaining mob AI, spawn rules, custom mob scan/aspect parity, full Flux Rift event consequences, full turrets or golem logistics parity.
...
- Current validation: `TCFluxTaintBlockerAudit` passes `13/13` and writes `06_docs/audits/generated/thaumcraft_1_21_flux_taint_blocker_audit.md`; `TCTaintEcologyBlockerAudit` passes `15/15` and writes `06_docs/audits/generated/thaumcraft_1_21_taint_ecology_blocker_audit.md`. Full TaintCrawler/TaintSwarm/Taintacle AI/renderers, FallingTaint crust physics, GORE sound parity, Thaumic Slime spawning and broad rift consequences are still deferred.
...
- Special alchemy side effects, crucible-derived aspect generation, item pulling radius, exact Thaumatorium model/renderer/pixel parity, special void-jar stack-copy crafting, Flux Rift stabilization/consequences, TaintCrawler/TaintSwarm/Taintacle/FallingTaint/Thaumic Slime behavior, custom mob/projectile/construct/golem AI and renderers, broad worldgen, broad rendering polish, full equipment/Curios discount integration, remaining dependency-heavy recipe/page families, and final measured valve/vent/mirror/Thaumatorium visual parity remain deferred.
...
| Aura | Started with server-side core | `AuraHelper`, per-level `SavedData`, chunk `base/vis/flux`, automatic chunk initialization, legacy formula for aura base generation, main-thread 20-tick legacy-like update loop, permission-level-2 debug commands, Flux Rift entity consumer/source bridge, Flux Goo/Taint Fibre and first TaintSeed/terrain ecology consumers are implemented. The biome category mapper is legacy-like because 1.21 has no `BiomeDictionary`. HUD sync, broad aura FX, research-aware preservation, full rift consequences and taint mob ecology remain intentionally narrow or deferred. |
...
| Entities/golems | Foundation started | `TCEntityTypes` registers the first two non-mob item-entity types, `special_item` and `follow_item`, plus focused `flux_rift`, `arcane_bore`, `taint_seed` and `taint_seed_prime` foundations, and keeps the full 43-entry legacy entity catalog explicit for future batches. Runtime entity foundation audit passes `11/11`; focused Flux/Bore/Thaumatorium blocker audit passes `11/11`; taint ecology blocker audit passes `15/15`. Remaining mob AI, spawn rules, broad custom renderers, focus projectile/cloud/mine entities, full turrets and golems remain deferred. |
...
| Blocked | Thaumonomicon final search/visual parity, recipe drilldown/history, final Arcane Workbench GUI polish, optional Curios/accessory slot adapter wiring, final robe/goggles armor model geometry, special void-jar stack-copy behavior, taint mob/FallingTaint/Thaumic Slime and rift event consequences, measured infusion/mirror/Thaumatorium visual parity, blueprint/fake recipe systems, full warp effects/sync, custom mob/projectile/construct/golem AI/renderers, remaining focus effect execution/projectiles/clouds/mines, Focus Pouch GUI and ScanSky celestial-note side effects. | These need focused design and validation slices. |
| Next subsystem | Continue non-focus blocker removal from `06_docs/migration/remaining_subsystem_unblock_plan.md`: likely next choices are taint mob ecology/Thaumic Slime, Thaumonomicon final navigation/page renderers, crucible special alchemy/automation, or the next focused entity/projectile/device family. | Focus/caster row 7 is paused by user request; do not expand medium/effect execution until explicitly resumed. |
...
| Check | Result | Notes |
|---|---|---|
| `.\gradlew.bat build --no-daemon` | Passed | Latest Thaumatorium recipe-selection UI batch builds successfully. Re-run after each recipe, UI, menu, networking, gameplay or asset expansion batch. |
| `.\gradlew.bat runClient --no-daemon` | Startup/resource smoke reached client resource reload and atlas creation | Client reached mod bootstrap, resource loading, sound engine startup, and texture atlas creation after the focus/caster core batch. The process was stopped intentionally after the smoke window, before world-join gameplay testing. Latest reviewed log had no Thaumcraft `ERROR`, `FATAL`, or crash stacktrace. It still reports known deferred missing model/texture warnings for unported item families such as Brain-in-a-Jar, blueprints, masks, fortress armor, lamps, golem seals and other not-yet-owned systems; these are not introduced by the focus/caster core. Remaining benign warnings include vanilla/NeoForge asset URL schema noise, missing vanilla goat horn sounds, and one vanilla shader sampler warning. |
...
Do not implement or expand aura, research, arcane crafting, crucible/alchemy, infusion, BlockEntities, menus, networking, worldgen, or large rendering systems by direct legacy class copying. Each one needs a small design document first. Current arcane crafting work is limited to the documented exact-recipe/Arcane Workbench server slice; current crucible work is limited to the documented manual/collision/spill/client-FX boundary plus focused Flux Goo/TaintSeed world-mutation slices and must not grow further into full rift consequences, taint mobs/FallingTaint/Thaumic Slime, item pulling radius, special alchemy or essentia networks without a new focused slice; current aspect work is limited to the documented data layer and read-only inventory tooltip consumer.
...
8. For the next crucible/Thaumatorium gameplay batch, audit legacy alchemy side-effect and Thaumatorium machine calls before wiring full flux-rift consequences, taint mobs/FallingTaint/Thaumic Slime, liquid death or automation behavior.
