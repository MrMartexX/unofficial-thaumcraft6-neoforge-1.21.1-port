...
Last reviewed checkpoint: `2026-07-13` Pech entity/trade foundation
...
- Current validation: `TCFluxTaintBlockerAudit` passes `13/13` and writes `06_docs/audits/generated/thaumcraft_1_21_flux_taint_blocker_audit.md`; `TCTaintEcologyBlockerAudit` passes `15/15`; `TCTaintMobBlockerAudit` passes `14/14`; `TCFallingTaintBlockerAudit` passes `10/10`; `TCFluxRiftConsequenceAudit` passes `16/16`; `TCWispBehaviorAudit` passes `11/11`; `TCEntitySpawnPolicyAudit` passes `29/29` for the Wisp Nether, Angry Zombie overworld and Firebat Nether/Halloween rows; `TCEntityFoundationAudit` passes `57/57` including Pech. Full custom taint mob renderers/animations, exact Wisp/Firebat/Pech/FallingTaint landing particles/render pixel parity, Pech magical-biome/Thaumcraft-biome spawn placement and Flux Rift focus-cloud event execution are still deferred.
...
| Entities/golems | Foundation started | `TCEntityTypes` registers the first two non-mob item-entity types, `special_item` and `follow_item`, plus focused `alumentum`, `causality_collapser`, `bottle_taint`, `eldritch_orb`, `golem_orb`, `flux_rift`, `arcane_bore`, `falling_taint`, `taint_seed`, `taint_seed_prime`, `wisp`, `firebat`, `pech`, `brainy_zombie`, `giant_brainy_zombie`, `thaum_slime`, `taint_crawler`, `taintacle`, `taintacle_tiny`, `taint_swarm`, `cultist_portal_lesser`, `cultist_knight`, `cultist_cleric`, `mind_spider` and `eldritch_guardian` foundations, and keeps the full 43-entry legacy entity catalog explicit for future batches. Runtime entity foundation audit passes `57/57` checks including Pech state/pack/trade/menu/wand/scan/aspect contracts; focused WarpEvents behavior audit passes `35/35`; focused Special Alchemy behavior audit passes `32/32`; focused Flux/Bore/Thaumatorium blocker audit passes `11/11`; Flux Rift consequence audit passes `16/16`; Wisp behavior/render contract audit passes `11/11`; entity spawn-policy audit passes `29/29` for Wisp Nether, Angry Zombie overworld and Firebat Nether/Halloween; taint ecology blocker audit passes `15/15`; taint mob blocker audit passes `14/14`; FallingTaint blocker audit passes `10/10`. Remaining Pech magical-biome/Thaumcraft-biome spawn placement, exact measured entity render pixel parity, exact Alumentum/Causality fire-mote pixel parity, exact Guardian/orb/cultist/portal/Pech renderer pixel parity, focus projectile/cloud/mine entities, full turrets and golems remain deferred. |
