...
Last reviewed checkpoint: `2026-07-01` item/equipment behavior closure
...
- `TCEquipmentHelper` provides a no-hard-dependency bridge for armor plus future accessory-provider stacks. The item/equipment runtime audit passes `17/17`; final armor model geometry, Curios slot integration, focus/caster behavior, recharge logic and bauble-specific movement/status effects remain separate subsystem work.
...
| Next subsystem | Start row 7 from `06_docs/migration/remaining_subsystem_unblock_plan.md`: Focus/caster/Focal Manipulator core. | Use the closed item/equipment contracts and Arcane Workbench/aura foundations; keep cast effects behind server-authoritative data/payload audits. |
...
| Check | Result | Notes |
|---|---|---|
| `.\gradlew.bat build --no-daemon` | Passed | Latest item/equipment behavior batch builds successfully. Re-run after each recipe, UI, menu, networking, gameplay or asset expansion batch. |
