...
Last reviewed checkpoint: `2026-07-02` Thaumatorium recipe-selection UI slice
...
| Next subsystem | Continue non-focus blocker removal from `06_docs/migration/remaining_subsystem_unblock_plan.md`: exact Thaumatorium visual/model parity can be handled under row 17, so the next implementation batch can move to row 13 standalone device blockers if visual polish is deferred. | Focus/caster row 7 is paused by user request; do not expand medium/effect execution until explicitly resumed. |
