...
- Thaumonomicon protocol audit: `48/48` checks passed. The first real item/open/browser/entry/search/drilldown flow, server-snapshot-backed vanilla crafting page renderer, exact arcane page renderer, crucible page renderer, infusion page renderer, blueprint construct page renderer, fake/display page renderer, legacy-style recipe tabs, side-panel knowledge/aspect state, selected-stage warp propagation, runtime PNG category background paths, open-time known-sibling completion, server-owned recipe-output search aliases, server-built index revision, stale-action/stale-drilldown rejection without mutation, server-side recipe output-stack drilldown resolution, creative locked-start behavior, legacy start visibility, legacy-recursive debug `research all`, new flag surfacing, empty knowledge totals and inventory-style stack tooltips are active. Catalog live availability is `203 READY`, `0 DEFERRED`, `0 LEGACY_MISSING`; measured pixel-level polish remains row-17 work.
