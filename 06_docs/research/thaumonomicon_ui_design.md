...
- Latest protocol result: `43/43` checks passed; live vanilla crafting, arcane, crucible, infusion, blueprint and fake/display catalog entries produce valid server snapshots, runtime category backgrounds use PNG texture paths, and recipe search plus recipe drilldown/history validate server-side output-stack resolution, client-cache acceptance and stale-revision rejection.
