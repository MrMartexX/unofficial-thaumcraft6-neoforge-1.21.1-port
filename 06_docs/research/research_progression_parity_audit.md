...
- Server-authoritative warp mutation/sync and the current item-warp crafting bridge are active. Full random `WarpEvents` symptoms/hallucinations and their broader consequences remain outside this progression parity slice.
...
- Server-authoritative Thaumonomicon index/entry/action/drilldown payloads, item/open flow, first browser/entry/search screens, recipe drilldown/history, current-scope entry UI, and vanilla crafting, arcane, crucible, infusion, blueprint and fake/display page snapshots/renderers are implemented and the protocol audit passes `48/48`. The audit covers exact legacy browser start, checked stage advance, entry acknowledgement flag semantics and known-entry final-stage progression, legacy open-time known-sibling completion, visibility filtering, server-owned unlockability/flags, client-cache invalidation, server-built revision freshness, stale-action and stale-drilldown rejection without mutation, explicit-open-versus-refresh separation, valid server snapshots for every live crafting/arcane/crucible/infusion/blueprint/fake-display catalog entry, runtime PNG category background paths, selected-stage warp propagation, side-panel knowledge/aspect cache state, server-owned recipe-output search aliases, server-side output-stack drilldown resolution, creative locked-start behavior, initial Artifice/Golemancy visibility, legacy-recursive debug `research all`, new flag surfacing, empty knowledge totals and inventory-style stack tooltips. Browser search filters only the server-visible index plus server-owned recipe output aliases and reuses the same action payload path as graph clicks. The remaining boundary is measured pixel-level browser/entry polish and the cheat variant.
