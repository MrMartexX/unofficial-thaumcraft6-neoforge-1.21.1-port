...
- Server-authoritative Thaumonomicon index/entry/action/drilldown payloads, item/open flow, first browser/entry/search screens, recipe drilldown/history, current-scope entry UI, and vanilla crafting, arcane, crucible, infusion, blueprint and fake/display page snapshots/renderers are implemented and the protocol audit passes `43/43`. The audit covers exact legacy browser start, checked stage advance, entry acknowledgement flag semantics and known-entry final-stage progression, legacy open-time known-sibling completion, visibility filtering, server-owned unlockability/flags, client-cache invalidation, server-built revision freshness, stale-action and stale-drilldown rejection without mutation, explicit-open-versus-refresh separation, valid server snapshots for every live crafting/arcane/crucible/infusion/blueprint/fake-display catalog entry, runtime PNG category background paths, selected-stage warp propagation, side-panel knowledge/aspect cache state, server-owned recipe-output search aliases and server-side output-stack drilldown resolution. Browser search filters only the server-visible index plus server-owned recipe output aliases and reuses the same action payload path as graph clicks. The remaining boundary is measured pixel-level browser/entry polish and the cheat variant.
