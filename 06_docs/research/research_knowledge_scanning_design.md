...
- The permanent research recipe/page catalog, server-authoritative Thaumonomicon protocol, real item/open flow, first browser/entry/search/drilldown screens, current-scope entry UI, and vanilla crafting, arcane, crucible, infusion, blueprint and fake/display page renderers are implemented. The catalog preserves `253` research occurrences, `203` direct references, and `325` entries including group members with `0` parity differences; the protocol audit passes `43/43`, including the legacy known-entry final-stage progression path, legacy open-time known-sibling completion, explicit-open-versus-refresh separation, server-built index revision, stale-action and stale-drilldown rejection without mutation, server-owned crafting/arcane/crucible/infusion/blueprint/fake-display snapshots, runtime PNG category background paths, selected-stage warp propagation, side-panel knowledge/aspect cache state, server-owned recipe-output search aliases and recipe output-stack drilldown resolution.
