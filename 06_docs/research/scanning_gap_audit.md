...
Guide rule applied: scanning must stay server-authoritative, data-driven where practical, and must not depend on client-only rendering or legacy `SimpleNetworkWrapper`. Scan-key mutation now goes through the current server research progression layer. Observation rewards, research progression and the focused warp mutation/sync bridge are active; celestial-note side effects, non-observation scan rewards, full random `WarpEvents`, and final Thaumonomicon UI polish remain blocked until their subsystems exist.
...
|---|---|---|
| Full research progression after scan | Permanent page/catalog resolution, random `WarpEvents`, cancellable events, and UI polish | Research stage storage, flags, parent requisites, `progressResearch`, `completeResearch`, sibling propagation, entry rewards, addendum notifications, exact stage warp awards, focused warp mutation/sync, final knowledge sync, and known-vs-new scan success are active. The permanent recipe/page catalog is implemented; cancellable events, random `WarpEvents` symptoms/hallucinations and final GUI polish are deferred. |
