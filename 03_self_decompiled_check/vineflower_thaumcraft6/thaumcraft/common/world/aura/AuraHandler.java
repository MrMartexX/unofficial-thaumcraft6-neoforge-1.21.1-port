...
public class AuraHandler {
   ...
   public static AuraChunk getAuraChunk(int dim, int x, int y) {
      ...
      addAuraWorld(dim);
      return auras.containsKey(dim) ? auras.get(dim).getAuraChunkAt(x, y) : null;
   }
   ...
   public static boolean shouldPreserveAura(World world, EntityPlayer player, BlockPos pos) {
      return (player == null || ThaumcraftCapabilities.getKnowledge(player).isResearchComplete("AURAPRESERVE"))
         && getVis(world, pos) / getAuraBase(world, pos) < 0.1;
   }
   ...
   public static void generateAura(Chunk chunk, Random rand) {
      ...
      if (BiomeHandler.getBiomeBlacklist(Biome.func_185362_a(bgb)) == -1) {
         ...
         for (int a = 0; a < 4; a++) {
            ...
            Biome bgb2 = chunk.func_177412_p()
               .func_180494_b(new BlockPos((chunk.field_76635_g + dir.func_82601_c()) * 16 + 8, 50, (chunk.field_76647_h + dir.func_82599_e()) * 16 + 8));
