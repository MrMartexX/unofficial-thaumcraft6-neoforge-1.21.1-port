      ...
      if (!knowledge.isResearchComplete(researchkey) && doesPlayerHaveRequisites(player, researchkey)) {
         ...
         if (re != null) {
            ...
            if (re.getStages() != null) {
               ...
               if (re.getStages().length == 1
                  && cs == 0
                  && re.getStages()[0].getCraft() == null
                  && re.getStages()[0].getObtain() == null
                  && re.getStages()[0].getKnow() == null
                  && re.getStages()[0].getResearch() == null) {
                  cs++;
               } else if (re.getStages().length > 1
                  && re.getStages().length <= cs + 1
                  && cs < re.getStages().length
                  && re.getStages()[cs].getCraft() == null
                  && re.getStages()[cs].getObtain() == null
                  && re.getStages()[cs].getKnow() == null
                  && re.getStages()[cs].getResearch() == null) {
                  cs++;
               }
   ...
   private static ResearchEntry parseResearchJson(JsonObject obj) throws Exception {
      ...
      for (JsonElement element : stagesJson) {
         ...
         if (stageObj.has("required_craft")) {
            String[] s1 = arrayJsonToString(stageObj.get("required_craft").getAsJsonArray());
            stage.setCraft(parseJsonOreList(entry.getKey(), s1));
            if (stage.getCraft() != null && stage.getCraft().length > 0) {
               int[] refs = new int[stage.getCraft().length];
               int q = 0;

               for (Object stack : stage.getCraft()) {
                  int code = stack instanceof ItemStack ? createItemStackHash((ItemStack)stack) : ("oredict:" + (String)stack).hashCode();
         ...
         if (stageObj.has("required_knowledge")) {
            String[] sl = arrayJsonToString(stageObj.get("required_knowledge").getAsJsonArray());
            if (sl != null && sl.length > 0) {
               ArrayList<ResearchStage.Knowledge> kl = new ArrayList<>();
               ...
               if (kl.size() > 0) {
                  stage.setKnow(kl.toArray(new ResearchStage.Knowledge[kl.size()]));
               }
            }
         }
